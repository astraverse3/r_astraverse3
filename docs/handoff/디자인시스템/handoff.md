...
- **펼쳐진 그룹은 한 덩어리**: 그룹 헤더 + 서브행에 **같은 `bg-slate-100`** 을 주고, 묶음 위쪽에만 `border-t border-slate-200/80`, 내부 경계는 `border-b-0`으로 지운다. 시안톤(primary) 강조 X.
  > ⚠️ **서브행이 대여섯 줄을 넘는 목록은 예외** — 내부 선을 전부 지우면 **회색 덩어리**가 되어 행이 구분되지 않는다.
  > 그때는 서브행에 `border-t border-slate-200/70`을 둔다(2026-09-04 제품재고 실화면 확인).
  > 서브행이 2~3줄인 원물재고·생산자관리는 선 없이도 읽히므로 원칙대로 지운다.
...
🔴 **단, 서브행이 대여섯 줄을 넘는 목록에서는 내부 선을 남긴다**(`border-t border-slate-200/70`).
제품재고는 한 품종에 서브행이 열 줄 가까이 붙어 선을 다 지우면 **행이 구분되지 않는 회색 덩어리**가 됐다
(2026-09-04 실화면 확인 → `package-row.tsx`는 이 예외를 적용한다).
「내부 선 제거」는 서브행이 2~3줄인 원물재고·생산자관리를 전제로 한 원칙이다.
