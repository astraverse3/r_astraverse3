...
### 도정 포장 입력 소실 방지 — P1~P5 `fix` `c9ed665`

계획서 [plan-포장소실-방지.md](plan/plan-포장소실-방지.md) · 보고서 [report-포장소실방지-2026-09-02.md](report/report-포장소실방지-2026-09-02.md).

🔴 **원인 재특정 — 「동시 편집」이 아니라 「낡은 화면」이었다.** 사람 확인 결과 동시 편집은 아니었고 에러도 안 떴다("정상 입력했는데 나중에 보니 없더라"). 코드를 보니 포장 다이얼로그가 **열릴 때 서버를 다시 읽지 않고** 페이지 로드 스냅샷(`initialOutputs`)만 봤다 — **탭 하나만 오래 켜두면 화면이 낡는다.** 낡은 화면으로 저장하면 `diffPackaging`이 화면에 없던 행을 「지운 것」으로 읽어 조용히 삭제한다(`// 입력에서 빠진 행 = 삭제`). 결번 `#1365`가 「롤백된 INSERT」가 아니라 **「커밋된 뒤 낡은 화면이 지운 행」**으로 설명되고, 에러가 안 떴다는 증언과도 맞는다. ※ 확정은 아님 — 감사로그가 없어 `#1365`의 커밋 여부는 증명 불가.

- **P1 감사로그** — `updatePackagingLogs`가 로그를 하나도 안 남겨 추적이 막혔던 것이 1순위였다. 🔴 삭제 행은 `lotNo`·`productCode`·`productTypeId`까지 **통째로**(복구 가능해야 의미가 있다), 스냅샷은 **삭제 실행 전**에 뜬다. 차단된 저장도·변경 0건도 남긴다.
- **P2 `isClosed` 검증** — 다른 액션(`:65`·`:340`·`:857`)은 이미 막는데 여기만 없었다. 결번 `#1368`이 마감 후 발급된 건.
- **P3 열 때 서버 재조회** 🔴근본 — 새 액션 `getBatchOutputs`. **select·필터·정렬을 `getMillingLogs`의 `outputs`와 일치**시켰다(어긋나면 화면에 없는 행을 지운다 — 결정 #61). 조회 실패를 조용히 넘기지 않고 쓰기 버튼 3개를 막는다. ⚠️deps에서 `initialOutputs`를 뺐다 — 열려 있는 동안 prop이 갱신되면 입력 중인 값을 덮어써 그게 또 「입력 날림」이다.
- **P4 baseline 충돌 감지**(사용자 결정 B안) — `STALE_BASELINE`·`mergeUnseenRows`(순수함수·테스트 9건). `baselineIds`는 **선택 인자가 아니다**(undefined면 옛 동작으로 빠진다). 🔴거부해도 **내 입력은 값도 순서도 그대로**, 남의 줄만 합쳐 배너로 알리고 다시 저장하면 통과 — 거부는 한 번뿐인 확인 단계여야 한다. UI는 **배너+자동병합+amber 강조**(모달은 내용을 가려서, 토스트는 놓쳐서 기각).
- **P5 `count=0` 조용한 누락 금지** — 기존 행이면 저장 중단, 새 줄이면 버리되 알림.
- **실사용 중 발견(기존 결함)** — 화면을 다 비우면 저장·초기화가 **둘 다 비활성**이라 마지막 한 줄을 지울 방법이 없었다. 저장은 「화면에도 서버에도 없을 때」만, 초기화는 **서버 기준**으로 막고, 다 비우고 저장하면 확인 한 번으로 진행.
- 덤: `handleCloseBatch`의 변경 판정 기준을 낡은 prop → `serverOutputs`로 교정(안 그러면 P3 재조회만으로 「변경됨」이 돼 마감마다 헛저장).

**적용 범위는 포장 다이얼로그 하나** — `app/actions/`의 `deleteMany` 전수 조사 결과 「목록 통째 대체」 패턴은 여기뿐이고 나머지는 전부 대상 지정 삭제다(근거는 계획서 §7).

검증: tsc ✅ · test **173/173**(신규 9) ✅ · lint 신규 0 · **브라우저 실왕복 5항목 사용자 확인**(활동로그에 P1·P2·P4 기록 실측).

남은 것: 배너에 작성자 이름 없음(`createdBy` 필드 부재) · 16:00 저장 실패 정체는 미해결(재발 시 P1 로그로 추적) · 타임아웃 방식은 기각(거짓 양성이 곧 입력 날림).

### 규격별 합계 분리 — 800줄 대응 `refactor`

포장 다이얼로그가 861줄이 돼 제한을 넘겼다. 다시 세어 보니 **주석·빈 줄 뺀 실질 코드는 718줄**(주석 94·빈 줄 49) — 늘어난 154줄 중 상당수가 「왜 이렇게 했는지」 주석이라 급하진 않았지만, 여유가 82줄뿐이라 **인터페이스가 제일 좁은 한 덩어리만** 뺐다.

**신규 2**: `spec-summary.tsx`(계산 `computeSpecSummary` + 밴드 UI, **노출 조건을 컴포넌트가 스스로 판단** → 호출부는 한 줄) · `packaging-constants.ts`(`PKG_REMAINDER`·`PKG_TONBAG`·`PACKAGE_TEMPLATES` — 둘 다 쓰는 값을 한쪽에 두면 **순환 import**).

**861 → 806줄** (실질 코드 718 → 669). 전체 기준 6줄이 남지만 억지로 깎지 않았다.

보류: 포장 줄 렌더 JSX(~100줄, props 11개) · `computeLotGroups`(~35줄, `stocks: any[]` 정리가 딸려온다) · outputs 조작 묶음(❌ 서버액션·ref·상태 3개와 얽혀 인자 8~9개짜리 훅이 된다). ⚠️`add-form.tsx`의 동명 배열은 **내용이 달라**(톤백·3kg·잔량 없음) 통합하지 않았다 — 상수 파일 주석에 남김.
...
### 도정 포장 입력 소실 제보 — 1차 추적 `docs` (코드 변경 없음)

보고서 [report-포장소실-추적-2026-09-01.md](report/report-포장소실-추적-2026-09-01.md). **원인 미확정 상태로 조사 중단, 내일 이어감.**

제보(민채→문희준): 배치 #222(박오주/서농22호/백미)에서 **이영명이 14:30에 입력한 톤백이 16:00에 사라져 있었고, 저장도 반복 실패**했다. 민채가 16:02에 재입력해 복구(`#1366` 4kg×267 · `#1367` 톤백 550kg). **현상이 두 개**라는 게 핵심 — 「사라짐」과 「저장 안 됨」은 원인이 다를 수 있다.

🔴 **물증 = `MillingOutputPackage` id 결번 `1365`·`1368`** (시퀀스 `last_value=1368`). id가 발급됐다 = `createMany`까지 실행됐다가 롤백됐거나 커밋 후 삭제됐다는 뜻. `1365`가 이영명의 톤백일 가능성이 높다.

읽기 전용 조회로 **배제**: 재고차감(오늘 차감은 전부 제품재고 테스트분이고 전량 취소됨) · 이영명 권한(`OPERATION_MANAGE` 보유) · 톤백 포장지 마스터(`Packaging #7` 존재) · 톤백 SKU(`ProductType #18`, 6/16 생성분) · 마이그레이션 장애.

🔴 **추적을 막은 근본 문제 — `updatePackagingLogs`가 감사로그를 하나도 안 남긴다**(`deletePackagingLog`만 남김). 삭제 흔적이 0이라 결번 역산 말곤 방법이 없었다. **1순위 수정 대상.**

원인 후보 4개(미확정): A 낡은 화면 덮어쓰기(diff의 「payload에 없는 기존 행=삭제」. 단 `d18487e` 이전에도 결과가 같아 이번 리팩터링의 회귀는 아닐 듯) · B `count>0` 필터의 조용한 누락 · C `INVALID_LINE`이 새 줄 전체를 막음(「저장 안 됨」쪽 유력) · D 트랜잭션 롤백.

**결정: 동시 편집 대응은 B안(충돌 감지)** — baseline id 집합을 함께 보내고, 내가 못 본 행이 생겼으면 저장 거부. A안(중복 허용)은 기각. 🔴 **단 「저장 차단」이 「입력 날림」이 되면 안 된다** — 거부 시 화면 입력값을 초기화하지 말고 남의 줄을 합쳐 보여준 뒤 재저장으로 통과시킨다(오늘 사고 후반부가 정확히 재입력 강요였다).

**다음**: 사람 확인 먼저(이영명—에러 토스트 문구·톤백 중량칸 입력 여부 / 민채—실패 메시지·「모두 삭제」 클릭 여부 / 동시 편집 여부) → 계획서 `plan-포장소실-방지.md`.
...
미결 4건 사용자 결정: A=포장일자 자리 대체 · **B=primary**(시안 amber 기각 — 재포장 「손실 인정」이 primary라 amber 솔리드는 새 어휘) · C·D=계획서안 유지.

🔴 **브라우저 검증에서 구조 결함 3건**(시안·계획서만으론 안 나왔을 것들):
1. **다이얼로그 푸터가 잘려 버튼을 못 누름** — `DialogContent` 기본이 `grid`라 92dvh에 맞춰 행이 줄지 않는다. `flex flex-col` + 본문 `flex-1`로 해결. **`repack-dialog`에도 같은 결함이 잠복**해 있어 함께 고쳤다(내부 max-h 덕에 안 터졌을 뿐).
2. **다이얼로그 바탕이 회색** — 이 앱은 `--background: #f1f5f9`(회색)인데 shadcn `DialogContent`가 `bg-background`를 쓴다. 카드 성격이니 `--card`(흰색)여야 맞다. `bg-white` 명시로 막았고 **전역 결함이라 백로그 §22**에 남겼다(앱의 거의 모든 다이얼로그가 같은 상태).
3. **같은 품종·규격이 9행씩 온다** — 실 데이터에서만 드러났다. 구분자가 로트뿐인데 로트는 앞부분이 전부 같아 뒤를 자르면 서로 같아 보인다. sticky 컬럼 헤더 + 생산자·포장일 컬럼 + **로트 앞말줄임**(모바일)으로 해결.

시안과 의도적으로 다르게 간 것 1건: 행 구분선 `#f1f5f9` → `slate-200/60`(시안 값은 흰 배경에서 실제로 안 보였다. 목록 서브행이 쓰는 톤이라 새 어휘 아님).

검증: tsc ✅ · test 164/164 ✅ · lint 신규 0 · **브라우저 실왕복 확인**(D1 일괄 액션 실 DB 왕복 첫 실행). 모바일은 사용자가 「나중에 한꺼번에」로 보류(§23).
백로그 신규: §22 다이얼로그 배경 전역 · §23 재고차감 모바일 · §24 묶음 되돌리기(스키마 보강 선행).
**다음**: 605행 소급 정리 실행 → D2 매트릭스.
...
계획서 [plan-재고차감.md](docs/plan/plan-재고차감.md). **코드 변경 없음 — 협의·설계만.**

**규모 실측(Neon 실 DB)**: 제품재고 **629행**(벼 625/잡곡 4) · 64,479개 · 719.9톤, 가용>0 **605행**. `PackageMovement`는 **24건 전부 REPACK** — 판매·비판매 차감이 **0건**이다. 2026-02월 생성분 201행이 아직 살아 있다. 즉 시스템은 재고가 나간 적이 없다고 믿고 있고, 이 상태로 D2를 켜면 FIFO가 이미 팔린 재고를 집는다. **D2보다 이 작업이 먼저인 이유.**

협의 확정 7건:

| # | 결정 |
|---|---|
| N1 | 이름 = **재고차감** (「비판매차감」 폐기 — 사유에 판매가 들어간다) |
| N2 | 사유 = 판매/증정/분실/파손/기타. 과거 판매분 정리는 **SALE**로 |
| N3 | 일상 차감과 대량 정리는 **같은 흐름 하나** — 1건은 1건만 고른 경우일 뿐 |
| N4 | 벼·잡곡 **둘 다** 차감 버튼 |
| N5 | 확인 절차 **항상** |
| N6 | 되돌리기 진입점 = 목록의 **「차감된 재고 보기」 스위치** (탭 신설은 층위가 안 맞아 기각) |
| N7 | **판매 내역 목록은 D2와 함께** — `type=SALE` 필터 하나라 그때 발주서분이 자동 합류 |

🔴 **조사 중 기존 결함 발견 — `cancelMovement`가 `repackId`를 안 본다.** 지금은 `orderItemId != null`만 거부한다. 취소 버튼이 화면에 없어 안 터졌을 뿐, 「차감된 재고 보기」를 켜면 재포장 소진 24행이 노출되고 되돌리면 **원본만 복원돼 재고가 이중 계상**된다(`REPACK_DELETE_BLOCKED`가 막으려던 것과 같은 사고). 구현 시 함께 막는다.

🔴 **일괄 액션 신설이 필수다.** 단건 액션을 200번 루프로 부르면 Neon 왕복 250~300ms × 200 ≈ 1분 — 배송·상차 적재에서 이미 터뜨린 그 모양이다. `findMany 1 → createMany 1 → groupBy 1` + timeout 30초로 **왕복을 행 수와 무관하게 고정**한다.

다음: `docs/handoff/재고차감/` 스펙 → Claude Design 시안 2개(차감 다이얼로그 / 차감 이력) → 구현.

---
### 포장 줄 순서가 저장할 때마다 흔들리던 것 `fix`

**커밋:** `c1251cf` · 사용자가 §19 브라우저 검증 중 발견.

포장 내역을 고쳐 저장하면 **그 줄만 맨 아래로 이동**했다(배치 #126: `10kg → 4kg → 잔량` 이던 것이 저장 뒤 `10kg → 잔량 → … → 4kg`).

원인은 `outputs` 관계 조회에 **`orderBy`가 없던 것.** Postgres는 정렬을 안 주면 heap 물리 순서를 반환하는데, **UPDATE는 MVCC라 새 튜플을 heap 끝에 쓴다** — 고친 줄이 맨 뒤로 밀린다.

🔴 **정렬은 원래 없었다. 있는 것처럼 보였을 뿐이다.** 포장수정을 diff로 바꾸기 전(`d18487e`)에는 저장마다 전 행을 지우고 재생성해 매번 순서대로 새 id가 붙었다. diff가 「안 건드린 줄은 그대로, 고친 줄만 UPDATE」로 바꾸면서 그 우연이 깨졌다. **데이터는 멀쩡하고 표시 순서만** 흔들렸다.

「단위중량 큰 것부터」 정렬은 위쪽 **「규격별 합계」 배지에만** 있다(`add-packaging-dialog.tsx:402`, `PACKAGE_TEMPLATES` 순). 입력 줄 목록엔 정렬 코드가 없었다.

→ 만든 순서(`id: 'asc'`)로 고정. 새 줄은 항상 맨 아래에 붙고 입력 중에 줄이 튀지 않는다.

- `app/actions/milling.ts` — `getMillingLogs` (포장 다이얼로그 복원)
- `app/actions/dashboard.ts` — 최근 도정내역의 포장내역 팝업

엑셀·통계 3곳도 같은 `MILLED_OUTPUTS`를 쓰지만 `reduce` 집계라 순서와 무관해 두었다.

검증: 실DB 배치 #126 원래 순서 복원 확인 · `tsc` · 테스트 164 · eslint 신규 0건. **브라우저 확인 완료.**

---

### 재포장 버튼 톤다운 — 핸드오프 §1만 적용 `fix`

계획서 [plan-재포장버튼-톤다운.md](docs/plan/plan-재포장버튼-톤다운.md) · 커밋 `b5bb67d` · 핸드오프 `docs/handoff/재포장/`.

지금 재포장은 `+ 포장하기`와 **똑같은 outline이라 등록 버튼과 동급으로 튀었다.** 사용빈도가 낮은 기능이니 방향은 올리는 게 아니라 내리는 것 — `outline → ghost`, 켜짐은 `primary solid → 파랑 틴트`, 위치는 구분선 오른쪽(액션)에서 왼쪽(도구)으로. **벼 탭은 등록 버튼이 없어 구분선 오른쪽이 비므로 구분선도 지웠다.** 새 어휘는 만들지 않았다(ghost는 `재포장 취소`에, 파랑 틴트는 필터 활성에 이미 쓴다).

🔴 **핸드오프 4건 중 §1만 적용했다 — 시안이 R2-후속(`5a86ba1`) 이전 코드 기준으로 그려졌다.**

| # | 지시 | 판정 |
|---|---|---|
| §1 버튼 톤다운 | 적용 |
| §2 행 높이 44px | 네 값 **전부 이미 일치** |
| §3 선택 바 sticky | 이미 `sticky` + `anchorLabel`까지 |
| §4 다이얼로그 940px 2열 | 🔴 **기각** — R2-후속 #44가 940→720·2열→1열로 **일부러 갈아엎은 것** |

§1 안에서도 `disabled` prop 2건은 이미 적용돼 있어 **6개가 아니라 3개 파일**만 고쳤다. `gap-2`는 유지했다(핸드오프는 `gap-1.5`지만 낡은 시안이라 검증된 값이 아니고 이번 목적과 무관).

- `app/(dashboard)/packages/repack-toggle-button.tsx` — 전체 교체
- `app/(dashboard)/packages/rice-package-panel.tsx` — 순서 + 구분선 제거
- `app/(dashboard)/packages/misc-package-panel.tsx` — 순서

표시 계층만 변경 — 서버 액션·권한 판정은 그대로. `tsc` · eslint 신규 0건. **브라우저 확인 완료.**
...
**🔴 삭제보다 수정이 위험했다.** 삭제는 FK(`Restrict`)가 대신 막아주긴 했다 — 다만 「포장 삭제에 실패했습니다」밖에 못 말한다(포장수정 diff 이전의 벼와 같은 상황). 그런데 **UPDATE는 참조 무결성과 무관해 아무도 안 막는다.** 10개 중 3개를 팔았는데 `count`를 2로 줄이면 가용재고가 −1이 되고, 목록 필터(`available <= 0`)에 걸려 **깨진 행이 화면에서 사라진다.** 에러 하나 없이 조용히 틀어지는 게 가장 나쁘다. `updateMiscPurchase`는 `repackId`조차 안 보고 **품종까지 바꿀 수 있었다.**

**#66 규칙은 한 곳에서만** — [lib/package-guard.ts](lib/package-guard.ts) 신설(`guardDelete`/`guardUpdate`/`guardCountChange`/`guardIdentityChange` + 문구 상수). **단건 액션(잡곡)과 배열 diff(벼)가 같은 것을 쓴다.** `batch-outputs.ts`(#61) 때 지점마다 손으로 붙이다 두 번 놓친 전례를 따르지 않으려는 것이다. **기존 `packaging-diff.test.ts` 33개를 한 줄도 안 고치고 통과** = 리팩터가 동작을 안 바꿨다는 증거.

**#67** 삭제는 FK가 아니라 규칙으로 — FK는 최후 방어선으로 남기고 그 앞에서 이유를 적어 막는다. **#68** 차감량 밑 축소 거부(벼 #63과 같은 규칙·문구). **#69** 재포장 결과는 수정도 막는다(삭제만 막던 비일관 해소, 문구는 삭제와 다르게). **#70(A안 확정)** 차감 뒤 **품종·규격만** 거부 — 매입처·매입일·수량 증가 정정은 계속 허용. **#71** 화면 배지 없음(R3 #57 판단 유지).

**트랜잭션 경계도 넓혔다** — `deleteMiscPurchase`·`updateMiscPurchase`는 조회가 트랜잭션 밖이라 검사와 쓰기 사이에 차감이 끼어들 수 있었다.

**🔴 계획서의 「벼도 같은 구멍」은 절반만 맞았다.** 서버(`packaging-diff.ts`)는 규격 변경을 허용하지만 **화면에 그 입력이 없다**(규격은 새 행 추가 때만 정해짐). 대신 **진짜 구멍은 단중·포장지**였다 — 차감된 벼 16개 배치가 전부 `잔량` 줄인데 잔량·톤백은 단중 입력칸이 열려 있다. 재포장이 3kg를 소진해 간 행의 중량을 10kg으로 바꿀 수 있다(중량 보존 파괴). **백로그 §19**로 넘겼다(사용자 확정). 가용재고 계산 4중복은 **백로그 §20**.

- `lib/package-guard.ts`, `lib/package-guard.test.ts` — 신규(테스트 21개)
- `scripts/check-misc-guard.ts` — 신규, 실DB 노출 현황 + guard 판정(읽기 전용)
- `lib/packaging-diff.ts` — guard 사용으로 정리(동작 변화 없음)
- `app/actions/packages.ts` — 잡곡 4개 액션에 검사 추가
- `docs/리팩토링-백로그.md` — §19·§20 추가

검증: guard 21 / packaging-diff 33(무수정) / 전체 138 통과 · `tsc` 통과 · eslint 신규 0건(`packages.ts` 4→4, 기존 부채) · `next build` 미실행. **실DB: 잡곡 1행(찰보리 1kg×100, 차감 0) — 보호 대상 0개. 차감이 쌓이기 전이라 지금이 가장 쌌다.** 브라우저 확인 대기.
...
**후속 2 — 마감 버튼의 변경 감지가 무의미했다.** `handleCloseBatch`가 `JSON.stringify(outputs) !== JSON.stringify(initialOutputs)`로 변경을 판정했는데, `outputs`는 `restoreOutputs`를 거쳐 **`packagingId` 키가 하나 더 붙은** 값이라 **아무것도 안 고쳐도 항상 「변경됨」**이었다. 즉 마감 버튼은 언제나 `updatePackagingLogs`를 호출해왔고, 예전 코드에서 그건 곧 **전 행 삭제 후 재생성**이었다(차감된 배치는 마감조차 FK로 실패). 같은 가공을 거친 값끼리 비교하도록 고쳤다 — 안 고치고 마감하면 확인 문구가 「작업을 마감하시겠습니까?」로 뜨고 DB 왕복 3회가 빠진다.

**후속 1(같은 날)**: 중량 0 차단은 **유지 확정**(0kg 행은 재고·수율 오염 — #269가 그 사고 결과). 다만 메시지가 「17번째 줄」이라고만 나와 **화면이 로트별로 쪼개진 배치에선 못 찾았다**(#73은 22줄·생산자 5명) → **규격을 함께 낸다**: 「2번째 줄(톤백): 단위 중량을 입력해 주세요.」 테스트 33개로 증가.
