...
**후속(같은 날)**: 중량 0 차단은 **유지 확정**(0kg 행은 재고·수율 오염 — #269가 그 사고 결과). 다만 메시지가 「17번째 줄」이라고만 나와 **화면이 로트별로 쪼개진 배치에선 못 찾았다**(#73은 22줄·생산자 5명) → **규격을 함께 낸다**: 「2번째 줄(톤백): 단위 중량을 입력해 주세요.」 테스트 33개로 증가.
