...
### 발주서 톤백 — 쪼갤 몫 「요구 + 3kg/자루」 추천 · ±1kg 조절 · 행별 가위 삭제 (D2d 결정 K) `feat` `__HASH__`
...
- **`lib/purchase-order-bulk.ts`** — `BULK_TARE_KG = 3` · `bulkTargetKg(remainingKg, remainingBags)`. 추천 목표 = 남은 요구 + 3 × 남은 요구 자루 수.
...
- ⚠️ +3을 **자루당**으로 잡았다(2자루 주문 = +6). 사용자 확인 대기
