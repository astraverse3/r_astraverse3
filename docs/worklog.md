...
### §H 택배 행 펼침 — 폰에 뜨면 안 될 선택 바가 글자째 쪼개져 있었다

§H 10항목은 H-8(설명 요청) 빼고 **전부 통과**. 그런데 **캡처에 딴 게 찍혀 있었다** —
목록 위 흰 박스에서 「54 수 령 처」가 **한 글자씩 세로로** 쪼개져 있었다.

**① 왜 폰에 떴나 — 데스크탑 전용 UI가 조건 없이 렌더됐다**

그건 **매트릭스 선택 바**다. 선택은 행 체크박스로만 하는데 매트릭스는 `hidden sm:contents`라
**폰에는 켜고 끌 수단이 아예 없다.** 그런데도 뜬 건 목록 푸터 「작업필요 n건 검토」가
`onOpenGate`에서 `setSelected(new Set(ids))`를 채우기 때문. 게이트를 닫으면 **선택만 남아**
정체불명의 바가 눌러앉는다 — 🔴 **닫는 X조차 깨진 박스 안에 있어 없앨 방법이 없었다.**

**② 왜 세로로 쪼개졌나 — `left-1/2 -translate-x-1/2`가 폭을 절반으로 자른다**

`translate`는 레이아웃이 끝난 **뒤의 시각 이동**이라 폭 계산에 안 들어간다.
그래서 shrink-to-fit 상한이 `100vw - left`, 즉 **화면의 절반**이 된다 — 390px 폰에서 195px.
그 안에 버튼과 X가 자리를 차지하니 남은 폭이 거의 없어 한글이 글자 단위로 줄바꿈됐다.
📌 **데스크탑에서 멀쩡했던 건 고쳐져서가 아니라 절반이 넉넉해서다 — 창을 좁히면 똑같이 깨진다.**

`hidden sm:flex` + 가운데 정렬을 `inset-x-0`·`justify-center`로 바꿔 `translate` 패턴을 걷어냈다.
같은 패턴 3곳을 더 찾았지만 `sm:` 한정이거나 `w-max`가 있어 안전하다
(`pwa-install-guard`는 `w-max`라 잘리진 않되 화면 밖으로 나갈 수 있다 — 별건).

**🔴 곁가지 — A-2를 「낡았다」고 지운 게 틀렸다(되살림)**

캡처에 **상태 칩 3종**(`재고부족 20 · 대기 34 · 완료 13`)이 멀쩡히 있었다. 어제 나는
**필터 칩**(작업필요·전체 2개 고정)만 보고 「상태별 칩은 만든 적 없다」고 단정해 항목을 지웠다.
둘은 **헤더의 서로 다른 줄**이고, 상태 칩은 `n === 0`이면 `return null`이라 A-2는 애초에
유효한 항목이었다(실측도 통과). 사용자가 「대기·완료·재고부족 합이 전체와 같은지 묻는 거지?」라고
되물은 것이 **화면을 보고 한 말**이었는데 나는 그걸 오해로 읽었다.
📌 **되물음은 확인 요청이 아니라 반증일 수 있다 — 코드를 다시 보기 전에 답하지 말 것.**

덤으로 A-1은 수치를 못 박지 않게 고쳤다 — 하루 사이 `61 → 58 → 54`로 세 번 바뀌었다.

검증 tsc 0 · eslint 0 · test 418/418.

### 건 목록 §A 점검 — 그룹 하나가 통째로 사라져 있었다

발주서 M1 점검 목록 **§A(건 목록)** 9항목을 사용자가 폰으로 훑었다. 6개는 통과,
**A-3에서 「그룹 개수가 안 맞고 미지정이 있다」**는 신고가 나왔다.

**🔴 그룹 축이 표시용 함수를 빌려 쓰고 있었다**

`groupNameOf`가 `nameTiersOf(decl, row)[1]`를 그대로 썼는데, 그 함수는 **이름 열 표시**용이라
앞뒤가 같으면 뒤를 생략한다 — 「서대문마을생협 ｜ 서대문마을생협」은 군더더기니 옳은 동작이고
테스트에도 고정돼 있다. 문제는 그 `null`이 **그룹 축에서는 「소속 없음」**이 된다는 것.

택배는 수령인 칸이 비면 파서가 발주처를 복사한다(#26). 실 DB 67건 중 3건이 그랬다:

```
id=121 서대문마을생협 → 서대문마을생협   id=163 예은농산 → 예은농산
id=168 해남로컬푸드   → 해남로컬푸드
```

앞 둘은 제 그룹이 1씩 모자랐고, 🔴 **해남로컬푸드는 1건짜리라 그룹이 통째로 사라졌다.**
9/21 기대값(9그룹)이 **오히려 맞았고 코드가 틀렸다** — 미지정 3건을 제자리로 돌리면 한 건도 안 틀린다.

📌 **백로그 §45와 같은 자리다** — *공용 순수함수를 쓰는 것과, 그 함수가 쓰이는 화면 구조가
같은 것은 다르다.* 어제 §45에서 똑같은 교훈을 적었는데 바로 옆 줄에서 재발했다.

`lib/purchase-channel.ts`에 **`groupAxisOf`**를 따로 뒀다(축 = `primary`의 반대쪽).
그룹 축이 **제 이름과 테스트**를 가져야 다음에 또 빌려 쓰지 않는다. 실 DB 재검증에서
택배 9그룹·기대값 일치, `#17`·`#18`·`#19`는 그대로 헤더 없음.

**예외 문구를 수령인 옆으로** (사용자 제안)

`재고부족`·`N품목 매칭실패`·`N품목`을 이름 **아래**에 깔던 걸 **옆**으로 옮겼다.
예외 있는 행만 2줄이라 목록 높이가 들쭉날쭉했다. 이름이 `truncate`·문구가 `shrink-0`이라
좁아지면 **이름이 먼저 줄어든다** — 예외가 잘리면 적는 의미가 없다.
🔴 **행 높이는 안 올렸다**(약 38px, 44px 기준 아래) — 사용자 판단으로 **택배는 건별 집중도가 낮아
조밀하게 훑는 화면**이다. 다른 채널에 그대로 적용하지 말 것.

**곁가지 — 점검 목록의 「미배포」가 전부 낡아 있었다**

`origin/main` == 로컬이라 §D-5~8·§H·§I가 이미 배포돼 있었는데 문서는 「아직 실서버에 없다」고
붙들고 있었다. §A-4가 「배포 뒤 H-9에서 확인」으로 미뤄 둔 것도 같이 풀었다.
A-1의 `61`은 그 사이 3건이 완료돼 **58**이 된 것(데이터 변동) · A-2는 **상태 칩이 여러 개이던
옛 설계**를 전제한 문구라 삭제했다(칩은 「작업필요」·「전체」 2개 고정).

검증 tsc 0 · eslint 0 · test **418/418**(새 4개). **폰 재확인 대상**: A-3 해남로컬푸드 그룹 ·
A-5 긴 이름 + `재고부족` 동시 행의 잘림.

## 2026-09-22

### 포장 다이얼로그 — 규격 버튼이 안 보이던 진짜 원인 `3fa46af`

어제 「미해결」로 남긴 **도정관리 모바일 포장 등록** 건을 다시 잡았다.
**진단이 빗나가 있었다** — 어제는 「모바일 생산량 `-`가 안 눌린다」고 적었는데,
사용자는 **상태 배지로 잘 들어가고 있었고** 문제는 **다이얼로그 안 규격 버튼**이었다.
📌 **재현 못 한 진단을 단정형으로 적으면 다음 사람이 엉뚱한 데를 판다.** 해당 절에 정정 표시를 달았다.

확인 과정에서 **증상이 애초에 둘**이었음이 드러났다. 둘 다 「처음 켰을 때」에 몰려 겹쳐 보였다.

**① 규격 버튼이 안 나온다 — 세션 미주입 (오늘 고침)**

`components/providers/session-provider.tsx`가 `<SessionProvider>`에 **`session`을 안 넘기고 있었다.**
그러면 클라이언트가 마운트된 뒤 `/api/auth/session`을 **네트워크로** 가져올 때까지
`useSession()`이 `undefined`고, `hasPermission(undefined, …)`은 `if (!user) return false`라 **false**다.
→ **권한 있는 사용자에게 권한 없는 화면이 먼저 그려진다.**

```
add-packaging-dialog.tsx:604   {!isClosed && canManage && ( 규격 버튼 )}
add-packaging-dialog.tsx:781   {canManage && ( 총 포장 kg · 기록 저장 )}
```

🔴 **`useSession`을 쓰는 화면이 27개인데 `status === 'loading'`을 보는 곳은 0개**였다 —
「권한 없음」과 「아직 모름」이 **같은 화면**이었다. 데스크탑 유선에선 깜빡할 새라 안 보이고
**폰에서만 몇 초 보이므로 「모바일 전용 버그」로 신고된다.**

`app/layout.tsx`를 `async`로 바꿔 `getServerSession(authOptions)`를 주입했다.
이미 `auth-guard`·`admin/notices`·`admin/users`에서 쓰는 패턴이라 새 것이 아니다.

**② 「불러오는 중」 5초 — 왕복 + 콜드 스타트 (내일 측정)**

사용자가 「기록저장 버튼이 불러오는 중이라는데?」라고 한 게 **결정적 단서**였다.
🔴 **그 문구가 보인다는 건 푸터가 렌더됐다는 뜻 = `canManage=true`** → ①은 이미 배제된다.
그리고 **데스크탑에서도 5초**라 폰 세션도 PWA 캐시도 아니다. 멈춘 게 아니라 **느린 것**이다.

`auth.ts`의 jwt 콜백이 **매 요청마다** `prisma.user.findUnique`로 권한을 다시 읽어서
**액션 하나당 DB 왕복 2회**(세션 + 실제 쿼리)다. 여는 순간 액션이 2개 나가고
Next는 Server Action을 **순차**로 보내므로 **4회가 줄줄이** — Neon 250~300ms 기준 약 1.2초.
나머지는 Vercel 콜드 스타트 몫으로 추정. `suggestProductType`은 **규격 버튼을 누를 때** 불려서 무관.

측정 절차·판정 기준은 `docs/점검-포장다이얼로그-지연.md`에 남겼다(실서버, 아침 첫 사용).
**①을 먼저 넣은 이유**가 여기 있다 — 세션 지연이 숫자에서 빠져야 **왕복만 순수하게** 잴 수 있다.

**곁가지 — 실패가 조용히 먹힌다 (별건, 미해결)**

- `add-packaging-dialog.tsx:277` `.then()`에 **`.catch()`가 없다.** 요청이 reject되면
  `setOutputsLoading(false)`가 영영 안 돌아 **진짜 먹통**이 된다(저장이 `writeBlocked`로 잠기고 토스트도 없음).
  같은 패턴 **10곳**(재포장·잡곡 포장·생산자 등록 등).
- `milling.ts:454` `getBatchOutputs`의 **`requireSession()`이 `try` 밖**이라 throw가 샌다.
  🔴 **권한 단순화 때 이미 배운 교훈이 같은 자리에 또 있었다** — 가드를 try 밖에 두면 에러가 통째로 뭉개진다.
  액션 파일 28개 전수 확인 필요.

**남은 것**: 모바일 생산량 `-`가 데스크탑에서만 눌리는 불일치(막힌 길은 아님) ·
액션 2개 합치기(측정 후 판단) · 위 곁가지 2건.

검증 tsc 0 · eslint 0 · test 414/414. **브라우저 확인 전**(푸시는 완료 = 배포됨).

### 채널별 건 처리 — 택배는 목록에서 펼친다 (P0~P5) `aee1218`·`f0167f8`·`ca43857`

시안 갱신을 받아 **택배만 건 상세를 없애고 목록 안에서 행을 펼치도록** 했다.
급식·이마트·기업별은 건 상세 그대로다. 계획서 `docs/plan/plan-발주서-채널별-건처리.md`.

**분기축은 `ChannelDecl.detail`** — 🔴 **데이터가 아니라 채널로 가른다.** 건수·평균 품목수로
판정하면 같은 채널인데 **시트마다 화면이 달라져** 「이 채널은 이렇게 쓰는 것」이라는 학습이
안 선다. 다음 주 택배가 3건만 들어와도 화면이 바뀌어서는 안 된다.
근거는 실측 — 택배 67건 **평균 1.18품목**(1품목 건 58) vs 나머지 건당 3~8품목.

🔴 **구현 전 대조에서 시안의 낡은 것 3가지를 걸렀다.**

| 시안 | 실제 |
|---|---|
| `n라인 FIFO 차감` | 오늘 「품목」으로 바꾸고 FIFO는 화면에서 뺐다 |
| 96라인 · 완료 24 · 실패 2 · 작업 41 | **79품목 · 완료 6 · 실패 0 · 작업 61** |
| 「48건이 1라인」 | **58건** |

📌 **시안 수치는 스냅샷이고 금방 낡는다 — 구조만 취하고 숫자는 실측으로 다시 채운다.**

**새로 만든 건 둘뿐이었다**(펼침 상태 · `rowNoteOf`). 나머지는 이미 있었다 — 라인은
`buildOrderLines`(순수, 서버 왕복 0), 일괄차감은 게이트가 `orderIds[]`라 **건 하나짜리 배열**로
열면 되고, 「다음 건」도 M1-3 로직이다. `LineCard`는 `order-line-card.tsx`로 올려 공유한다.

- 🔴 **접힌 줄에는 예외만 적는다** — 모든 줄에 「1품목」을 반복하면 부족·실패가 묻힌다
- 🔴 **부족은 개수를 안 적는다** — `MatrixCell`에 부족분이 없고 `remainingQty`는 「아직 차감
  안 한 전부」라, 주문 10·가용 3이면 부족은 7인데 **10으로 찍힌다.** 시안대로 하면 틀린 수를
  보여주게 된다. 정확한 값은 펼친 뒤 `buildOrderLines`가 낸다
- 🔴 **「다음 건」 스냅샷 함정이 여기도 그대로** — 필터가 「작업필요」라 차감하면 그 건이 빠지고
  뒤가 당겨져 **한 건을 건너뛴다.** 펼칠 때 형제 순서를 스냅샷으로 잡았다(M1-3과 같은 자리)

**P0 — `matrix-client.tsx` 1022 → 527줄.** `matrix-layout.ts`(53) · `matrix-head.tsx`(248) ·
`matrix-header.tsx`(108) · `matrix-bits.tsx`(124). 동작은 안 바꿨다.

- 🔴 **상수는 흩지 않고 한 파일에 모았다** — `W_*`와 `L_*`는 **한 쌍**이라 따로 놀면
  **가로 스크롤할 때만** 어긋나 보이고 눈으로는 원인을 못 찾는다
- 📌 **덩어리를 옮길 땐 줄 번호가 아니라 그 줄의 내용으로 잡는다** — 하드코딩했다가
  `}: {`(props 타입 여는 줄)가 `^}`에 걸려 한 번 깨졌다

**P4 — 백로그 §45 해결.** 확인 중 사용자가 **「수령인 가나다가 그룹 안에서만 정렬돼서**
**의미없는 거 아니냐」**고 지적했고 사실이었다. 그룹 축은 `nameTiersOf` tail이라 택배는
**발주처**로 묶이는데, 「발주처별」이 이미 2차 키로 `byRecipient`를 써서 **묶고 나면 그룹 안**
**순서가 두 정렬에서 같다.** → `sort === 'recipient'`면 **그룹을 안 묶는다**.

- 📌 **공용 순수함수를 그대로 쓰는 것과, 그 함수가 쓰이는 화면 구조가 같은 것은 다르다.**
  「매트릭스와 같은 `sortMatrixRows`를 쓴다」는 원칙은 지켰는데 화면에 그룹이 붙으며 깨졌다

**정렬 라벨 단일화**(`ca43857`). 매트릭스는 「작업필요」, 목록은 「작업 필요 먼저」로 같은 정렬을
다르게 부르고 있었다. 🔴 목록엔 **「매트릭스 헤더와 같은 3종」이라는 거짓 주석**까지 있었다 —
그걸 믿고 한쪽만 고치면 차이가 더 벌어진다. → `status-meta.ts`의 `MATRIX_SORTS` 한 벌.
라벨은 **「작업필요 먼저」**(폰엔 필터 칩 「작업필요 n」이 이미 있어 정렬까지 같은 말이면
한 화면에 같은 문구가 둘이 된다). 띄어쓰기도 **「작업필요」**로 화면 6곳 통일.

`tsc` 0 · `eslint` 0 · `test` **414/414**. 🔴 **6커밋 전부 미푸시**(브라우저 확인 전).

- ⚠️ 점검 목록을 **배포됨/미배포로 갈랐다** — §A~§G는 지금, **§D-5~8·§H·§I는 푸시 뒤**

### 배포가 두 번 깨졌다 — P1002 → `directUrl` 분리, 그리고 P1013 `5a0bb1b`

메모리에 「재발 가능」으로 적어 둔 백로그 §27이 실제로 터졌다. **적용할 마이그레이션이 0건인데도**
`migrate deploy`가 pooler 주소로 붙어 `pg_advisory_lock`을 못 걸고 10초 만에 포기했다.

→ `directUrl` 분리. 런타임(`url`)은 pooler를 그대로 쓴다 — 서버리스는 커넥션 풀링이 필요하다.
검증은 `npx prisma migrate status`로 했다. **advisory lock을 안 잡아 안전**하고 접속 호스트가
로그에 찍혀 직결 주소를 쓰는지 바로 보인다.

🔴 **순서(Vercel 환경변수 → 로컬 `.env` → 스키마)를 지켰는데도 P1013으로 또 깨졌다.**
`Datasource "db":` 뒤에 **호스트가 안 찍히면 값 파싱 실패**다 — Vercel UI에 따옴표째
붙여넣었거나 `postgresql://user:pass@` 앞부분이 빠진 것이다.
📌 **로컬 `.env` 값을 그대로 클립보드로 넘기는 게 제일 빠르다** — 비밀값을 화면에 띄우지 않는다.
⚠️ `DIRECT_URL`을 **Production만**으로 두면 Preview 빌드가 같은 이유로 깨진다.

- ⚠️ **방아쇠 주의**: P1002 직전에 §5 기대값을 재실측하려고 같은 Neon DB에 붙었고, 그중 하나가
  오류로 중단되며 `$disconnect()`를 못 불렀다. 단정할 수는 없지만 **배포가 돌 만한 때에는**
  **DB 접속 스크립트를 돌리지 않는 편이 낫다**

### 화면 튐 1차 실측 — 용의자 A~D 전부 기각, 덫 보강 `22d20a5`·`9dead3e`

사용자가 아침에 또 튀어서 폰 기록을 받았다. **튄 것은 2건이고 사용자 기억도 「두 번」으로
독립 일치했다.** 경로변경 15건 중 나머지 13건은 전부 정상 이동이었다.

```
09:59:48.775  /raw-stocks → /milling · 직전 <input 입력칸> (8.3초 전)
10:00:03.287  /raw-stocks → /milling · 직전 <span> (1.0초 전)
```

- 🔴 **용의자 A·B·C 기각** — `push`가 찍힌 13건은 전부 정상이고 **튄 2건에는 push가 없다.**
  계측이 걸린 자리로 이동이 오지 않았다
- 🔴 **용의자 D(뒤로가기) 기각** — `popstate` 0건. `load /milling · back_forward`는 여러 번
  있지만 전부 `visibility hidden` 뒤 한참 있다가 나온다 = 앱 재개 시 정상 복원이다.
  📌 **SPA 내 튐(`route`)과 문서 복원(`load back_forward`)은 구분해야 한다**
- ⚠️ **8.3초 공백에 pointer가 0건** — 클릭 인과로 보기 어렵다. 같은 입력칸을 누른 뒤
  **7.2초 뒤 「적용하기」를 눌러 정상 진행한 경우**가 나란히 있어서 더 그렇다.
  전에 정적 분석으로 두 번 빗나갔던 자리라 **추측을 더 쌓지 않았다**

**덫 자체의 결함 4건을 고쳤다. 진단을 가리고 있었다.**

1. 🔴 **용의자를 하나씩 계측하는 방식이 근본 문제였다** — 계측 안 한 자리로 빠져나가면
   못 잡는다. → **history API를 감싼다**(Next App Router는 어떤 경로로 옮겨가든 여기를
   거친다) + `stackHint`로 호출한 앱 프레임(파일:줄)을 남긴다.
   📌 **호출 지점을 미리 고르지 말고, 지나가는 목을 지킨다**
2. 🔴 **깊이 제한이 두 곳에 있었다** — `describeElement(maxDepth=3)`과
   `toDescribable(depth>3)`. **한쪽만 풀면 효과가 없다.** 3단계로는 행 전체를 감싼 링크가
   `<span>`으로 뭉개져 「직전 `<span>`」이 링크였는지조차 가릴 수 없었다 → 둘 다 12,
   `<a>`는 **href까지** 남긴다. 📌 「그 프리미티브를 쓰는 전부를 grep」의 사례
3. **날짜가 없었다** — `formatEntry`가 시·분·초만 찍었다. 🔴 **저장은 epoch ms라 날짜가
   온전했고 깎이던 곳은 표시뿐이다**(발주서 DTO 사례와 같은 자리) → `MM-DD` 추가.
   복사 제목도 `(복사 시각 …)`으로 — 그게 기록 시각으로 읽혀 혼동을 키웠다
4. 버퍼 200 → **400**. 실측에서 200칸이 꽉 차 앞이 밀려나고 있었다

🔴 **덫 기록은 localStorage라 origin별로 분리된다** — 실서버·개발서버·폰이 전부 남남이다.
📌 **「모바일에서만 튄다」면 데스크탑 기록을 아무리 봐도 없다** — 이걸 모르고 데스크탑
로그로 한참 헤맸다.

`tsc` 0 · `eslint` 0 · `test` **407/407**. `this`를 쓰면 React Compiler가 그 함수를
건너뛴다는 경고가 떠서 수신자를 직접 적는 방식으로 고쳤다.

- 📌 **다음**: 튀면 폰에서 `/admin/trace` 복사 — 날짜·호출자·링크 href가 함께 찍힌다.
  ⚠️ 폰을 완전히 닫고 다시 열 것(서비스워커가 옛 코드를 붙들 수 있다)
- 🔴 **원인 확정 즉시 덫 제거** — `e9113a8`·`22d20a5`·`9dead3e`

### 발주서 화면 용어 통일 — 묶음→시트 · 라인→품목 · FIFO 제거 `2a91668`

어제 결정만 해 둔 용어 교체를 구현했다. 화면에 뜨는 문자열만 바꾸고
변수·타입·DB 필드명·주석은 그대로 뒀다. 19개 파일 48곳.

- **묶음 → 시트** 12곳 + 동어반복 4곳 재작성 · **FIFO** 화면에서 제거 2곳 ·
  **라인 → 품목** 34곳(UI 19 · 액션 에러 15)

**🔴 동어반복 4건은 단순 치환이 불가능했다.** 「묶음 = 시트 1장」을 *설명하던*
문장들이라 통일하면 "시트 1개 = 시트 1건"이 된다.

- 📌 **용어를 합치면 그 둘의 관계를 설명하던 문장은 설명할 게 없어진다** —
  치환이 아니라 걷어내야 하는 자리다
- `upload-dialog` 푸터는 **같은 값(`checkedSheets.length`)을 「시트」와 「묶음」이라는
  두 이름으로 두 번 세고 있었다.** 이름이 하나가 되자 중복이 드러나 3줄 → 2줄이 됐다

**🔴 조사가 바뀐다 — 묶음(받침 ㅁ) ≠ 시트(받침 없음).** 「묶음이」→「시트**가**」,
「묶음을」→「시트**를**」, 「묶음과」→「시트**와**」. 기계적 치환이면 "시트이 없어요"가
됐다. **라인 → 품목은 둘 다 받침이 있어 조사가 안 바뀐다.**

**전수 대조로 계획서에 없던 3건을 더 찾았다.** 계획서가 지목한 자리가 아니라
「묶음·FIFO·라인」을 쓰는 전부를 grep한 결과다.

- `review-gate-dialog` 「라인」 8곳 → **9곳**(`:361` 이름을 못 찾았을 때의 대체 표기)
- 액션 에러 11곳 → **15곳**. `lib/purchase-order-cell.ts` · `-db.ts` —
  🔴 **`lib/` 아래에도 화면에 뜨는 `throw` 문구가 있다**
- 🔴 **「라인」 grep은 「인라인」을 6건 잡는다**(statistics 3곳 등) — 전부 발주서 밖

**푸터 버튼은 A안.** `31라인 FIFO 일괄차감`(14자) → `31품목 일괄차감`(8자).
점검 **B-3(360px 2버튼)이 미확인**이라 글자를 줄이는 쪽을 골랐고, 순서 단서는
게이트 상단 요약의 로트 개수 뒤로 옮겼다 — `포장 59개 · 로트 8개 · 오래된 것부터`.
게이트는 「이만큼 이렇게 나갑니다」를 보여주는 자리다.

**범위에서 뺀 것은 계획서 §4 그대로.** 🔴 **감사로그 `description` 3곳**
(`purchase-order-assign:240` · `-batch:330` · `-upload:431`)은 과거 기록이 「라인」으로
쌓여 있어 형식을 바꾸면 검색이 갈린다 — 최종 grep으로 남아 있는 것을 확인했다.
주석·타입 주석·다른 화면의 「묶음」(제품재고 `deduct-dialog` 등)도 그대로다.
최종 전수 grep **화면 문자열 잔존 0건**.

**덤으로 낡은 문장 1건.** `upload-table` 하단의 「매트릭스 차감 화면은 다음 단계에서
연결됩니다」를 지웠다 — D2~D3가 끝나 이미 연결돼 있다. 같이 훑은 3건은 정상이라
...
### 건 목록 모바일 배경 누락 · 용어 정리 착수 (용어는 미착수)

실기기 확인에서 2건. 하나는 고쳤고 하나는 결정만 하고 내일로 넘긴다.

**① 건 목록 화면에 배경이 없었다** — 루트가 `flex flex-col`뿐이라 대시보드 페이지
배경(회색)이 그대로 비쳤다. 시안은 흰 면 위의 목록이다. 루트에 `bg-card`(#ffffff).
`min-h-full`을 같이 준 건 건이 적어 목록이 짧을 때 아래쪽만 회색으로 남는 걸 막기 위해서다.

- 📌 **행·헤더에 각각 배경을 주는 구조가 아니라 「면」이 하나 있어야 한다** —
  필터바(`bg-slate-50`)와 그룹 헤더가 회색인 건 의도이고, 그 대비가 성립하려면
  바탕이 흰색이어야 했다

**② 용어 — 「묶음」이 안 읽힌다.** 사용자: 「묶음 삭제」는 **「시트 삭제」**라야 인식이 빠르다.
전수 조사 후 범위를 정했다(계획서 `docs/plan/plan-발주서용어정리.md`).

- **묶음 → 시트** 16곳(발주서 화면 전부 + 설정 2곳) · **FIFO → 오래된 것부터** 2곳 ·
  **라인 → 품목** 약 30곳 · **매트릭스는 그대로**(사용자 판단)
- 🔴 **감사로그 `description` 3곳은 뺐다** — 과거 기록이 「라인」으로 쌓여 있어
  형식을 바꾸면 검색이 갈린다. 화면 문자열과 성격이 다르다
- 🔴 **푸터 버튼 글자 수가 선결 과제** — `n라인 FIFO 일괄차감`(14자)이 모바일 푸터에서
  「다음 건」과 한 줄을 나눠 쓴다. 점검 **B-3(360px 2버튼)이 아직 미확인**이라
  글자가 늘면 바로 깨진다 → `n품목 일괄차감`(8자) + 순서 단서는 다른 자리로 제안
- **변수·타입·DB 필드명은 안 건드린다** — 화면 문자열만

`tsc` 0 · `eslint` 0. **용어 교체는 내일 착수.**

### 묶음 목록 모바일 헤더 정리 — A안 (상단 3줄 → 2줄)

실기기에서 제품판매 탭 상단이 **3줄**이었다 — 탭 / 업로드 버튼이 혼자 쓰는 행 / 칩 2줄.
업로드 하나가 한 줄을 통째로 차지했고 칩은 6개라 390px에서 반드시 2줄이 됐다.

```
SalesTabs        rightSlot?: ReactNode 신설 · 모바일 블록만 flex items-stretch
page.tsx         tab === 'product'일 때만 <UploadDialog compact />
product-sales    업로드 단독 행 → hidden sm:flex (데스크탑 유지)
UploadDialog     compact → size="icon-lg" 아이콘 전용 + aria-label
upload-table     칩 줄 flex-wrap → overflow-x-auto + sm:flex-wrap, Chip shrink-0
```

- 🔴 **`scrollbar-hide`가 정의 없이 쓰이고 있었다**(9곳). 칩 줄에 그대로 갖다 썼으면
  「숨겼다」고 믿은 채 데스크탑 좁은 창에서 스크롤바가 보였을 것이다 → 유틸을 먼저
  정의하고 **별도 커밋**(`8ddcd51`)으로 쪼갰다. 📌 **기존 클래스를 재사용할 땐 그게
  살아 있는지부터 확인한다** — 이번엔 9곳이 쓴다는 사실이 오히려 근거처럼 보였다
- 🔴 **세그먼트에 `min-w-0`**을 안 주면 탭 라벨 폭이 칸의 하한이 돼 오른쪽 버튼이 밀린다
- **구분선은 회색 세그먼트 「밖」**에 둔다 — 업로드는 탭이 아니라 다른 기능이라는 걸 시각으로 가른다
  (시안 갱신 의도. 처음 시안은 세그먼트 안에 3번째 칸으로 넣었었다)
- ⚠️ **`UploadDialog` 인스턴스가 2개**가 된다(모바일 슬롯 + 데스크탑 행, 서로 `hidden`).
  한쪽만 화면에 있어 기능상 무해하다. 하나로 합치려면 위치가 달라 레이아웃이 꼬인다
- 기본 트리거의 모바일 분기(`px-2.5 sm:px-4` · 라벨 `hidden sm:inline`)는 **닿지 않는 코드**가
  돼 같이 걷어냈다
- **범위 밖(별건)**: 정렬 전환 드롭다운(C2안, 실측상 실효 낮음) · `UploadSummaryRow.createdAt`
  DTO 원본 시각(정합성 건) · 지난 상차 묶음이 쌓이는 문제(숨김·기간 필터)

✅ **브라우저 확인 종결** — 확인에서 1건이 나왔다.

#### 칩 스크롤에 「더 있다」 단서가 없었다 `713515c`

1줄 스크롤은 됐는데 **칩이 중간에서 잘리지 않고 여백에서 깔끔하게 끊겼다.**
칩 폭과 컨테이너 폭이 우연히 맞아떨어진 것이다.

- 🔴 **잘린 칩에만 기대면 안 된다** — 칩 개수가 바뀔 때마다 단서가 있다 없다 한다.
  폭과 무관하게 항상 뜨는 신호가 필요하다 → `mask-fade-r` 유틸 신설(오른쪽 2.5rem을 깎는다).
  스크롤이 끝에 닿으면 클래스를 떼서 마지막 칩이 흐린 채 남지 않게 한다
- **`-mx-3 px-3`으로 부모 패딩 밖까지** 스크롤 영역을 넓혔다 — 패딩 안에 갇히면
  칩이 화면 가장자리에 **걸칠 수가 없어서** 깔끔하게 끊긴다
- 데스크탑은 wrap이라 넘침이 없어 판정이 저절로 false가 된다(별도 분기 없음)
- 📌 **이번 건도 [[affordance_not_missing_feature]]와 같은 자리다** — 기능은 됐고 단서가 없었다

`tsc` 0 · `eslint` 0 · **test 401/401**. 계획서 `docs/plan/plan-묶음목록헤더정리.md`.

### 문서에 적힌 커밋 해시 8개 정정 (리베이스 잔재)

푸시 범위를 쪼개려고 권한 커밋만 cherry-pick하고 로컬을 rebase했는데,
**그 아래 푸시 전이던 커밋들까지 같이 새로 쓰였다.** 문서·메모리에 적어 둔
해시가 저장소 어디에도 없는 상태로 남아 있었다.

```
569c425 -> 33c568f  M1-1·M1-2         84a8571 -> 527f07e  연도 경계 9월
91636df -> 2030e21  M1-3              6986338 -> e959247  생산연도 셀렉트
9aae548 -> 5aee1d9  M1 점검 목록       aff3ece -> 2be8593  대시보드 11월 경계
b33dd4e -> 00a4840  묶음목록 헤더시안   fbf00c9 -> 35f37e7  작목반 등록 셀렉트
```

- 🔴 **아래 4개는 「배포 완료」라고 기록해 둔 커밋이다.** 나중에 근거를 되짚으려고
  `git show`를 하면 빈손이 된다 — 미푸시분만 어긋난 게 아니었다
- 📌 **교훈: cherry-pick + rebase로 푸시 범위를 쪼갰으면, 리베이스에 딸려 들어간
  커밋의 해시도 문서에서 같이 갱신한다.** 검사는 문서의 7자리 해시를 긁어
  `git branch --contains`로 한 번 훑으면 된다(258개 중 8개 적발, 재검사 0건)
- 코드 변경 없음. `docs/worklog.md` · `plan-26년산전환.md` · `모바일-점검-발주서-M1.md`

### 원물 엑셀 업로드 권한 ADMIN → SUPPLY_MANAGE `27212fc` (배포됨)

26년산 벼 첫 입고를 등록하려는데 **정윤미 PC에서만 엑셀 업로드가 실패**했다.
"이 PC에서는 되는데 저 PC에서는 안 된다"로 들어왔지만 **PC도 캐시도 아니었다.**

```
버튼 노출  stock-excel-buttons.tsx:30   hasPermission(user, 'SUPPLY_MANAGE')
서버 가드  stock-excel.ts:131           requireAdmin()        ← 여기만
수동 등록  stock.ts:22   createStock    requirePermission('SUPPLY_MANAGE')
생산자엑셀 excel.ts:74   importFarmers  requirePermission('SUPPLY_MANAGE')
```

실 DB를 조회하니 **ADMIN은 문희준 1명**이고 정윤미·이민화·윤상훈은
`role=USER` + `SUPPLY_MANAGE`였다. 즉 **ADMIN 한 명 빼고 전원이
"버튼은 보이는데 누르면 항상 실패"** 상태였다. 같은 데이터를 손으로 넣는
`createStock`도, 같은 성격의 생산자 엑셀 import도 이미 `SUPPLY_MANAGE`인데
**엑셀 import 한 곳만 빠져 있었다.**

- 🔴 **에러가 "파일 분석 중 오류"로만 뭉개진 이유**: `requireAdmin()`이 `try` 블록
  **밖**이라 `ForbiddenError`가 `ExcelImportResult`에 안 담기고 그대로 reject된다
  → 클라이언트 `catch`가 generic 토스트만 띄운다. 실패 지점이 **dry-run**이라
  "몇 건 중 몇 건" 미리보기 요약조차 안 떴다 — 그래서 증상이 더 알아보기 어려웠다
- 📌 **교훈: UI 노출 조건과 서버 가드는 한 쌍이다.** 어긋나면 "보이는데 안 되는"
  기능이 되고, 쓰는 사람에게는 **자기 PC 고장처럼 보인다**
- 재로그인 불필요 — 버튼이 보였다는 게 세션에 이미 `SUPPLY_MANAGE`가 있다는 증거

**푸시 범위를 쪼갰다.** 로컬 main에 M1-1~M1-3(브라우저 확인 전 보류분)이 쌓여
있어서 그냥 밀면 확인 안 된 발주서 모바일 화면이 같이 배포된다. `origin/main`
위에 권한 커밋만 cherry-pick해 올리고 로컬은 rebase로 정리했다.
🔴 **푸시가 곧 배포인 저장소에서는 「무엇을 고쳤나」만이 아니라 「무엇이 같이 올라가나」를 본다.**

`tsc` 0 · **test 389/389**. eslint `any` 5건은 기존 코드(19·20·150·166·241행), 변경 위치와 무관.
파일: `app/actions/stock-excel.ts` · `docs/permission-matrix.md`(표 2곳+이력) · 계획서 신규.
계획서 `docs/plan/plan-26년산전환.md`. **정윤미 계정 실제 업로드 확인 대기.**

### 26년산 연도 경계 10월·11월 → 둘 다 9월 `527f07e`

26년산 조생종이 **9/15에 실제로 들어왔다.** 경계가 현실보다 한두 달 늦어
**DB에 26년산이 있는데 기본 목록에서 안 보였다**(`#2313` 슬로미2, 입고 9/15).

```
검색 RICE_HARVEST_MONTH  10 → 9    1~8월 ['2025'] / 9~12월 ['2026','2025']
등록 RICE_NEW_CROP_MONTH 11 → 9
```

- 🔴 **주석의 근거가 무너진 것이라 값과 같이 고쳤다.** "10월엔 아직 당해년도 벼가
  안 들어온다"를 남겨두면 다음 사람이 되돌린다
- 두 값이 같아졌지만 **상수는 합치지 않았다** — 검색 범위와 등록 기본값은 다른
  판단이고, 실제로 한 번 갈렸던 이력이 있다
- 🔴 **테스트 하나는 값 수정이 아니라 삭제였다.** `벼 검색은 10월에 올해를 포함하지만
  등록은 전년을 찍는다`는 두 경계가 같아지면서 **주장 자체가 폐기**됐다. 12개월 전체로
  "두 경계가 같다"를 보는 테스트 + 26년산 첫 입고 회귀 테스트로 교체
- 📌 **증상은 「등록했는데 사라졌다」로 나타난다** — 연도 경계는 저장이 아니라
  **표시**의 문제라 데이터를 의심하면 헛짚는다

### 생산연도 입력 셀렉트 전환 + 연도 목록 단일 원천화 `e959247`

브라우저 확인 중 사용자 지적 — **벼 입고 등록창만 연도를 텍스트로 친다.**
같은 화면의 잡곡은 이미 셀렉트였다. 실동작도 나빴다:

- `parseInt(v) || defaultYear` — **지우면 기본값으로 튀어** 다시 칠 수 없다
- 타이핑 중간값 `2`가 그대로 들어가 생산자 목록이 `cropYear !== 2`로 전부 걸러진다
- `min`/`max`가 없어 `226`·`20026`이 통과한다

🔴 **오타의 대가가 크다.** 바로 위에서 고친 그 증상("등록했는데 사라졌다")이 나오고,
톤백 중복검사가 `(연도+생산자+품종+번호)`라 **연도가 틀리면 중복 검사까지 무력화**된다.

**같이 푼 것 — 같은 `[2026..2023]`이 화면 5곳에 복붙돼 있었다**(`farmer-filters`만
3년치로 따로 굳어 있었다). **2027년이 되면 다섯 곳을 손으로 고쳐야 하는 구조**였다 —
`production-year.ts`가 애초에 풀려던 문제가 연도 목록에 그대로 남아 있던 셈이다.

```
productionYearOptions(now)            올해부터 과거 3년, 최신 앞
productionYearFilterOptions(now)      MultiSelect용 {label, value}
productionYearOptionsWith(value, now) 목록 밖 값을 끼워 넣는다
```

- 🔴 **`productionYearOptionsWith`가 핵심 방어다.** 수정 다이얼로그는 옛 재고를 여는데
  연도가 목록에 없으면 Select가 **빈칸으로 열리고 저장하는 순간 연도가 날아간다**.
  지금 DB엔 2025·2026뿐이라 안 터지지만 시간이 지나면 반드시 온다
- ⚠️ **Select는 `name`으로 FormData에 안 실린다** — 두 다이얼로그의
  `formData.get('productionYear')`를 state 직접 참조로 같이 고쳤다
- ⚠️ `farmer-filters`는 3년 → 4년으로 **늘어난다**(통일). 작목반 `cropYear` 필터라 무해

9파일 · `tsc` 0 · **test 397/397**. eslint는 **변경 전후 16건으로 동일**(전부 기존 코드).

### 대시보드 집계 기준 연도 — DB 최신 → 11월 경계 `2be8593`

배포 후 사용자 확인: **대시보드가 26년 기준으로 세팅돼 버렸다.**
위 연도 경계(9월)와 무관했다 — `dashboard.ts:11`이 기준을 `production-year.ts`가
아니라 **「DB에 있는 가장 최신 productionYear」**로 잡고 있었다.
...
🔴 **신곡이 한 톤백만 들어와도 대시보드가 통째로 넘어간다.** 26년산 15행이
적재되자 실제로 그랬다. 아직 25년산 **40만kg이 주력**인데 화면이 빈 것이다.

- **집계 기준은 등록·검색(9월)과 일부러 다르게 11월에 넘긴다.** 등록·검색은 신곡이
  들어오는 즉시 보여야 하지만, **한 해를 통으로 보는 값(진행률·수율)은 신곡 몇
  톤백에 기준이 옮겨가면 의미가 사라진다**. 두 해를 섞지도 않는다(사용자: 「연도가
  섞이면 의미가 희석될 것 같다」) · 최근 도정 내역은 제외(원래 필터 없음, 그대로)
- ⚠️ **폴백 한 줄** — 신곡 입고가 11월을 넘기면 그 해 재고가 0일 수 있다. 그땐
  전년으로 물러선다(11월 1일에 화면이 통째로 비는 것보다 낫다)
- 📌 **같은 「생산연도」라도 화면 성격에 따라 경계가 다르다.** 목록은 들어오는 즉시,
  집계는 주력이 넘어간 뒤. 한 상수로 묶으려 하면 둘 중 하나가 반드시 틀린다

**연도 토글은 넣지 않았다(사용자 결정).** 비용을 재보니 구현은 작지만 전환마다
**2초 재조회**가 붙고 모바일 카드가 터치 스와이프 캐러셀이라 충돌 확인이 필요했다.
무엇보다 11월 경계면 **오늘부터 내년 10월까지 25년산이 계속 보여** 당장 필요가 없다.

**곁가지 — 🔴 결과를 아무도 읽지 않는 쿼리 2개를 지웠다.** `millingBatch.count()`
(`totalBatches`)와 투입량 `millingBatch.aggregate`가 반환 객체에 담기기만 하고
**화면 참조가 0**이었다(수율 분모는 9번 `yearBatches`가 따로 구한다 — 결정 #61).
실측 **418ms → 209ms**. 대시보드는 쿼리 병렬에도 1.8~2.2초 걸리는 무거운 화면이다.

`tsc` 0 · **test 401/401** · eslint 미사용 경고 해소(`any` 3건은 기존 코드).

### 작목반 등록 연도 셀렉트 · 통계 3곳 연도 기준 통일 `35f37e7`

사용자 지적: **작목반 새로 등록할 때도 연도가 텍스트 입력**이었다.
`type="number"`조차 없는 순수 `Input`이라 벼 등록창보다 더 열려 있었다.

🔴 **작목반 `cropYear`는 표시용이 아니다** — 재고 등록 화면이
`group.cropYear !== productionYear`로 생산자를 거른다(`add-stock-dialog:61`).
잘못 치면 **그 작목반 생산자가 재고 등록에서 통째로 안 보인다.**

**훑다가 같은 성격을 3곳 더 찾았다.**

- `statistics/milling` · `statistics/millingtype` — `today.getMonth() >= 9`
  (**10월 경계**)가 두 페이지에 복붙돼 있었다. 대시보드는 11월에 넘어가는데
  통계만 10월에 먼저 넘어가 **10월 한 달 동안 두 화면이 다른 해를 가리킨다.**
  오늘은 둘 다 2025라 티가 안 났다 — 열흘 뒤면 갈라졌을 것이다
- `statistics/stock` — `initYear`를 `productionYears[0]`(**DB 최신**)로 잡고 있었다.
  🔴 **대시보드가 겪은 것과 같은 구조**라 여기도 26년산으로 열리고 있었다.
  집계 기준 연도가 목록에 있으면 그걸 쓰고, 없을 때만 목록 최신으로 폴백

```
작목반 등록 기본 2026 · 셀렉트 [2026, 2025, 2024, 2023]
통계 도정·도정구분     2025
재고 통계 initYear     2025   (이전 로직이면 2026)
2026-11-01 시뮬 →      셋 다 2026
```

📌 **연도 규칙이 두 개로 정리됐다** — 목록·등록 **9월**(원물·제품재고, 벼·잡곡·작목반
등록, 생산자 필터) / 집계 **11월**(대시보드, 통계 도정·도정구분·재고).
작목반은 「그 해 농사를 짓는 작목반」을 만드는 것이라 등록 쪽(9월)에 뒀다.

⚠️ Select는 `name`으로 FormData에 안 실린다 — `formData.get('cropYear')`도 같이 고쳤다.
`tsc` 0 · **test 401/401**. eslint 잔여(`any` 2 · `resolveGroupBy` 미사용)는 HEAD에도 있던 것.

### 작목반 기본 검색 필터 제한 (커밋만 · 확인 전)

사용자 지적: **작목반 검색 필터 기본이 「전체」라 제한이 필요하다.**

🔴 **이 화면은 필터 위젯 기본값만으로는 안 걸린다** — 조회는 URL만 본다(`page.tsx`).
메뉴로 그냥 들어오면 전 연도가 쏟아졌다. **서버에서** 기본값을 걸었다.
...
최근 2년([2026,2025])   → 작목반 71/75 · 생산자 163/179
```

26년산 작목반이 **1개뿐**이다(신곡 작목반은 수확기에 하나씩 만들어진다).
📌 **「기본 연도」를 새로 거는 곳마다 실제 행 수를 세어볼 것** — 바로 위에서 고친
대시보드와 같은 함정이다.

- `cropYear=ALL`이 명시적 전체 탈출구(원래 `page.tsx`에 처리가 있었다). 연도를 모두
  해제하면 `ALL`을 보낸다 — 파라미터를 생략하면 서버가 기본값을 다시 끼워 넣는다
- 위젯 기본값도 서버와 같은 값으로. 어긋나면 **검색창은 「전체」라 말하는데 목록은 두 해만** 나온다

**📌 앞선 진단 정정.** 오늘 1단계 조사에서 「26년산을 등록해도 기본 목록에서 안 보인다」고
했는데 **과한 표현이었다.** `raw-stocks/page.tsx`는 URL에 `productionYear`가 없으면
전체를 조회하고 메뉴 링크에도 파라미터가 없다 — 메뉴로 들어가면 26년산은 보였다.
목록이 실제로 걸러지는 건 「적용」·「초기화」로 URL에 연도가 박힌 뒤다(2단계 수정 자체는 유효).
🔴 **「기본 필터」를 말할 때 위젯 기본값과 서버 조회 조건을 구분할 것.**

### ~~🔴 미해결 — 도정관리 모바일에서 포장내역을 처음 등록할 수 없다~~ → **진단 빗나감 (2026-09-22 정정)**

> 🔴 **아래 진단은 틀렸다.** 2026-09-22 사용자 확인 결과 **상태 배지로 잘 들어가고 있었고**,
> 실제 신고는 **「포장 등록 화면에 규격 버튼 자체가 안 나온다」**였다 — 목록이 아니라 **다이얼로그 안**이다.
> 아래에 적힌 데스크탑/모바일 코드 차이는 **사실이지만 막힌 길이 아니었다**(별건으로 남김).
> 📌 **재현하지 못한 채 코드만 보고 세운 진단을 단정형으로 적어서 생긴 일.** 「추정」이라고 적었어야 했다.
> 진짜 원인과 조치는 2026-09-22 자 「포장 다이얼로그 — 규격 버튼이 안 보이던 진짜 원인」 참고.

사용자 신고: 「도정관리 목록에서 포장내역 등록하려는데 모바일에서만 안 된다.」
**데스크탑과 모바일의 코드가 다르다.**

```tsx
// milling-table-row.tsx:161  (데스크탑) — onClick이 바깥 span에 있다
<span onClick={...setPackagingOpen(true)} title="포장 내역 보기">
  {totalRiceKg > 0 ? totalRiceKg.toLocaleString() : <span>-</span>}
</span>                                    ↑ 0이어도 「-」를 누르면 열린다

// mobile-milling-card.tsx:157  (모바일) — 버튼 자체가 조건부다
{totalRiceKg > 0 ? (
    <button onClick={...setPackagingOpen(true)}>…kg</button>
) : (
    <span className="text-[10px] text-slate-300">-</span>   ← 죽은 텍스트
)}
```

🔴 **산출이 0인 배치(= 포장을 이제 처음 등록하는 배치)에서 모바일엔 진입점이 없다.**
남은 경로는 상태 배지(`handleStatusClick`)뿐인데 「진행중/마감」 배지라 포장 등록으로
읽히지 않는다. 카드 본문을 누르면 **재고 목록**이 열린다(`handleCardClick`).

⚠️ **지금 DB로는 재현 불가** — 200배치 전부 산출이 있다(오늘 만든 `#237`·`#238`도 이미
등록됨). 재현하려면 새 배치를 만들고 포장 전에 모바일로 들어가야 한다.
📌 [[affordance_not_missing_feature]]의 **반대 사례** — 그때는 기능이 있는데 안 보인
것이었고, 이번엔 **데스크탑에만 있고 모바일엔 실제로 없다**.

**수정 방향**: 모바일도 데스크탑처럼 `-`를 누를 수 있게 조건부 렌더를 풀면 된다.
다만 화면 변경이라 브라우저 확인이 필요해 이번 세션에선 손대지 않았다.

**확인 중 별도로 발견 — 🔴 `importStocks`가 dry-run에도 감사로그를 남긴다.**
`recordAuditLog` 앞에 `dryRun` 분기가 없어 **미리보기만 하고 취소해도
"가져오기 완료 성공 14건"이 찍힌다.** 오늘 IMPORT 로그 3건 중 실제 저장은 1건뿐이었다.
감사로그가 실제보다 부풀어 있다 — 별도 작업 후보(계획서 §6).

### 발주서 M1-3 — 건상세 확장(완료 접기 · 푸터 일괄차감 · 다음 건)

먼저 **미커밋이던 M1-1·M1-2를 커밋**했다(`33c568f`). 이어 M1-3을 얹었다.

지시서 §4.0이 꼽은 4개 중 셋을 붙였다 — 라인 탭 → FIFO는 시트가 M1-5라 거기서.
**새 서버 액션 0개**: 집계는 M1-1의 `sumOrderLines`를 꺼내 쓰고, 일괄차감은
**이미 있는 검토 게이트를 건 하나짜리 `orderIds`로 여는 것**이다(전용 확정 경로를
지었으면 4갈래 구성이 두 벌이 된다).

- 🔴 **「다음 건」 순서는 열 때 찍은 스냅샷.** 모바일 목록 기본 필터가 「작업 필요」라
  차감하는 순간 그 건이 목록에서 빠지고 뒤 건이 당겨진다 — 살아 있는 목록으로
  `indexOf + 1`을 하면 **바로 다음 건을 건너뛴다**. 순서 원천도 `shown`이 아니라
  **그룹으로 묶은 뒤**(발주처별 정렬은 그룹을 건수순으로 다시 세운다)
- 건마다 `key`로 새 마운트 — 스크롤·접힘을 물려받으면 새 건을 중간부터 보게 된다
- 완료 그룹은 **작업 필요가 있을 때만** 접는다(없으면 그게 화면의 내용이다)

**곁가지 — 🔴 모바일에서 게이트 「이 줄」이 무반응이었다.** 폰에는 갈 셀이 없다
(매트릭스가 `hidden sm:contents` = `display:none`). `querySelector`는 찾지만 스크롤이
아무 일도 안 한다 — **D3 `data-cell` 사고(`dc5651b`)와 같은 증상**. 브레이크포인트를
JS에 복제하는 대신 **그려져 있는지를 DOM에 묻고**(`offsetParent`) 아니면 건 상세로
데려간다. `lineIndex`에 `orderId` 추가(`cellKey`를 쪼개 쓰지 않는다).
데스크탑에서 셀로 갈 때는 **패널을 닫는다**(열린 채면 468px이 그 셀을 덮는다).
...
### 발주서 D4-4 종결 — 매칭실패 23 → 0 `verify`

사용자가 품종·SKU 마스터를 채우고 재매칭을 돌려 **5채널 126라인 전부 매칭**됐다.

```
#15 택배 79 / #16 해남급식 10 / #17 이마트 9 / #18 서울급식 24 / #19 시아스 4  — 실패 0
```

**마지막 1건은 포장지 문제가 아니었다** — 「유기농 프로틴 라이스 IPS」907g(#15 네이버스토어).
엑셀 포장지 칸이 빈칸(`rawPackaging=null`)이라 매처가 **포장지 이름을 아예 안 본다**
(`purchase-order-matcher.ts:218` — 기본 SKU, 없으면 후보 유일할 때 채택). 즉 `IPS/백미/907g`
조합으로 SKU를 하나만 만들면 포장지가 무엇이든 붙는 상황이었고, 포장지 선택은 **매칭 제약이
아니라 실물 문제**였다. 📌 **「포장지가 없어서 안 붙는다」가 언제나 참은 아니다** — 빈칸 라인은
포장지 축이 매칭에서 빠진다.

신규 SKU `id=89 IPS / 백미 / 907g`. 포장지는 사용자가 신규 등록(`id=11`).

### 포장지 이름 변경 「인디카」 → 「인디카라이스」 `data`

DB 직접 변경(코드 변경 없음). 사용자 요청 — 「인디카」는 원래 품종 축 단어(`Variety.type=INDICA`)라
포장지 드롭다운에서 혼동될 이름이었다.

바꾸기 전 영향 범위를 먼저 셌다: 이 포장지를 쓰는 SKU 1종(id=89) · 🔴 `rawPackaging`에 「인디카」를
문자열로 들고 있는 발주서 라인 **0건**. 후자가 이름 변경으로 깨질 수 있는 유일한 경로였다
(이미 매칭된 라인은 `productTypeId`가 박혀 있어 당장은 멀쩡해도 **재매칭 때 터진다**).
🔴 DB를 직접 고쳤으므로 열어둔 화면은 새로고침 필요.

### 열 머리글에서 잡곡 sentinel「기타」 생략 `c35a129`

품종명 뒤 `· 기타`를 뺀다. 생략 대상을 `HIDDEN_MILLING` 상수로 묶고 **사유를 갈라 적었다** —
백미는 「대부분이라 정보량이 없다」, 기타는 「`MISC_MILLING_SENTINEL`이라 도정 개념이 없다」로
근거가 다르다. 안 적으면 「기타도 도정 하나인데 왜 빼」가 다시 나온다.

🔴 **그룹 식별자는 안 건드렸다.** `groupKeyOf`는 `sku.millingType` 원본을 쓰므로 라벨만 바뀌고
열 묶임은 그대로다 — 표시값과 식별자가 원래부터 분리돼 있어서 공짜로 안전했다(`LotGroup.groupKey`
때와 같은 구조). 호출처도 `buildMatrix` 1곳.

영향 34라인 · 12종(귀리·차조·수수·흑미·발아현미·검정보리·기장·찰보리·누룽지·녹두(친환경)·
율무(친환경)·혼합곡(친환경)). test 379/379 · tsc · eslint.

### D4-5 — 사용자 화면 확인 완료, 차감은 #16 1건

5채널 화면 대조를 사용자가 확인했다. **차감 movement는 #16 해남급식 1건(66개)만 존재**하고
나머지 4채널은 0건 — 발주 status `PARTIAL 1 · PENDING 75`.
⚠️ 이 66개가 **시험분인지 실업무 차감인지 미확인**(시험분이면 D4-1과 같은 3단계 정리 필요:
하드삭제 + `recalcOrderStatus` + 감사로그).

**푸시** `5b6238d`까지 5커밋.

### D4-5 시험 차감 정리 — 묶음 #16 `3d43413`

사용자 확인: 오늘 차감 1건은 전부 시험분(기능 미오픈). `cancelCell` 3단계를 스크립트로 옮겨
정리했다 — 하드삭제 → `recalcOrderStatus` → 감사로그. 🔴 `movement`만 지우면
`PurchaseOrder.status`가 라인 집계 파생값이라 「완료」로 굳는다(D4-1과 같은 함정).
결과 `PARTIAL 1 → PENDING 76`, 백업 JSON은 저장소 밖.

### 🔴 상차정보 「수정이 안 된다」 — 기능은 있었고 없던 건 어포던스였다 `e5af21a`

사용자 요청은 「상차정보도 수정할 수 있어야 한다(보내기 전까지 바뀌니까)」였는데,
전수 대조해 보니 **수정은 원래부터 됐다**. `LoadingCell` 팝오버는 상태와 무관하게 열리고
기존 값을 전부 프리필하며, 비우면 「배차 미정」으로 되돌아간다. 데스크탑·모바일 공용.

없던 것은 **그 사실을 알릴 방법**이었다 — 「배차 미정」만 점선+＋로 눌러보고 싶게 생겼고,
채우고 나면 회색 텍스트라 클릭 가능한지 알 수가 없었다. 사용자에게 선택지를 물어
**목록 셀 발견 가능성만** 손보기로 확정(매트릭스 화면 편집 추가는 보류).

📌 **「기능이 없다」는 신고가 「안 보인다」인 경우** — D4에서 세 번째로 나온 같은 결의 함정이다
(앞의 둘은 「계획서에 남아 있다 ≠ 아직 안 됐다」).

🔴 **어포던스를 호버에만 걸지 않았다.** 이 셀은 모바일 묶음 목록에서도 같은 컴포넌트를 쓴다 —
호버 없는 기기에선 연한 연필이 유일한 단서다. 판단 근거를 `EditHint` 주석에 박았다.

### 묶음 목록 열 간격 재배분 `d731f36`

시트명이 `1fr`로 남는 폭을 독점해 상차까지 빈 구간이 뜨고, 수령인(우측정렬 숫자)과
진행(좌측정렬 배지)은 `gap-2` 하나만 사이에 두고 붙어 있었다. 남는 폭을 시트명 0.9fr ·
진행 1.1fr로 나누고(진행은 배지 3개라 180px 고정이면 줄바꿈), 상차 184px 고정,
수령인엔 `pr-3`(열을 넓히면 숫자가 중앙으로 뜬다).

### 🔴 「매트릭스엔 재고부족인데 목록엔 대기」 — 버그가 아니라 축이 다르다

사용자 질문. 확인 결과 **계산은 맞다.** 목록의 진행 배지는 `PurchaseOrder.status`만 세는
**차감 진행률**(대기=아직 차감 안 함)이고, 매트릭스의 「재고부족」(`SHORTAGE`)은 **가용재고 대비
주문량** 판정이다. `listPurchaseUploads`는 재고를 **한 번도 조회하지 않는다**(status ·
productTypeId · movements 개수만).

📌 즉 **재고부족은 목록에 존재하지 않는 정보**다. 매칭실패는 열이 있는데 재고부족은 없어,
상차 임박순 목록에서 「내일 나갈 건이 지금 나갈 수 있나」를 알 수 없다. ⚠️ 넣으려면 묶음마다
가용재고 조회가 붙는다(매트릭스 한 화면이 쿼리 9회·1.4초) — SKU를 한 번에 모아 조회하는
별도 설계가 필요해 **미결로 남긴다.**

**다음 = M1 모바일 3모드**(경계값을 D4-6 실측으로 확정 — 🔴 #17이 표준으로 가는 게 맞는지가
판단거리). 미결: 목록 재고부족 열.


## 2026-09-17

### 발주서 D3 브라우저 확인 — 실제 차감 1회 + 시험분 되돌리기 `verify`

어제 미뤄둔 **쓰기 경로 검증**을 끝냈다. 코드 변경 없음(검증·문서만). 보고서 §6 갱신.

**🔴 게이트 숫자와 실제 차감이 완전히 일치했다**

택배 묶음(#15) 전체선택 → 확정까지 실제로 돌리고, DB를 직접 읽어 대조했다.
게이트 「371kg · 포장 59개 · 로트 8개 · 31라인」 = DB 「movement 31건 · 59개 · 371.0kg ·
package 8개 · 라인 31개」. **dry-run 계획이 확정 결과와 한 글자도 안 틀렸다** — 게이트와 확정이
같은 순수함수·같은 순서를 쓴다는 설계가 실제로 지켜졌다는 뜻. 안전 검사도 전부 0건:
가용 음수 0 · 주문 초과 0 · `type≠SALE` 0 · 부분 차감 0(`partial`/`none` 분리가 실데이터와 맞음).

**🔴 보고서 §5 표와 화면 숫자가 달랐던 건 데이터가 움직인 것이었다**

어제 재현표는 `49/0/18/11 · 683kg`인데 오늘 게이트는 `31/17/11 · 371kg`. 추측하지 않고
movement 생성 시각을 봤더니 어제 브라우저 확인 중 **셀 단위로 19건을 차감해 둔 것**이 있었다
(+9/14 1건). 20라인이 「이미 전량 차감」으로 구성에서 빠져 79 → 59라인. 49−18=31, 18−1=17로
정확히 떨어진다. 📌 **재현 스크립트 수치는 그 시점의 스냅샷이다 — 며칠 뒤 화면과 다르면
코드를 의심하기 전에 데이터가 움직였는지부터 본다.**

**시험 차감 51건 되돌리기**(사용자 판단 — 한 번에 31라인은 셀 단위 취소로 되돌리기 부담)

uploadId=15의 SALE movement **51건 전부 하드삭제**(오늘 31 + 어제 19 + 9/14 1).
`cancelCell`이 하는 3단계를 그대로 스크립트로 옮겼다 — 🔴 **movement만 지우면 안 된다.**
`PurchaseOrder.status`는 라인 집계 파생값(#12)이라 `COMPLETED 45 · PARTIAL 4`로 굳어 있었고,
`recalcOrderStatus`를 안 돌리면 화면이 「완료」라고 거짓말을 한다. 하드삭제 → status 재계산 49건 →
감사로그 1건(삭제된 51건 전문을 `details`에 보존, 별도 JSON 백업은 저장소 밖).
사후 `PENDING=67`, 건드린 제품재고 9행 가용 전부 복원.
🔴 **DB를 직접 고치면 열어둔 화면은 안 바뀐다**(9/9 차감 정리 때와 같은 함정) — 새로고침 필요.

**푸시** `f2a886a` · `dc5651b` · `6db0c5a` · `c5552e9` + 이 문서 커밋. **다음 = D4**
(나머지 채널 대조 · 발주처≠수령인 표기 · ⋮ 메뉴 마감). 미확인으로 남긴 것: 확정 소요 시간
(갱신은 됐고 시간만 미측정) · 모바일 선택 바 위치(§8).


### 발주서 D4 착수 — 계획서 + 매처 결함 2건 `feat`

계획서 `docs/plan/plan-발주서판매처리-D4.md`. **코드 수정은 매처 2건이 전부**고, 나머지는
대조·실측·마스터 채우기다. 🔴 **미푸시**(재매칭이 아직 안 돌았다).

**🔴 상위 계획서 D4 4항목 중 2개가 이미 끝나 있었다**

- ②「발주처≠수령인 `A → B` 표기」 = **D2c에서 완료.** 표기법도 `A → B`가 아니라 **2단**으로 대체됨
- ③「⋮ 메뉴·비고 수정 마감」 = **D1c에서 완료.** 남은 `disabled`는 엑셀 다운로드인데 그건 **D5**
- ④ 5채널 업로드 = 데이터가 **이미 5채널 1묶음씩 적재돼 있다**
- 📌 **D3와 같은 함정 — 「계획서에 남아 있다」가 「아직 안 됐다」는 뜻이 아니다**

**시안 4개도 낡아 있었다** — 「보관 차감」(`v=보관차감`)은 결정 #28(음수금지→비고)로,
금액(`price`)은 결정 #25로 **이미 폐기된 개념**이다. 생산자 표기·`포` 단위는 사용자가 **안 넣기로**.

**🔴 매칭실패 23라인을 매처로 전수 분류했더니 65%가 이름 문제가 아니었다**

재매칭이면 끝 3 · **SKU 카탈로그 빈칸 12** · 포장지 조합 없음 3 · 품종 해석 실패 5.
📌 **D2e 교훈 재확인 — 수동지정을 철회한 판단이 옳았다.**

**결함 A — 브랜드 접두를 한 번만 뗐다** `3b63982`

`stripBrandPrefix`가 `for` 첫 매칭에서 `return`해 **겹친 접두를 못 뗐다.**
「유기농 프로틴 라이스 IPS」가 `유기농`만 떨어져 「프로틴 라이스 IPS」로 남았다.
→ 더 못 뗄 때까지 반복. 효과: 이 라인이 `variety_unresolved` → `sku_unresolved`로 이동.
**바로 풀리진 않지만 「품종을 모르겠다」가 「907g SKU가 없다」로 바뀌었다** — 실패 사유가
정확해야 사용자가 무엇을 등록할지 안다.

**결함 B는 결함이 아니었다 — 사용자 확인이 뒤집었다**

「녹두 (친환경)」이 실패하는 걸 보고 **정규화가 괄호를 안 뗀다**고 봤는데,
🔴 **`품종(친환경)`·`품종(일반)`은 매입 잡곡의 명칭 체계**이고 괄호는 **품종명의 일부**였다.
떼면 「율무 (친환경)」이 율무(id=41)로 붙어 **율무(친환경)(id=52)의 SKU를 못 쓴다.**
실증: D4-1 정리 때 **「율무 (친환경)」이 이미 정상 매칭돼 차감까지 돼 있었다.**
→ **코드 수정 취소.** 「괄호를 떼지 않는다」를 주석으로 박았다.

**결함 A' — 브랜드명이 곧 품종 별칭인 경우** `4739176`

사용자가 `프로틴 라이스`→IPS · `자스민 라이스`→CJ6를 **별칭으로 등록**했는데,
이 둘이 `BRAND_PREFIXES`에도 있어 **접두 제거가 먼저 돌아 브랜드명이 통째로 잘렸다** —
빈 토큰이 되어 **별칭을 볼 기회 자체가 없었다**(시뮬레이션으로 실증).
→ **남은 게 접두 하나뿐이면 떼지 않는다.**

⚠️ 처음엔 「빈 토큰이면 원본을 쓴다」로 잡았다가 **틀린 걸 알았다** — 「유기농 프로틴 라이스」의
원본은 `유기농 프로틴 라이스`라 별칭 `프로틴 라이스`와 **정확일치가 안 된다.** 필요한 건
「빈 문자열이 되기 **직전** 값」이었고, 그건 **아예 빈 문자열을 만들지 않으면** 자연히 얻어진다
(`return ''` → `return out`). 🔴 **접두 목록에서 브랜드를 빼도 안 된다** — 별칭은 정확일치라
`프로틴 라이스` ≠ `프로틴 라이스 IPS`가 되어 조합형이 깨진다.

**D4-1 시험 차감 정리** — #16·#18·#19의 movement **9건 하드삭제 + status 재계산 6건 + 감사로그**.
🔴 `movement`만 지우면 `PurchaseOrder.status`가 「완료」로 굳은 채 남는다(D3에서 배운 것).

**결정** ㉮ `(친환경)`=매입 잡곡 명칭 → 품종 등록, 코드 수정 없음 ·
㉯ 혼합곡은 ㉮와 같은 건 / **누룽지 = 품종 등록**(`PURCHASED` → 잡곡 탭에 임시 배치).
누룽지는 **부산물을 제조업체에 주고 완제품으로 받아오는 가공품**이라 `GERMINATION`(발아위탁)과
구조가 같다. 정확한 분류는 「가공품」 탭이지만 **한 종뿐이라 만들지 않는다** → **백로그 §44**
(미뤄도 비용이 안 는다 — 나중에 품종 type 한 칼럼만 바꾸면 통째로 옮겨간다).

검증 tsc · **test 378/378**(신규 6) · eslint. **다음** 사용자 화면 작업(품종 2·SKU 8종·재매칭)
→ 남은 매칭실패 수 확인 → **D4-5 5채널 화면·차감 검증**(🔴 서울급식 #18이 이름칸 순서가
뒤집힌 유일한 채널) → 푸시.
---

## 2026-09-16

### 발주서 D3 — 행 일괄선택 + 검토 게이트 `feat`
...
- 🔴 **「재사용 가능」이라던 `confirmOrderItem`·`cancelOrderItemMovements`가 호출처 0건이었다.**
  `confirmOrder`·`listPurchaseOrders`까지 **죽은 코드 4개** → 삭제(445→297줄). 현행 차감 경로는
  `confirmCell`/`cancelCell`. 같은 이름의 옛 일괄차감이 남아 있으면 다음에 또 헛짚는다.
  📌 **교훈(어제 품종관리와 같은 것): 「기존 X를 그대로 쓴다」는 X가 살아 있는지부터 본다**
- 시안의 「SKU 지정하러 가기」는 **오늘 철회한 수동지정**(`c2fc6f3`) → 재매칭 안내로 교체
- 시안 「라인마다 개별 취소」 → 실제 취소 단위는 **셀**
- 톤백(D2d)은 시안에 없던 갈래 → **일괄에서 제외** + 안내
- 「1,700라인」 전제 → 실측 최대 **67건 79라인**

**🔴 이번 작업의 알맹이는 UI가 아니라 쓰기 경로였다**

`applyAllocations`는 **배분 1건당 쿼리 4회**(기차감 집계·재고 조회·사용량 집계·INSERT)를 쓴다.
Neon 왕복 250~300ms라 68라인이면 **100초+** — 트랜잭션 timeout 30초를 넘긴다(2026-08-26 적재
사고와 같은 뿌리). 그래서 **배치 경로**를 새로 깔았다: 조회 4회 → 메모리 배분 → `createMany` 1회 →
건 상태 `updateMany` ≤3회. **라인 수와 무관하게 왕복 ≤ 8회.**

🔴 **dry-run을 `suggestAllocation` 그대로 돌리면 안 된다** — 같은 SKU를 쓰는 라인들이 **같은 재고를
각자 세어** 차감예정이 부풀려진다. 가용 풀을 깎아가며 `itemId` 오름차순으로 순차 배분하는
`planBatchAllocations`를 만들고, **게이트와 확정이 같은 함수·같은 순서**를 쓰게 했다.
확정은 `fingerprintBatchPlan` 지문으로 대조해 **어긋나면 아무것도 쓰지 않는다**(부분 커밋 없음).

**🔴 실측이 잡아낸 결함 1건 — 「부분 차감」이 아니었다**

실데이터로 재현해 보니 택배 묶음의 **부족 18라인이 전부 가용 0**이었다. 처음엔 이걸 전부
「부분 · 재고부족」으로 셌는데, 가용 0이면 부분 차감이 아니라 **아무 일도 일어나지 않는다**.
`partial`(일부만)과 `none`(차감 0)을 갈라 스택 바를 4구간으로. 시안이 3갈래였던 건
**시안의 예시 데이터가 그렇게 생겼을 뿐**이고 실제 데이터는 다르다.

**신규** `lib/purchase-order-batch.ts`(+테스트 16) · `app/actions/purchase-order-batch.ts` ·
`review-gate-dialog.tsx`. **수정** `matrix-client.tsx`(체크박스 칸·선택 바·셀 강조,
sticky 좌표는 상수 한 곳만) · `purchase-order.ts`(죽은 코드 삭제).

검증: tsc 통과 · `npm test` **372/372** · 신규/수정 파일 eslint 클린 · **실데이터 5묶음 재현**
(#15 683kg·114개/49라인 · #19는 톤백뿐이라 확정 0라인 = 의도한 동작). 계획 계산 0~3ms.
⚠️ 과대 계상은 현 데이터로 재현 안 됨(겹치는 SKU는 재고가 넉넉) → **단위테스트로 고정**.

**브라우저 확인 중 잡은 결함 2건** (같은 날 후속)

1. **게이트 「셀로 이동」이 무반응** `dc5651b` — 🔴 `Th`가 props를 **명시적으로만** 받아
   `data-cell`이 DOM까지 가지 않았다. `data-*`는 JSX에서 임의 허용이라 **타입이 안 잡아주고**,
   `querySelector`는 못 찾으면 조용히 아무 일도 안 한다 — 셋이 겹쳐 완전히 침묵했다.
   📌 **커스텀 래퍼에 `data-*`를 넘길 땐 그 래퍼가 DOM까지 흘려보내는지부터 본다.**
   같은 자리 방어 2건: 한 프레임 미뤄 스크롤(Radix 포커스 복원이 스크롤을 원위치시킨다) ·
   선택자 값 이스케이프(매칭실패 열 키는 엑셀 원본 문자열).
2. **가로 스크롤 시 소계·가용 줄만 튀어나옴** `6db0c5a` — 🔴 **`width`만으론 칸 폭이 지켜지지 않는다.**
   테이블 auto 레이아웃이 내용에 맞춰 칸을 늘리는데 sticky `left`는 상수라, **스크롤할 때만**
   뒤 칸이 당겨져 겹치고 `colSpan` 라벨만 실제 열 합 폭으로 남는다.
   실측(사용자 콘솔): 이름칸 184 지정 → **201로 렌더**, 좌측 합 387 vs 370.
   → `fixedW(w)={width,minWidth,maxWidth}` 6곳 적용 + `W_NAME` 184→204(실측이 원한 폭이 201이라
   억지로 묶으면 이름 잘림만 심해진다). D3 이전 3칸 시절에도 있던 어긋남이 칸이 늘며 드러난 것.

**같은 날 판단하고 안 고친 것** — 「매트릭스 열어둔 채 재포장했더니 가용재고가 그대로」.
재매칭 버튼으로는 안 된다(`rematchUpload`는 매칭실패 라인만 보고, 가용도 새로 매칭된 SKU만 읽는다).
한 사람 작업 흐름에선 페이지를 나갔다 오면 서버가 새로 읽으므로 문제가 안 된다 — **다른 창**을 띄운 예외였다(보고서 §7).

**다음** 🔴 **브라우저 확인이 7개 중 2개만 진행됐고 그 둘도 재확인 전.** 특히 **실제 차감(확정)을
한 번도 안 돌려봤다** — 게이트 수치↔실제 차감 일치 · 확정 후 갱신 · `/packages` 감소가 전부 미확인.
⚠️ 시험은 **행 2~3개만** 골라서(확정은 진짜 재고를 빼고 되돌리려면 셀마다 취소해야 한다).
그 뒤 **푸시**(커밋 3개 미푸시) → D4 나머지 채널.

---

### 네비게이션 덫 — 화면 튐 원인 추적 계측 `chore`
...
- ~~뒤로가기 제스처~~ — 사용자 폰은 **3버튼 네비게이션**이라 가장자리 스와이프 백이 존재하지 않는다
- ~~탭바 오터치 ghost click~~ — **실기기 검증**으로 기각(다이얼로그 연 채 '도정' 자리를 눌러도 다이얼로그만 닫힌다)
- ~~SW 캐시~~ — 캐시는 「그 주소에 틀린 내용」·「새로고침」까지만 가능. **리다이렉트 목적지 어디에도 `/milling`이 없다**
- `useEffect`·타이머의 자동 라우팅 0건, next-auth 목적지는 `/login`·`/`뿐
- → 남은 용의자는 **4개뿐**: A 장바구니 '수정' 버튼(`milling-cart-sheet.tsx:43`, **확인창 없이 즉시 저장+이동**,
  화면 최하단 가로 꽉 찬 버튼=왼손 엄지 위치) · B 도정시작 제출 · C 하단 탭바 · D 뒤로가기 · E 미지

**🔴 실측으로 걸러낸 결함 3건** (이번 작업의 실제 내용 — 셋 다 기록을 보고서야 드러났다)

1. **의심 배지 오탐** — 경로만 보고 판정해서, '도정관리'를 **직접 눌러** 간 정상 이동까지 빨개졌다.
   → 판정을 **원인** 기준으로: `원인 불명`·`뒤로가기`만 의심, `클릭`·`코드 이동`은 정상.
2. **뒤로가기가 화면 갱신보다 111ms 늦게 찍혔다** — Next가 `popstate`를 먼저 처리해 화면이 앞선다.
   「경로 변경 줄 **위**에 원인이 있다」는 전제가 깨진다. → capture 등록 + **변경 직후 1초까지 같은 묶음**(`after`).
3. 🔴 **「직전 행동 없음」이 거짓말이었다** — 직전 신호를 **모듈 전역 변수**에 들고 있었는데
   Next가 페이지를 옮기며 모듈을 다시 평가하면 초기화된다. → **저장된 기록에서** 찾도록 변경.
   📌 **교훈: 라우팅을 가로지르는 상태를 모듈 전역 변수에 두지 말 것.**

부수로 판정 규칙 하나 더 교정 — `popstate` 무조건 우선은 **3초 전 뒤로가기가 그 뒤 클릭을 이겨버린다**.
→ **가장 가까운 신호 하나**가 원인(예외: `after`의 `popstate`는 늦게 도착한 진짜 원인).

**설계 원칙**

- 🔴 **입력값은 기록하지 않는다** — 입력칸은 값 대신 종류만. 테스트 3개로 고정
- 🔴 **이벤트를 삼키지 않는다** — `passive`+capture, 읽기만. 덫이 증상을 가리면 잡을 게 없다
- 🔴 **조용히 실패한다** — localStorage 접근 전부 try/catch
- **시간 창으로 자르지 않는다** — dev에선 클릭→이동이 24초 걸리기도 했다(컴파일). 경과 시간을 적고 판단은 사람이
- 경로 변경은 **별도 슬롯 20건**에 따로 보관 + **줄 자체에 원인을 박았다**(버퍼에서 밀려나도 남게)

**신규** `lib/nav-trace.ts`(+테스트 33개) · `components/nav-trace.tsx`(UI 없음) ·
`app/(dashboard)/admin/trace/`(ADMIN 전용 보기 화면).
**수정**(1~2줄) layout · middleware · mobile-header · desktop-sidebar · breadcrumb-display ·
용의자 표식 3곳(milling-cart-sheet · start-milling-dialog · mobile-nav).

검증: tsc 통과 · `npm test` **356/356** · 신규 파일 eslint 클린 · 브라우저 3회(매회 결함 발견→수정).
⚠️ `mobile-nav.tsx`·`middleware.ts` 린트 에러 4건은 **변경 전에도 있던 기존 부채**(stash 확인).

**다음** 🔴 배포해야 작동한다(dev는 PWA가 꺼져 있고 증상은 실기기에서만).
튀면 `/admin/trace` → 「텍스트로 복사」. 🔴 용의자 A가 맞다면 감사로그의 `updateMillingBatchStocks`로
**이미 데이터가 바뀌었는지** 확인 필요(미실시).

**별건으로 드러난 실제 결함**(범위 밖·판단 필요): 🔴 `reloadOnOnline` 기본 켜짐 → `online` 이벤트마다
앱 전체 새로고침(**입력 중이면 날아간다**) · `others` NetworkFirst가 모든 페이지 요청을 가로채 10초 뒤
최대 24시간 된 캐시를 내줌 · `next-pwa` 5.6은 2022년 이후 배포 중단(Next 16 지원 밖, 후속 Serwist) ·
포장내역 다이얼로그가 **Server Action 3회 순차 왕복**(Next는 병렬로 안 보낸다 → DB 느린 순간 3배 증폭).

---

### 품종 관리 행 ⋯ 메뉴 + 곡종 필터 칩 · 검색 `feat`
...
- **지시서가 죽은 코드를 현행으로 착각했다.** 「삭제는 `DeleteVarietyButton`의 `handleDelete`를
  그대로 쓴다」고 했는데 그 파일은 **import만 되고 JSX 사용 0건**이었고, 6/4 이후 미수정이라
  **별칭 동반 소멸 경고(`883cd43`)가 없는 옛 버전**이었다. 실제 단건 삭제는 **수정 다이얼로그 푸터**에
  있었다. 따랐으면 어제 넣은 경고가 조용히 사라졌다 → 확인 로직을 `delete-variety.ts`로 **옮겨**
  다이얼로그와 ⋯ 메뉴가 한 함수를 쓰게 했다.
  📌 **교훈: 「기존 X를 그대로 쓴다」는 지시는 X가 살아 있는지부터 본다**
- `GRAIN_LABEL`을 새로 만들라 했지만 `lib/variety-labels.ts`가 **이미 있었다**(따랐으면 3번째 사본)
- 「기존 `typeOrder`」가 단수로 적혔지만 **2벌**이었고 **양쪽 다 `BLACK` 누락** →
  🔴 **흑미가 `?? 99`로 매입 뒤에 밀려 있던 정렬 버그**. 라벨·순서를 `lib/variety-labels.ts` 단일 원천으로
- ⋯ 메뉴를 생짜 `<button>`으로 제시했지만 앱에 **이미 7곳**(정본 `sales/upload-row-menu.tsx`) → 기존 패턴 채택
- 다이얼로그는 메뉴 밖 형제로 두고 `{editOpen && …}` 조건부 마운트 — **열 때마다 새 마운트**라
  상태 리셋이 구조적으로 보장된다
- 지시서 내부 모순 2건(`w-[52px]`↔`w-[8%]` · 「변경 없음」↔「prop 추가」), 전제 수치 낡음(36종→실제 41종)

**사용자 확인 중 나온 후속 3건**

- 칩 줄 여백이 어긋남 — 부모 grid `gap-1`(4px)이 이미 얹히는 걸 못 봤다. `pt-2 pb-2`로 양쪽 12px
- **삭제 확인 문구가 무섭다** → 제목 `확인`→`품종 삭제`, `정말 삭제하시겠습니까?`→`「흑미」 품종을
  삭제할까요?`, `매칭실패로 돌아갑니다`→`다시 이어 줘야 해요`. **정보는 줄이지 않고 품종명을 더했다**
- 🔴 **배경 딤이 꺼멓다** → `alert-dialog.tsx`만 `bg-black/80`, `dialog`·`sheet`는 `/50`이었다.
  같은 화면에서 수정은 은은하고 삭제만 확 어두웠다. **`/50`으로 통일**(앱이 이미 쓰던 값).
  영향: `confirmDialog` 33곳 + `AlertDialog` 직접 4곳. 📌 **「너무 어둡다」는 취향 진술로 들려도
  먼저 같은 앱의 같은 종류 값과 대조한다** — 이번엔 일관성이 깨진 자리였다

**변경 파일** — 신규 `variety-row-menu.tsx`·`delete-variety.ts`, 삭제 `delete-button.tsx`(죽은 코드)·
`use-bulk-delete-varieties.tsx`, 수정 `variety-list-client/dialog/page-client/page-wrapper`·
`lib/variety-labels.ts`·`components/ui/alert-dialog.tsx`(공용)·`app/actions/admin.ts`(고아 액션 주석).
빈 상태 문구 해요체, `@ts-ignore` 2개 제거(지워도 `tsc` 통과 = 잔재였다).
`deleteVarieties` 액션은 호출부가 없어졌지만 **남겼다**(주석만). 11파일 +208/−375.
검증 `tsc` 0 · `eslint` 0(변경분) · `test` 316/316 · 브라우저 확인 완료.

### 품종 삭제·개명 제약 보강 — 브라우저 확인 중 나온 4건 `fix` `883cd43` `2ca3ed6`

별칭 화면을 테스트하다 사용자가 던진 **두 질문이 결함 4건을 드러냈다.**

**질문 ①「품종이 삭제되면 어떻게 돼? 제약이 없지?」**

- 🔴 **삭제 가드가 제품유형(SKU)을 안 세고 있었다.** `ProductType.varietyId`는 2026-06-17
  제품유형 마스터가 들인 **세 번째 참조자**인데 `checkVarietyReferences`는 Stock·
  MillingOutputPackage 둘만 셌다(주석도 「두 모델에 의해 참조됨」인 채였다). FK가 `Restrict`라
  데이터는 안 깨졌지만 **가드 통과 → delete에서 터짐 → 「품종 삭제에 실패했어요」만 뜨고
  이유를 알 수 없었다.** 실측 검정보리·칼슘찹쌀 2종이 재고 0 / SKU 1개로 정확히 이 구멍에 있었다.
  📌 **교훈: 참조자가 늘면 가드도 같이 늘어야 한다** — 마스터를 새로 만들 때 기존 삭제 가드를
  훑는 절차가 없었다
- 🔴 **별칭은 품종 행에 얹혀 있어 품종을 지우면 같이 사라진다.** 확인 문구가 「정말
  삭제하시겠습니까?」뿐이라 발주서가 다음 업로드부터 매칭실패로 돌아가는 게 안 보였다 →
  단건·일괄 모두 별칭 나열 경고

**질문 ②「품종명을 수정할 때는 제약이 맞게 되어 있나?」**

- 🔴 **이름이 다른 품종의 별칭을 가로채는 걸 아무도 안 막았다.** 매처 `resolveVariety`는
  **name 정확일치를 먼저** 보고 실패해야 aliases를 본다 → 아무 품종을 「보리」로 바꾸면
  찰보리의 별칭이 **그 순간 죽는다.** `efdbeb7`에서 겪은 그 일을 **개명으로 다시 만들 수 있었다.**
  전날 만든 `validateAlias`는 「별칭 → 다른 품종의 이름·별칭」 **한 방향만** 막고 있었다
  → `validateVarietyName` 신규. 🔴**같은 파일에 둔 건 의도** — 이름과 별칭은 **같은
  네임스페이스를 공유**한다. 한쪽만 고치면 다른 쪽으로 같은 충돌이 들어온다
- **공백만 다른 이름도 막는다** — DB `name unique`는 「서농 22호」≠「서농22호」로 보지만
  **매처는 같다고 본다**(`stripSpaces`). 실측 기존 41종에 그런 쌍 0건이라 회귀 없음
- ⚠️ `updateVariety`는 이름·별칭 검증이 **서로를 본다**(이름은 「자기 별칭과 겹치나」, 별칭은
  「품종명과 같나」) → 순환을 끊으려고 **이번에 저장될 별칭을 먼저 깔고 이름부터 정한다.** 조회 1회
- ⚠️ **로트 품목코드가 품종명 문자열로 정해진다**(`getProductCode`의 `includes`).
  「검정보리」→「블랙보리」면 `215`가 `21`로 **조용히** 바뀐다. 실측 **41종 중 18종이 이름 의존**.
  막을 일은 아니라 저장 전에 「백미 215 → 21」을 보여 주고 확인만 받는다(곡종도 코드를 가르므로
  **이름+곡종 전후 비교**). 이미 만들어진 로트는 문자열 저장이라 그대로.
  📌 「검정보리쌀」로 바꿔도 경고가 **안 뜨는 게 정상** — '검정'·'보리'를 둘 다 품어 코드가 같다

**곁가지**: 곡종 라디오가 **크기 지정이 없어 body 기본 16px 상속** 중이었다(같은 다이얼로그
Label·Input은 14px, 다른 화면은 12.5px). 그래서 유독 커 보였고 7개가 2줄로 넘쳤다 → `text-sm`+간격 축소.

검증: tsc 0건 · eslint 새 경고 0건 · **test 316/316**(이름 검증 8건 신규) ·
실 Neon 재현(삭제 차단 2종 · 이름 거절 3종 · 기존 41종 자기 id 전부 통과 · 품목코드 경고) ·
**브라우저 확인 완료**. 보고서 §5.

### 품종 별칭 관리 화면 — 목록 열 + 칩 편집기 + 검증 `feat` `eed5939`

별칭이 **쌓이기만 하고 볼 수도 고칠 수도 없던** 상태를 끝냈다. 계획서 `plan-품종별칭-관리화면.md` A1~A3.
어제 수동지정을 제거(`c2fc6f3`)하면서 **별칭을 만드는 경로가 이 화면 하나뿐**이 됐다.

- `lib/variety-alias.ts` 신규(검증 순수 lib, 결정 X) — 🔴 `learnVarietyAlias` 삭제 이후
  **별칭을 막는 마지막이자 유일한 곳**이다. 거절 5종: 빈 값 · 품종명과 같음 · 중복 ·
  다른 품종과 충돌(구 결정 S) · 도정 단어 섞임(구 결정 T)
- `stripSpaces` export — 검증이 「매처와 같은 규칙」이어야 한다(결정 X). 따로 구현하면
  **공백만 다른 값이 화면엔 두 칩인데 매처엔 같은 값**이 된다
- 🔴 **별도 액션 대신 `updateVariety`에 합쳤다**(계획서 A2가 열어 둔 판단). 저장 버튼이
  하나인데 액션이 둘이면 **부분 저장**이 생긴다. 결정 V′로 권한이 `SUPPLY_MANAGE` 하나가 돼
  나눌 이유도 사라졌다. `aliases`는 선택 필드 — 미전달이면 기존 값을 안 건드린다.
  서버에서 전수 재검증 + 별칭 before→after 감사로그
- 별칭 열(결정 Y) — **권한 무관하게 모두에게 보인다**. 잘못 학습된 별칭은 목록에서 보여야
  발견된다. 모바일은 품종명 **아래 줄**(옆에 붙이면 이름이 밀려 잘린다)
- 🔴 **읽기 전용 표시는 목록 열이 담당한다** — 결정 V′는 「권한 없으면 별칭 칸을 읽기 전용으로」라고
  했는데, **수정(연필) 버튼 자체가 이미 `canManage`일 때만 뜬다.** 다이얼로그는 권한자만 연다.
  `AliasEditor`에 `readOnly`를 만들지 않았다
- 🔴 **다이얼로그를 열 때 상태를 현재 값으로 리셋**(계획서에 없던 추가). `useState` 초기값은
  최초 마운트에만 먹어서 저장 후 낡은 채 남는다. 별칭은 **배열 통째 저장**이라 그 사이 추가된
  별칭이 조용히 사라진다 — 포장 입력 소실(`d505f42`)과 같은 구조, 「동시 편집」이 아니라 **「낡은 화면」**
- `DialogContent`에 `flex flex-col max-h-[88vh]` — 기본 grid는 내용이 길어지면 푸터가 잘린다

검증: tsc 0건 · eslint(변경분) 새 경고 0건 · **test 308/308**(별칭 11건 신규) ·
**실 Neon 기존 별칭 6종 전부 새 검증 통과**(리스크 §6 해소 — 발아현미`가바발아현미` ·
백옥찰`찹쌀` · 서농22호`가바` · 찰보리`보리` · 천지향1세`천지향` · 흑미`가바흑미`).
브라우저 확인은 `docs/report/report-품종별칭-관리화면-2026-09-16.md` §4 —
🔴 마지막 항목(별칭 추가 → 매트릭스 재매칭)이 **매칭실패를 푸는 유일한 경로**다.

### 발주서 매칭실패 수동지정 제거 — 안내 전용 전환 `refactor` `c2fc6f3`

사용자 판단: 「초기 안정화되면 거의 발생하지 않을 일. 굳이 이 기능을 안 써도 된다.
품종 관리에서만 관리하고 안내만 해 주면 된다.」 **어제 구현한 D2e 수동지정(`c0f9f97`)을 되돌렸다.**

🔴 **D2e 실측이 이미 말해 주고 있었다** — 매칭실패 18건 중 지정으로 풀리는 건 **2건뿐**이었고
(#16 천지향5세 10kg · #17 가바현미 PET) 나머지는 SKU 카탈로그 빈칸이라 어차피 마스터 화면에
가야 했다. **팝오버가 풀어 주는 척만 하고 있었다.**

📌 **사용자 지적 2번의 정체**: 「SKU 없는 매칭실패인데 품종 셀렉트박스가 나오는 건 의미 없다」 —
단순 UI 군더더기가 아니라 **서버가 이미 아는 실패 사유를 사용자에게 다시 물어보는** 구조였다.
`varietyId === null`일 때만 「품종 관리에서 등록」을 띄우고, 품종을 골라야 비로소 「이 품종에
제품유형이 없어요」가 나왔다. 안내 전용은 사유를 곧바로 말한다.

- 삭제: `assignUnmatchedColumn`(+`AssignResult`) · `learnVarietyAlias`(+`AliasSkipReason`) ·
  `sortSkuCandidates` · 팝오버의 품종 셀렉트/SKU 라디오/별칭 체크박스/「지정 저장」/`NoSku` · `onMatch` 체인
- 축소: `getUnmatchedCellOptions` — 품종·SKU 후보 대신 **실패 사유와 열 범위만**(`varietyName` 추가)
- 안내(결정 Z): 사유 3종이 **갈 곳이 다르다**. `variety_unresolved`→품종 관리(별칭) /
  `packaging_unresolved`·`sku_unresolved`→제품유형 관리. 공통 꼬리 「등록한 뒤 재매칭」.
  범위 표시 유지(「3수령인 · 7라인(120개)이 같이 막혀 있어요」) — 마스터를 고칠 값어치를 보인다
- 🔴 **유지**: `rematchUpload`·`loadMatchPatch`·`applyMatchPatches`(매칭실패를 푸는 **유일한 경로**가 됐다) ·
  `hasMillingToken`+테스트 4건(별칭 관리 화면 검증 `lib/variety-alias.ts`가 이어받는다)
- 🔴 **별칭 편집 권한 `OPERATION_MANAGE` → `SUPPLY_MANAGE`**(별칭 계획서 결정 V → V′).
  사용자: 「품종관리 메뉴 권한자가 수정하는 걸로. 별칭을 중구난방으로 관리하면 안 될 것 같아.」
  근거였던 §2.2「학습이 이미 그 권한으로 열려 있다」가 **학습 경로 삭제로 통째로 무너졌고**,
  §2.2가 문제 삼던 「만들 수는 있는데 되돌릴 수 없는 상태」도 **만들 수도 없어져서** 자연 해소
- ⚠️ 감수한 비용: `OPERATION_MANAGE`만 가진 3명(임승원·민채·이영명)은 매칭실패를 **스스로 못 푼다**
...
= 결정 Y 목록 표시)으로 계획서 미결 0건.

## 2026-09-15

### 보리·찰보리 품종 통합 — 찰보리로 통일 `chore` `efdbeb7`

사용자 확인: 둘은 같은 품종. 방향은 **찰보리로 통일**.

🔴 **시드의 「보리」(31)가 재고·제품유형·제품재고 전부 0인 빈 품종인데 매칭을 가로채고 있었다.**
실제 물건은 그 다음 날 손으로 만든 「찰보리」(46)로 들어왔다(감사로그 2026-04-30 08:08 등록 → 08:25 도정위탁 2,900kg).
매처는 **이름 정확일치를 먼저** 보므로 발주서 「유기농 보리」가 빈 품종에 붙고, 거기 SKU가 없어 실패로 남았다.

찰보리 쪽에만 데이터가 있어 이 방향은 **옮길 행이 0개**(반대 방향은 Stock 1 + SKU 2 이동 + 표시명 전면 변경).
매칭 결과는 두 방향이 같아 싼 쪽을 골랐다.

- `scripts/merge-barley-into-chalbori.ts` — dry-run 기본, `--apply`로 반영. ①찰보리 aliases에 '보리' ②빈 보리 삭제 ③감사로그 2건.
  안전장치: 참조 3종이 0이 아니면 중단 · '보리'를 쓰는 다른 품종 있으면 중단 · 백업 JSON(`backups/`, 저장소 밖)
- 검증: 「유기농 보리 1kg」·「유기농 찰보리 1kg」 둘 다 SKU#70으로 ✅ · 4kg→#71 ✅ · 검정보리 영향 없음 ✅
- 🔴 **로트번호 안 바뀐다** — `getProductCode`가 "보리" 포함이면 21, '검정'만 215. 보리·찰보리가 원래 같은 코드였다
- ⚠️ 기존 매칭실패 3라인(#211·#212 묶음15 · #273 묶음16)은 매트릭스에서 **「재매칭」**을 눌러야 붙는다

📌 **곁가지 확인**: 로트번호 품목코드표(사내 엑셀)가 공식 자료인지 조사 — 농관원 세부실시요령·유기가공식품 인증기준·
농수축산물 표준코드·식품안전품목분류 어디에도 같은 체계가 없다. 법령 용어는 「제조단위번호」이고 형식 지정은 확인 안 됨.
문서 자체도 **「부여방법을 기재하여 주십시오」(자율 기재 문항)** · 설명과 예시 자릿수 불일치(4자리라면서 예시는 2자리) ·
빈 행 다수 · 시트명 「-정정」 → **내부자료로 보인다**(단정은 불가, 인증기관 확인 권장).

### 🔴 매트릭스 열 순서가 수동지정 후 뒤로 밀린다 — 라인 조회 `orderBy` 누락 `fix` `3fa98ef`

사용자 브라우저 확인 1번에서 발견 — 천지향5세를 지정했더니 그 열이 **백옥찰 뒤로 밀렸다**(「새로고침해서 그런건가?」 — 맞았다).

🔴 **`getUploadMatrix`의 라인(items) 조회에 `orderBy`가 없었다.** 열 순서는 `buildColumns`가 items 배열을 훑는
**차례**로 정해지고 **id 순 = 엑셀 원본 순**인데, orderBy가 없으면 Postgres가 물리적 순서로 준다.
**UPDATE된 행은 새 튜플로 테이블 끝에 다시 쓰이므로 방금 지정한 라인이 뒤로 간다.**

실측(묶음 #16, Neon): 수정 전 `268,269,267,271,…,270`(지정한 267·270만 이탈) → 수정 후 `267,268,269,270,…`,
그룹 등장 순 `pt38`(천지향5세)이 다시 맨 앞.

📌 **D2a부터 있던 잠복 결함.** 차감(`confirmCell`)은 movement를 **INSERT**할 뿐 라인을 UPDATE하지 않아 안 드러났고,
**PurchaseOrderItem을 UPDATE하는 경로가 D2e가 처음**이었다. 같은 파일의 `getPurchaseOrderDetail`·셀 조회엔 이미 있어 **이 한 곳만** 빠졌다.
⚠️ 순수 함수 테스트로는 안 잡힌다(DB 반환 순서). 주석으로 이유를 박아 뒀다.

### D2e 검증항목 오류 정정 + SKU 0개 품종을 못 고르던 모순 `fix` `dddeb50`

사용자 지적 「혼합곡 SKU가 없지 않나」. 맞았고, 옆에서 하나 더 나왔다.
🔴 **혼합곡은 SKU가 아니라 품종 자체가 마스터에 없다** — 검증 1번이 실행 불가능한 항목이었다.
🔴 **「천지향5세 품종이 없다」고 쓴 것도 틀렸다 — 품종도 SKU 5개도 있다.** 매처가 `varietyId: null`을 준 건
토큰이 `백미 천지향5세`라 **못 찾은 것**인데 **품종 부재로 읽었다.**
📌 **교훈: 매처의 `varietyId: null`은 「품종이 없다」가 아니라 「그 토큰으로 못 찾았다」다.**
품종 41종 중 SKU가 하나라도 있는 건 **22종뿐**.

- 코드 모순 1건 — SKU 0개 품종을 select `disabled`로 막아 **「제품유형 없음 + 등록하러 가기」 안내를 영영 못 봤다**
  (율무·녹두가 정확히 그 경로). disabled 제거 + 품종 마스터에 아예 없는 품목용 `/admin/varieties` 링크 추가
- 검증 대상 교체 → **#16 「유기농 백미 천지향5세 10kg」**(지정 가능 + **결정 T 안내가 뜨는 유일한 케이스**), 보조 #17 「가바현미 1kg PET」
- ⚠️ **별칭 학습 성공 경로는 현 데이터로 테스트 불가** — 품종 해석 실패이면서 품종·SKU가 둘 다 있는 조합이 없다
- 🔴 **결정 T 보류 근거 무너짐** — 「천지향5세 등록 때 재판단」이라 적었는데 이미 있다. 해남급식이 같은 양식을 또 보내면 **매번 수동지정**. 도정 접두 분리 여부 사용자 결정 대기

계획서 §2.1 신설 · §7·§8 정정 · 보고서 §6.1 신설. 테스트 299/299.
...
`isColumnShort`가 **발주 전체**(orderedQty)와 **지금 가용**을 비교해서, 확정으로 가용이 줄면 다 채운 열이 부족으로 보였다.
셀 판정(`cellStatusOf`)은 이미 남은 수량 축이었는데 열만 아니었다. 일반 규격 열도 같은 결함(재고가 빠듯하면 확정 직후 빨개짐).

- **`lib/purchase-order-matrix.ts`** `isColumnShort` — `remaining = max(0, orderedQty − allocatedQty)`로 비교(톤백 kg·일반 개수 둘 다). 테스트 +1 = 41건

### 발주서 톤백 — 🔴 **결정 M 리셋**: 팝오버는 선택만, 쪼개기는 제품재고에서, 허용 폭 +10kg 고정 `feat` `e0a3ec6`
...
- 계획서 결정 K · D5 · 검증 3′(구 3 취소)
- **결정 L `4d95e65`** — 사용자 4차 정정: 셀을 kg 덩어리로 보지 말고 **발주 자루 단위**로. 자루중량 ~ +1%(1,000~1,010) 안 재고 자루는 통째 1자루 만족,
  남는 발주 자루만 kg FIFO + 3. `fitsUnit`·`suggestBulkUnits`(lib, 테스트 +8 = 22). 실데이터 pt18 1,000×1은 이제 1,005 통째(587+332+84 아님)
- 브라우저 1차 확인: 「81 통째 + 369에서 3」으로 뜬 건 어제 테스트가 남긴 81/369 자루 때문(알고리즘 정상). 사용자가 합쳐서 재확인 예정

## 2026-09-14

### 발주서 매트릭스 채널별 2단 표기(C0-c) · 톤백 라벨 정리 `feat` `8940092`
...
### 매트릭스 정렬 4종 → 3종 — 「최신」 삭제 · 「작업필요」 상태 심각도순 `fix` `c5613d3`
...
시안이 하루에 두 번 들어왔다. 1차(매트릭스 6종: 소계 위계 + 택배 발주처 색상)는 **CSS는 유효했지만
가용 재고 값 계산이 C0-a 이전 스냅샷**이었고, 2차(`표기규칙-핸드오프.md`)는 **엑셀 원본**을 근거로 써서
**DB** 실측과 파서가 버린 행만큼 어긋났다 — 낡은 게 아니라 근거 소스가 다른 새 유형.

1차 대조에서 실측으로 지적한 셋(택배 67건 중 64건이 발주처≠수령인 · hue 색상 · 「발주 구청」 라벨)이
2차에 **전부 반영되어 돌아왔다.**

- **C0-b** `matrix-client.tsx` — `H_SUM 26` → `H_SUM_MAIN 40 / H_SUM_SUB 28`. 소계 15px extrabold, 가용 11.5px.
  🔴 `short`(재고부족 앰버)를 `strong`보다 먼저 판정 — 순서 바꾸면 경고가 덮인다. tsc·eslint ✅, **브라우저 미확인**.
- **계획서 C0-c 확정** — 채널별 `굵은 값 ｜ 세로선 ｜ 연한 값` 한 형태, 서울급식만 순서 반대. 택배는 전 행 2단이 기본.
- **계획서 C0-d 신설** — `MatrixRow.label`이 `"발주처 → 수령인"` 문자열이라 「가나다」가 발주처 정렬. 화면은 이미
  `vendor`/`recipient`를 직접 조합해 `label`은 정렬 키로만 남은 죽은 값 → 제거 + 정렬 4종.
- **기각(이유 기록)** — 발주 0인 행 표시(파서 `:362`가 버림·범위 밖) · 지시서 택배 「10곳/66행」(엑셀도 9곳 67행).
- **백로그 §41** — 이마트 시트 하단표 = **박스 적재 계산표**(결정 #35의 실물). `unitsPerBox`로 3행 합계까지 검산 일치.
  파서가 안 읽는 건 수량 0이라 **우연** — 숫자 채워지면 주문으로 잘못 적재.
...
관리자 설정의 수율 기준값이 **저장만 되고 화면엔 거의 안 먹던** 문제. 읽는 쪽이 4갈래로 갈라져
있었고 그중 설정값을 보는 건 포장 입력 다이얼로그 한 곳뿐이었다.

| 사용처 | 고치기 전 |
|---|---|
| 도정목록 PC / 모바일 / 통계 테이블 | `>= 70` **하드코딩** |
| 대시보드 최근 내역 | `DEFAULT_YIELD_RATES` 상수 (DB 아님) |
| 통계 `targetYieldRate` | `yield_rate_target` — **저장하는 코드가 없는 키**, 소비처도 없음 |

- 판정을 `lib/milling-yield.ts`의 `getYieldLevel()` **단일 원천**으로 통합. 배지 색 규칙도 통일
  (화면마다 2단계/3단계, 임계 60·70이 제각각이었다). 경고선 = `기준 - 5`p (사용자 결정).
- 소비처가 전부 클라이언트 컴포넌트라 `(dashboard)/layout.tsx`에서 **1회 조회 → Provider**로 공급.
  `/milling`은 props로 내리면 4단계 drilling이라 Context를 골랐다.
- 죽은 `yield_rate_target` 경로 삭제.

🔴 **계획서에 없던 발견 — revalidate 범위.** `saveYieldRates`가 `/admin/settings`만 revalidate해서,
layout이 기준값을 읽게 된 순간 **저장해도 다른 화면은 낡은 값을 계속 쓰는** 상태였다.
`revalidatePath('/', 'layout')`로 교체. 이 한 줄이 빠지면 작업 목표 자체가 성립하지 않는다.

🔴 **인디카는 도정구분이 아니라 품종 타입이었다(Variety.type).**
"설정 항목에 인디카 추가" 요청이었지만, DB 확인 결과 `millingType`에 '인디카'는 존재하지 않고
(백미 173 · 현미 12 · 칠분도미 2 · 오분도미 3) 인디카 벼도 도정하면 `'백미'`로 저장된다.
`MILLING_TYPES`에 넣었다면 **칸만 생기고 판정엔 안 먹고**, 그 상수를 공유하는 제품유형 마스터의
도정유형 드롭다운까지 오염됐을 것이다. → `getYieldTarget()`에 **품종 축을 도정구분보다 우선**하게 추가.

실적(마감 배치 안분): **인디카 백미 61.3%** vs 메벼 67.2% → 6%p 차이라 별도 기준(61/65).
**찰벼는 67.5%로 메벼와 0.3%p 차이라 넣지 않았다**(사용자 확인). 찰벼 현미 76.6%는 투입 981kg뿐.

실데이터 검증 — 등급이 바뀐 **19건 전부 INDICA**, 메벼·찰벼는 0건(회귀 없음):
```
품종 축 없음:  good 74  warn 69  bad 44
품종 축 적용:  good 82  warn 77  bad 28
```

**14개 파일** (신설 3 / 수정 11). `tsc` 통과 · `npm test` **251/251**(신규 20) · lint 새 이슈 0.
계획서 `docs/plan/plan-수율기준값-전화면반영.md` · 보고서 `docs/report/report-수율기준값-전화면반영-2026-09-10.md`

### 발주서 D2b 브라우저 확인 · D2c 계획 (톤백 열 결함 발견) `fix` `docs`

D2b 매트릭스 화면을 실묶음 2건으로 확인. 레이아웃은 이상 없었고 **결함 1건 수정**:
좌상단 모서리 칸(`HeadCorner`)만 배경이 `bg-slate-200/70` **반투명**이라 스크롤한 본문 행이
헤더 뒤로 비쳤다. 나머지 헤더는 전부 불투명이었다 — 행이 3개뿐인 묶음에선 드러나지 않는 결함.

D2c 계획서 작성(`docs/plan/plan-발주서판매처리-D2c.md`). 착수 실측 중 **D2b 결함을 하나 더 발견**해
C0(선행)로 넣었다.

🔴 **톤백 열이 자루중량을 무시한다.** 묶음 #19(시아스)에 `유기농 가바백미 톤백 1,000kg`과
`200kg`이 각각 1개씩 있는데, `columnKeyOf`가 매칭 라인을 `pt:18` 하나로만 식별해서
**두 라인이 같은 열로 합쳐졌다.** 규격별 소계에 「톤백 2개」라는, 중량이 다른 것을 더한 수가 찍힌다.
바로 아래 `unitWeightOf`는 라인의 `unitWeightKg`를 제대로 쓰는데 **열을 가르는 키에만 안 들어갔다.**

딸린 결함 2개 — 열의 `unitWeightKg`가 **먼저 만난 라인 값으로 굳고**(1000),
총 가용 kg이 `가용개수 × 그 값` = **11,000kg**으로 뜬다. 톤백 재고는 자루마다 중량이 제각각이라
(pt=18 가용 11자루: 203·332·375·450·550·587·788·876·887·1005·1014kg) **실제는 7,067kg**이다.
→ 톤백 열의 가용은 **kg 총량으로 표시**하기로 결정(개수는 의미가 없다). 액션이 `availabilityKg`를
따로 반환해야 한다.

착수 실측에서 잡았던 「셀당 라인 2개 = 2건」도 **전부 이 결함**이었다. 열을 제대로 가르면
현재 데이터의 라인 2개 셀은 0건. 그래도 셀 API는 `itemIds` 배열로 두기로 했다(D2d 톤백에서 필요).

차감 후 화면 갱신은 **`buildMatrix` 재실행**으로 결정 — 피벗이 순수 함수라 서버 재조회(1.4초)
없이 **15ms**로 끝난다. 파생을 손으로 다시 쓰지 않는 것이 요점.


## 2026-09-09

### 605행 소급 정리 — 6월말 이전 벼 재고 일괄차감 `chore`

스크립트 `scripts/bulk-deduct-before-july.ts`(일회성). 🔴 **실서버 Neon DB.**
D2 매트릭스보다 먼저 해야 하는 작업 — 시스템이 「재고가 나간 적 없다」고 믿는 상태로 D2를 켜면
FIFO가 이미 팔린 재고를 집는다.

**사용자 결정**: 2026-06-30까지는 전부 나갔다고 보고 턴다. 7월 이후는 실물 조사로 맞춘다.
사유 = `OTHER` / `기능 오픈 전 일괄차감`. **잡곡은 제외**(사용자가 따로 확인).

| | 결과 |
|---|---|
| 차감 | **467행 · 53,315개 · 564.2톤** |
| 남은 벼 재고 | **165행 · 175.5톤** (+ 잡곡 4행은 손 안 댐) |
| 사후 검증 | 6월말 이전 가용>0 **0행** · 가용 음수 **0행** |

- 백업 `MillingLog/backup-일괄차감-2026-09-09.json`(저장소 밖). 감사로그 1건.
- 시간대 경계는 깨끗했다 — 6월 마지막 행이 KST 6/30 17:39, 다음이 7/5라 **UTC/KST로 갈리는 행 0건.**
- 가용 계산은 `lib/package-available.ts`의 공용 `availableOf`를 그대로 썼다(단일 원천).
  처음엔 스크립트 안에 손으로 짰다가 타입 에러로 드러나 교체 — **결과적으로 앱과 같은 계산이 됐다.**
- `createMany` 한 번. 루프 안 INSERT 금지(Neon 왕복 250~300ms).

🔴 **준비 중 날짜 결함을 발견했다 — 백로그 §39 신설.**
어제 넣은 일괄차감 14건이 이력에 **하루 전날**(`26-09-07`)로 떠 있었다.
**저장은 맞다**(KST 자정을 UTC로 환산 = `09-07T15:00Z`). 되읽을 때
`toISOString().slice(0,10)`이 **UTC로 잘라** 하루 밀린 것이다(`package-movement.ts:408`).
재포장만 멀쩡해 보인 건 우연 — `now()`가 낮 시간이라 그렇고, **KST 00~09시엔 똑같이 밀린다.**
전수로 보니 상차일·발주일·매입일·입고일 등 **7곳**이 같은 패턴이다.
🔴 **정답 패턴은 이미 코드에 있다** — `lib/loading-schedule.ts:119`가 +9시간 후 자른다. 한 곳만 맞았다.

**이번 회피**: `occurredAt`을 **UTC 자정**(`2026-06-30T00:00:00Z`)으로 넣었다.
지금 표시(UTC slice)로도 `06-30`, §39를 고친 뒤(KST 변환)에도 `06-30`이라 양쪽에서 맞는 유일한 값이다.
따라서 §39 수정 시 **이 467건은 손대지 않아도 된다.**

### 제품재고 차감 테스트 데이터 일괄 정리 `chore`

계획서 `docs/plan/plan-차감테스트데이터정리.md` · 스크립트 `scripts/cleanup-test-movements.ts`(일회성).
🔴 **실서버 Neon DB 대상**이라 백업·사전검증·사후검증을 갖춘 스크립트로 처리했다.

| 대상 | 전 | 후 |
|---|---|---|
| `PackageMovement` | 46건 | **0건** |
| `Repack` | 10건 | **0건** |
| 제품재고 | 659행 | **636행**(재포장 결과 23행 제거) |

- 백업 `MillingLog/backup-차감정리-2026-09-09.json`(저장소 밖). 감사로그 1건.
- 발주서 69건은 **그대로 뒀다**(사용자 결정) — 재고 차감 0건이라 제품재고에 영향이 없고 D2 테스트에 쓸 수 있다.
- 「기능 오픈 전 일괄차감」 14건도 지웠다(사용자 결정). **605행 소급 정리는 처음부터 다시 한다.**

🔴 **movement와 재포장 결과 행은 짝으로 지운다** — 한쪽만 지우면 `lib/package-guard.ts`가
막으려던 사고가 그대로 난다(재고 증발 또는 이중 계상). `deleteMany` 3회를 한 트랜잭션에.
루프 안 개별 삭제는 하지 않았다(Neon 왕복 250~300ms).

**연쇄 1건이 있었다** — `Repack #3`의 결과 `pkg#1356`이 `Repack #4`의 소스였다.
movement를 통째로 먼저 지우는 순서라 순서 문제가 저절로 풀렸다.

⚠️ **원물·도정 관계는 안 끊어진다**(사용자 질문). 지운 23행도 `batchId`·`stockId`를
갖고 있지만 그건 **자식이 부모를 가리키는 참조**다 — 자식을 지워도 배치·원물·원래 포장 행은
그대로다. 남은 636행 중 632행이 `batchId`를 유지하고 있음을 실측으로 확인했다.

🔴 **실행 후 「이력이 0건인데 ⋮ 메뉴가 뜬다」** — 데이터가 아니라 **캐시**였다.
스크립트는 `revalidatePath`를 거치지 않으므로(앱 액션에만 붙어 있다) Next가 삭제 전 스냅샷을
계속 그렸고, 그 스냅샷은 `qty > available`이라 메뉴 조건을 만족했다. 다이얼로그는 열 때
새로 조회하니 비어 있었다 — **목록(캐시)과 다이얼로그(실시간)가 어긋나 보인 것.**
새로고침으로 해소. 스크립트 마지막에 안내 문구를 넣었다.
**교훈: DB 직접 수정 스크립트는 화면 캐시를 못 턴다. 의심하기 전에 새로고침부터.**

### 벼 원물재고 모바일 배경 — 백로그 §38 해소 `fix`

「카드 사이가 흰색인데 기본 바탕은 회색이라 자연스럽지 않다」는 제보.
`stock-list-client.tsx`의 조건부 배경 **두 줄을 다 지웠다**(68행·121행).
`grep "md:bg-white"` 전수 2곳(벼·잡곡)이 이제 완전히 같은 문자열이 됐다.

🔴 **git blame이 「의도」를 뒤집었다 — 설계가 아니라 사고였다.**
- `7522644`(모바일 UI 개선)가 `md:`를 붙이면서 조건부를 같이 넣었는데 **`\${...}`로 이스케이프**돼
  문자열이 그대로 박혔다 → **처음부터 죽은 코드.**
- 살아난 건 `1e9a6f6`(**fix: 모바일 P0 버그 일괄 처리 (escape**, pb, 이모지)) —
  「className에 `${}`가 찍히는 것」을 걷어내는 일괄 수정에 **딸려 살아났다.**
- 즉 **작성자는 흰 배경이 적용된 화면을 한 번도 본 적이 없다.** 「좋다고 확인한 적 없는 코드」다.
- **교훈**: 「왜 이렇게 돼 있지?」에 답이 안 나오면 **그 줄이 언제부터 실제로 동작했는지**를 본다.
  일괄 수정에 딸려 살아난 코드는 의도가 없다. `git log -L <행>,<행>:<파일>`이 한 번에 보여준다.

⚠️ **§38 최초 기록의 증상 설명이 틀렸다.** 「필터를 켜야 나타난다」의 이유는 배경이 아니라
`MOBILE VIEW (Search First Strategy)` — 122행이 `filters.length === 0 ? null : ...`이라
**모바일은 필터가 없으면 목록을 아예 안 그린다.** 그러니 간헐적 결함이 아니라
**모바일에 카드가 보이는 모든 경우의 상시 결함**이었다.

**121행도 함께 정리**(사용자 승인) — `filters 없음 ? 'bg-transparent' : 'py-3 gap-3 bg-transparent'`.
양쪽 다 투명인 데다 필터가 없으면 자식이 `null`이라 `py-3 gap-3`이 화면을 못 바꾼다.
**동작 변화 0인 죽은 조건부**였지만 읽는 사람에게 「필터에 따라 배경이 갈린다」고 거짓말을 한다
(이번 조사에서 실제로 한 번 속았다). 잡곡과 같은 고정값으로 맞췄다.

검증: `tsc --noEmit` 통과 · eslint 26 problems는 **변경 전과 동일**(기존 `any`·`@ts-ignore`, 백로그 §29).
전수 재확인: `Object.keys(filters).length`로 **클래스를 가르는** 곳 0곳.

변경: `app/(dashboard)/raw-stocks/stock-list-client.tsx` · `docs/리팩토링-백로그.md`(§38 ✅)

### 벼 포장하기 — 관행 그룹 식별자 분리 · 모바일 실기기 §23 종결 `fix` `docs`

실기기 ④ 확인 중 콘솔에 `Encountered two children with the same key, 관행`이 떴다.
**레이아웃 문제가 아니라 식별자 문제였다.**

- **원인**: `LotGroup.lotNo`가 **표시값과 식별자를 겸하고 있었다.** 그룹핑은 `관행-농가번호`로
  제대로 나뉘는데, 담을 때 관행이면 표시용 `'관행'`으로 덮어썼다 → 관행 농가가 둘이면 값이 겹친다.
- **증상 1** React key 중복(콘솔 에러).
- **증상 2** 🔴 이쪽이 진짜다 — `customWeights[group.lotNo]` · `customInputs[group.lotNo]`가
  같은 키를 보므로 **한 농가에서 「기타」(직접 입력)를 열면 다른 관행 농가 칸도 같이 열리고
  값까지 공유됐다.** 콘솔 에러는 증상이고 이게 결함이었다.
- **처방**: `LotGroup.groupKey` 신설 — 그룹핑에 쓰던 고유키를 그대로 담는다.
  React key와 상태 인덱스 9곳을 `groupKey`로 교체, 표시용 `lotNo`는 그대로 둔다.
- 데이터는 안 섞였다 — `getGroupOutputs`는 `stockIds` 기반이라 무사했다.
- 🔴 **교훈: 화면에 뿌리는 값을 식별자로 겸용하지 말 것.** 표시값은 사람이 읽기 좋게
  뭉개지므로(관행 → 전부 `관행`) 유일성이 깨진다.

**모바일 실기기 §23 종결** — ①~⑥ 전부 확인, 미확인 항목 0.

- ④ 레이아웃은 이상 없었다. 제일 빠듯하다던 ④-3(수량 88px·중량 58px, 계산상 **여유 0**)을
  톤백 876kg·375kg + 잔량 2kg + 4kg 2,572개가 든 건으로 봤는데 **안 밀린다.** 계산이 실제와 맞았다.
- ⑤ 공통(하단 버튼·배경 토큰)은 ①~④ 도는 동안 함께 봤고 따로 걸린 것 없음.
- **남긴 것**(결함 아님, 조건이 안 맞아 못 봄): 생산자명 12자 초과(작목반명) 건 ·
  재포장 결과 3종 초과 시 「외 N종」 · 화면마다 다른 로트 표시 규칙(별건).

변경: `app/(dashboard)/milling/add-packaging-dialog.tsx` ·
`docs/모바일-실기기-체크리스트.md` · `docs/리팩토링-백로그.md`(§23 ✅ 종결)


### 차감 이력 — 실기기 피드백 3건 (③-1 · ③-2 · ③-6) `feat` `fix`

계획서 `docs/plan/plan-차감이력-실기기피드백.md` · 실기기 확인 §23 ③에서 나온 제보.
③-3(헤더 strip)·③-4(로트 전체)·③-5(잔여 앰버)는 **이상 없음**으로 확인됐다.

**1. 🔴 ③-6 — 안내 문구 자리를 「재포장 결과」로 바꿨다** (사용자 제안)
- 예전엔 재포장 행마다 「재포장으로 나간 차감은 취소할 수 없어요…」 두 줄이 **늘** 붙었다.
  한 번 알면 자리만 차지하고, 정작 **「그래서 뭐가 됐나」** 는 화면 어디에도 없어
  재포장 화면을 따로 열어야 했다.
- 지금은 그 자리에 `→ 10kg × 4 · 잔량 3kg × 1`. 안내 문구는 **자물쇠를 눌러야** 뜬다
  (자물쇠가 `span` → `button`이 됐다. 발주서 차감 문구도 같은 규칙).
- **데이터는 이미 있었다** — `PackageMovement.repackId → Repack.results`. 스키마 변경 없음.
- 🔴 **병합 재포장**(소스 2행 이상)이면 `results`는 「이 행이 만든 것」이 아니라
  **작업 전체의 결과**라 `합쳐서 →` 로 갈라 적는다(`Repack._count.sources > 1`).
- 같은 규격이 여러 줄이면 **합쳐서 센다** — 사람은 「10kg 4개」로 읽지 「10kg 3개 + 10kg 1개」로 읽지 않는다.
- 왕복 **2회 고정**(`repackIds`를 모아 `in` 한 번). 재포장 행이 없으면 두 번째 쿼리를 건너뛴다.
  (루프 안 왕복 금지 — 발주서 적재 타임아웃에서 얻은 교훈.)

**2. ③-2 — 모바일에선 사유·거래처를 자르지 않는다**
- 제보: 「생략되는 부분은 볼 수가 없나?」 → **없었다.** 잘린 뒤가 `title` 속성에만 있었는데
  **title은 마우스 호버용이라 폰에선 열 수가 없다.**
- ⚠️ 계획은 `line-clamp-2` + 탭 펼침이었는데 **구현 직전에 바꿨다.** 「넘칠 때만 펼침 힌트」를
  하려면 `scrollHeight > clientHeight` DOM 측정이 필요하고 그 결과를 state에 넣으면
  `react-hooks/set-state-in-effect`에 걸린다. **①에서 이미 「자르느니 줄을 늘린다」로 답을 냈다** —
  같은 화면에서 자르기 규칙이 둘일 이유가 없다.
- 데스크탑은 컬럼 정렬이 우선이라 한 줄 `truncate` 유지 + `title` 추가(호버로 전체 확인).

**3. ③-1 — 되돌리기 버튼을 모바일에서 아이콘만으로** (90px → 32px)
- 막힌 행의 자물쇠(`h-8 w-8`)와 폭이 같아져 **행 끝이 가지런해진다.** 데스크탑은 라벨 유지.
- 누르면 확인 다이얼로그가 뜨므로(`confirmDialog`) 오탭 위험은 낮다. `aria-label`·`title` 부여.

**검증**: `tsc` · `eslint`(변경 2파일 무경고) · `npm test` 200/200. 화면은 사용자 실기기 확인 대기.

### 커밋 분할 + 체크리스트 ③④⑤ 정적 점검 `chore` `docs`
**커밋:** `1351e87` 외 6개 — origin/main 푸시(`a271377..1351e87`)

09-08 작업분을 6덩이로 쪼개 커밋했다. 상세는 09-08 항목 참조.
🔴 **커밋 순서를 뒤집어야 했다** — `shortLot`을 차감에서 지우고 이력의 참조를 없앴으므로
**이력 → 차감** 순이 아니면 중간 커밋이 타입 에러로 깨진다.

체크리스트 ③④⑤를 계산으로 먼저 훑었다(`1351e87`). **넘치는 곳은 없다.** 대신 볼 자리를 좁혔다:
🔴 **④-3의 수량 88px·중량 58px 두 칸은 여유가 0** — 계산과 1px만 어긋나면 밀린다.
④-1은 생산자명 12자 초과 시 넘침(`shrink-0`인데 `truncate` 없음). ⑤-1·⑤-3은 구조 확인으로 종결.

## 2026-09-08

### 제품재고 다이얼로그 3종 재구성 + 모바일 실기기 ①② 종결 `feat` `fix`

**커밋:** `83fb311`(정렬) · `55290d7`(플로팅바) · `f38c24e`(이력) · `60b14f9`(차감) · `fa19842`(재포장)
작업은 09-08, 커밋 분할은 09-09.

시안 3건(Claude Design) · 실기기 제보 다수.

**1. 목록 정렬 — 그룹 안쪽이 사용자 정렬을 무시했다** (`app/actions/packages.ts`)
- 오래된순을 골라도 카드 안은 **규격(weightPerUnit) 우선 고정**이라 날짜가 뒤죽박죽이었다.
  (하이아미 카드에서 07-22가 02-23들 사이에 낀 채로 보였다)
- 그룹 내부 정렬을 `sort` 옵션에 맞췄다 — 날짜 1순위, 같은 날짜 안에서만 규격 asc.
  `weight_desc`일 땐 그룹 안도 행 무게 내림차순.

**2. 모바일 플로팅바가 하단 탭바와 겹쳤다** (`package-list-client.tsx`)
- `bottom-16`(64px)이 탭바 상단(nav 60 + mb-4 16 + safe-area = 76px+)을 파고들었다.
  `env(safe-area-inset-bottom)`도 `mb-4`도 빼먹은 값. → `bottom-[calc(5.25rem+env(...))]`
- **같은 뿌리 3곳(통계 필터 시트)은 백로그 §36으로 분리** — 오프셋이 파일마다 제각각이고
  탭바 높이가 `mobile-nav.tsx` 안에만 있어 CSS 변수로 단일화가 필요하다.

**3. 차감 다이얼로그** (`deduct-dialog.tsx`)
- **로트 전체 표시** — 앞말줄임(`shortLot`) 폐기. 폭은 자르지 말고 **정보 재배치**로 냈다:
  포장일을 윗줄 생산자 뒤로 올리고 보조줄을 로트에 통째로 내줬다.
  (윗줄 오른쪽 끝에 붙였다가 **입력칸 유무로 위치가 흔들려** 생산자 뒤로 다시 옮김)
- **「제외」 배지 제거** — 행이 이미 `opacity-55`로 흐려져 신호가 겹쳤고, 배지가 생겼다
  사라지며 입력칸 옆 폭이 들쭉날쭉했다. 행 수는 하단 요약이 센다.
- **확인 단계 A안(푸터 승격)** — 같은 요약이 세 번 나오던 걸 한 번으로. 푸터 자체가 확인 패널이
  되고 죽은 `차감하기`(disabled)·`취소`가 사라진다. 총계는 버튼 안으로
  (`확인하고 차감 20개`). `<br>` 제거, 버튼 `h-8` → `h-11 sm:h-8`.

**4. 차감 이력 다이얼로그 재구성** (`movement-history-dialog.tsx`)
- 헤더 한 줄 → **3층**(품종·규격 / 로트 전체 / 숫자 strip). 「670개 중 670…」로 잘리던 게 해소.
- **잔여 신규** — `qty − deducted`를 사람이 머리로 계산하고 있었다. 0이면 앰버.
  (`lib/package-available.ts`의 available과 같은 공식이나, row가 스냅샷이라 화면에서 재계산)
- 행: **날짜를 맨 앞으로**, `py-2.5`, `border-b last:border-b-0`. 데스크탑 고정폭 컬럼.
- 막힌 건: 박스 제거 → **자물쇠 버튼 + 한 줄 문구**(행 높이가 일정해진다).
- ⚠️ **날짜는 YY-MM-DD로 연도 유지**(사용자 결정) → 컬럼 42→58px, 문구 들여쓰기 204→220px.

**5. 재포장 — 백로그 §37 해소** (`repack-dialog.tsx` · `repack-result-row.tsx`)
- 사용자 제보 「일부만 재포장하는 기능이 없네」 → **기능은 있었고 화면이 침묵했다.**
- 푸터에 「남는 N kg ○○으로 남기기」 상시 제안. 손실 경고는 **푸터를 대체**(버튼 중복 제거).
- 🔴 규격 판정 **두 번 고침** — 소스 규격을 무조건 물려받아 「20kg인데 630kg인 자루」가 나왔다.
- warn 톤 회색→앰버. **쓸 재고 기본 펼침(결정 #44 뒤집음).**

**문서**: 백로그 §36 신규 · §37 신규→해소 · 모바일 체크리스트 ①② 종결.
**시안**: `docs/handoff/` 4건 (차감이력·재고차감확인·재포장분할·standalone). 전부 20KB 안팎이라
걱정하던 **오프라인 번들(4.1MB)은 없다** — 그대로 커밋했다.

**다음**: 체크리스트 ③④⑤(이력·벼 포장하기·공통) 실기기 확인 → 발주서 D2.

> ⚠️ **커밋 순서를 뒤집었다** — `shortLot`을 차감 쪽에서 지우고, 그걸 쓰던 이력 쪽 참조를
> 함께 없앴다. 차감을 먼저 커밋하면 중간 커밋이 타입 에러로 깨진다. **이력 → 차감** 순.

### 제품재고 모바일 3건 — 체크박스 클릭 차단 · 잡곡 툴바 넘침 · 「N종」 표기 `fix`

보고서 `report/report-제품재고-모바일3건-20260908.md` · 정렬 확인 중 **사용자가 연달아 발견**

🔴 **① 모바일 체크박스가 아예 눌리지 않았다** — 재포장·차감 모드에서 선택 자체가 불가능.
hit-area 44px를 만들려고 둔 투명 오버레이(`absolute -inset-2.5`)가 **체크박스를 덮고 있었다.**
**positioned 요소는 static 형제 위에 그려진다** → 터치가 오버레이에서 죽고 input에 안 닿았다.
→ **`<label>` 패딩**으로 교체(음수 마진으로 자리 상쇄). 클릭이 네이티브로 전달돼 터치 영역과 동작을 함께 얻는다.

🔴 **판별 기준 — 오버레이가 「핸들러 가진 조상 안」이면 정상, 「형제」면 클릭을 삼킨다.**
전수 4곳 중 **2곳이 결함**(제품재고 행·원물재고 선택), ⋮ 메뉴 2곳은 `<Button>` 안쪽이라 정상.

⚠️ **원물재고는 왜 신고가 없었나** — **카드 본문 클릭으로도 선택**돼서 티가 안 났다(사용자 증언).
**대체 경로가 결함을 가리고 있었다.**

**② 잡곡 툴바 넘침** — `justify-end`는 넘치면 **왼쪽 항목을 컨테이너 밖으로 밀어내고 스크롤도 안 된다**
→ 엑셀 버튼에 손이 안 닿았다. 잡곡만 버튼 6개. 실계산 439px vs 가용 382px.
`min-w-[96px]`가 두 버튼에서 34px를 버리고 있었다. → 엑셀 모바일 숨김 + `sm:min-w-[96px]` = **365px**(라벨 유지).
**작업 모드에선 활성 토글이 `차감 중 ✕`로 길어져 또 넘쳤다** → 모드 중엔 나머지를 모바일에서 숨긴다
(어차피 전부 disabled). 래퍼는 `contents`로 레이아웃에 안 끼어들게. `flex-wrap`은 안전판.

**③ 「N종」이 행 수를 세고 있었다** — `rows.length`는 규격 종류가 아니다.
**작업 단위로 행을 나누는 게 원칙**이라 재포장할 때마다 갈린다 → `specCount`(spec의 Set 크기)로.
찰보리 실사례: `1kg×90`+`1kg×8` = 「2종」으로 표시됐으나 규격은 1종.
DB 확인 결과 **재포장 2회**(08:40 `1kg×98`→`1kg×90+4kg×2`, 08:41 `4kg×2`→`1kg×8`)로 **데이터는 정확**.
🔴 **사용자 결정: 행은 작업 단위로 계속 나눈다**(합치면 차감 대상 선택·이력 추적이 흐려진다). 표기만 수정.

**검증**: tsc 통과 · eslint는 기존 위반만(변경 구간과 안 겹침) · **사용자 실기기 확인 완료**.


### 제품재고 기본 정렬을 「오래된순」으로 `feat`

보고서 `report/report-제품재고-기본정렬-20260908.md` · 계획서 `plan/plan-제품재고-기본정렬-오래된순.md`

**오래 묵은 재고부터 소진해야 한다**(선입선출)는 요청. 기본 정렬 `weight_desc` → **`oldest`**,
벼·잡곡 두 탭 모두. 정렬 선택지도 **기본값이 맨 위**로.

**정렬 로직은 무변경** — 이미 맞게 돼 있었다. 그룹 대표일을 `oldest`일 때 **min**으로 잡아
「가장 오래된 줄이 그룹 위치를 정한다」(`repDate`). 기준 날짜는 매입=입고일 / 도정산=포장 등록일.

🔴 **기본값 리터럴이 4파일 8곳에 흩어져 있었다** — 하나라도 어긋나면
「기본값인데 URL에 `sort=`가 붙는다」「기본값인데 배지가 뜬다」로 **조용히** 틀어진다.
→ **`lib/package-sort.ts` 신설**(`DEFAULT_PACKAGE_SORT`·`PACKAGE_SORT_OPTIONS`·
`PACKAGE_SORT_VALUES`·`isDefaultPackageSort()`). `package-where.ts`·`production-year.ts`와 같은 단일 원천 패턴.

남은 `'weight_desc'`는 전부 정당 — 타입 정의와 **「무게순 정렬」 동작 분기**, 원물재고는 별개 기능.

⚠️ **배지가 반대로 뜬다** — 이제 「재고량 많은순」을 골라야 배지가 뜬다. 의도된 동작.
기존 `?sort=oldest` 링크는 그대로 열린다.

**검증**: tsc·eslint 통과 + 사용자 브라우저 확인(벼·잡곡 두 탭).


### 검색·필터 다이얼로그 가로 넘침 수정 — 실기기 §23 첫 실물 결함 `fix`

보고서 `report/report-검색다이얼로그-넘침-20260908.md` · 근거: 백로그 §23 · 체크리스트 §⑥ 신설

**체크리스트에 없던 화면에서 걸렸다.** 390px에서 제품재고 검색 다이얼로그의 오른쪽 컬럼
(정렬·인증구분·종료일)이 통째로 화면 밖으로 밀렸다. 페이지는 정상이고 **내부만 11px 넘침**
(`client 356 / scroll 367`).

**원인** — grid 칸(`div.space-y-2`)에 `min-w-0`이 없어 `min-width:auto`가 걸렸고,
안의 드롭다운이 `whitespace-nowrap`이라 **「재고량 많은순」 글자폭 163px이 칸의 하한**이 됐다.
`146×2`로 줄어야 할 컬럼이 `163×2`로 굳으면서 308px 자리에 343px가 들어갔다.

- `components/ui/select.tsx` — `SelectTrigger` `w-fit` → **`w-full min-w-0`**
  shadcn 기본값은 툴바용 전제인데 이 프로젝트는 전부 폼 필드다. `w-full` 없는 **15곳 전수 확인** 후 변경
- grid 칸 `min-w-0` 25곳 — `package-search-dialog`(6) · `stock-filters`(6) ·
  `misc-stock-filters`(6) · `milling-filters`(7)
- `docs/모바일-실기기-체크리스트.md` — **§⑥ 검색·필터 다이얼로그 신설**(⑥-1~⑥-6)

🔴 **교훈 1 — 눈대중으로 두 번 헛짚었다. 실측 한 번이면 끝났다.**
`clientWidth`/`scrollWidth` + 자식 트리 `w=`를 찍자마자 범인이 한 줄로 나왔다.

🔴 **교훈 2 — 콤마 든 Tailwind 임의값(`grid-cols-[minmax(0,1fr)]`)은 CSS가 생성되지 않는다.**
클래스가 DOM에 붙어 있어도 적용된 게 아니다. 기본 유틸리티로 풀 수 있으면 임의값을 쓰지 말 것.

**2차 — 전수로 넓힘**: `grid-cols` 쓰는 파일 전수 대조에서 **동일 결함 1곳 추가**
(`admin/farmers/farmer-filters.tsx`, MultiSelect 2개). 1차 grep은 **건수로 정렬해
「한 곳뿐인 파일」을 놓쳤다.** 🔴 **교훈 3 — 결함은 빈도가 아니라 패턴으로 찾는다.**

예방 4곳 선제 적용(원물 입고·수정 · 잡곡 입고 · 농가 추가) — `edit-stock-dialog`·
`add-farmer-dialog`는 옵션이 **동적**(품종명·작목반명)이라 길어지면 터진다.
`min-w-0`은 칸이 부풀 때만 작동해 **현재 화면은 안 바뀐다.**

⚠️ **함께 나왔으나 결함 아님** — 검색·필터 다이얼로그 **12곳 전부 `max-h` 없음**.
길어지면 위아래가 잘리고 스크롤도 안 된다. 700px 안팎 경계선이었으나 **실기기에서
도정 필터 하단 버튼 정상 → 현시점 결함 아님.** 구조 변경 비용이 커서 보류, 체크리스트에 기록.

**실기기 확인 완료**: ⑥-1~⑥-6 · ⑤-2(키보드) · 도정 필터 세로. **①~⑤ 본체는 여전히 미확인.**

①~⑤는 여전히 미확인 — 이번 건은 §23의 일부다.


### 목록 표준규격 §32 적용 — 표준이 안 닿은 목록 5곳 `style`

커밋 `9ca002d`(S1) · `2ef5a39`(S2) · 보고서 `report/report-목록표준-백로그32-20260908.md`
계획서 `plan/plan-목록표준-백로그32.md` · 근거: 백로그 §32 · 기준서 §4.2

**§32가 적어 둔 3곳이 전부가 아니었다.** 「목록처럼 보이는 것」 전수(생 `<th>` · CSS Grid ·
`divide-y` · `hidden md:flex` · 토큰 오용)로 훑으니 **2곳이 더 나왔다** —
`sales/upload-dialog.tsx:231`은 `uppercase`를 안 써서 §32의 grep에 안 걸렸고,
`packages/deduct-dialog.tsx:294`는 목록 헤더인데 §32가 **「섹션 라벨」로 오분류**하고 있었다.

**S1 `9ca002d`** — 관리자 목록 2곳 `ui/table` 교체 (`NoticeTable`·`UserTable`)
- 프리미티브가 이미 표준값이라 **개별 클래스를 걷어내는 것만으로** 표준이 됐다
- `NoticeTable` **세로 구분선 `border-l` 5곳 제거**(R3의 감사 로그 원칙)
- 2줄 셀만 `py-2` 남겨 높이 자연 증가 · 보조값 폰트 R6 규약 2단(12.5/11.5px)으로

**S2 `2ef5a39`** — 헤더 토큰 3곳, **행은 무변경**
- `recent-logs-list:49` · `upload-dialog:231` · `deduct-dialog:294`(`bg-white`→`bg-slate-50`도)
- 행을 안 건드린 이유: 대시보드 위젯은 **카드 하나에 10건**이 목적이고, 나머지 둘은
  체크박스·select·date가 든 **편집형 행**이라 44px 강제 시 컨트롤이 연쇄로 끌려온다
- `recent-logs-list`는 `rounded-lg` 유지 — 부모가 `rounded-[24px]`+패딩 카드라 pill이 맞다

**결과**: `grep "uppercase tracking-wider"`에 **목록 헤더 0건**. 남은 12곳은 전부 섹션 라벨 = §33.

🔴 **부수 발견 — 헤더 호버가 의도와 반대다**(§35). 헤더 `TableRow` 8곳이 전부
`hover:bg-transparent`인데, 프리미티브의 `hover:bg-slate-50`을 덮어 **마우스를 올리면
회색 배경이 사라진다**. 지시서 R1은 `hover:bg-slate-50`(배경 유지)이라 적었고 기준서도
「호버에 반응하지 않는다」다 — 구현이 의도를 뒤집었다. 이번엔 **새 2곳도 기존 관례에 맞췄다**
(새 화면만 다르면 더 나쁘다). 고칠 거면 10곳 일괄.

문서: 백로그 §32 ✅ + **§33**(섹션 라벨 12곳) · **§34**(9px 컬럼 힌트) · **§35**(헤더 호버) 신설
기준서 §4.2에 🔴**「적용 범위를 잡는 법 — 목록처럼 보이는 것 전수」** 명문화 —
훑을 대상 **5종 검색 명령을 표로** 박았다. **판정 기준은 마크업이 아니라 화면**이고,
**편집형 행·위젯은 헤더만** 올린다는 예외도 함께.

교훈: §32는 「`ui/table` 범위로 잡으면 계속 샌다」고 써 놓고 **자기 대상은 grep 한 줄로 잡았다.**
다음엔 grep 한 줄로 범위를 잡지 않는다.

검증: `tsc` 통과 · `eslint` **신규 0건**(HEAD와 동일한 12 errors/2 warnings = 기존 `any`·`<img>`)
로직 변경 **0줄**(순수 마크업) · 사용자 브라우저 확인 통과

## 2026-09-04

### 제품재고 목록 표준규격 적용 — 백로그 §30 해결 `style`

커밋 (아래) · 보고서 `docs/report/report-제품재고-표준적용-2026-09-04.md` · 계획서 `plan-제품재고-표준적용.md`
근거: 지시서 `list-spec-instructions.md` **R6**(디자이너 확정 A안)

표 10곳만 14px로 올린 탓에 **가장 많이 쓰는 목록 하나만 작아져 격차가 벌어져 있던 것**을 되돌렸다.
코드는 `packages/package-row.tsx` **한 파일**(31+/15−) — `PKG_GRID`가 여기에만 있어 예상보다 좁았다.

- 헤더 `text-[10.5px] uppercase tracking-wider bold px-4 py-2.5` → `h-10 items-center px-3 text-sm font-medium text-foreground bg-slate-50`
  (`uppercase`·`tracking-wider`는 한글에 효과 없는 **영문 라벨 토큰 오용** — 버그 수정 성격)
- 행 3곳 → `text-sm px-3 h-11` · 펼친 묶음 → `bg-slate-100 border-t border-slate-200/80`(ring 제거)
- 보조값 폰트 **4단(`12/11.5/11/10px`) → 2단**(`[12.5px]` 수치 · `[11.5px]` 칩·배지)
- **밀도 우려는 계산으로 기각**: `text-[13px] px-4 py-3` = 13×1.5+24 ≈ **43.5px**라 `h-11`로 +0.5px뿐

🔴 **실화면에서 표준을 하나 꺾었다** — §4.2.6 「묶음 내부 가로선 제거」를 적용하니 서브행 9줄짜리 그룹이
**구분 안 되는 회색 덩어리**가 됐다. 그 원칙은 서브행 2~3줄인 원물재고를 전제로 한 것이었다.
`border-t border-slate-200/70`으로 되살리고 **기준서 §4.2.2·§4.2.6에 예외로 명문화**(안 그러면 또 지워진다).
서브행 호버(`hover:bg-slate-200/70`)는 원래 없어서 함께 넣었다. 그룹 헤더 호버도 펼침 시 분기 —
**묶음톤보다 밝은 배경을 얹으면 얼룩**이 된다(어제 벼 하이라이트와 같은 유형).

추가 요청 반영: **그룹 헤더 생산자 `—` → 「N명」**(원물재고 `farmerSetSize`와 같은 표기).
`item.rows`로 클라이언트 계산 — 서버 무변경. 도정구분·로트는 `—` 유지.

기준서: §4.2 도입부 적용완료 표시 + **모바일 카드 = 의도된 예외** 명시(카드와 표는 다른 밀도 문맥)
\+ **「왜 제품재고만 grid인가」** 기록 — 설계 판단이 아니라 **시점 차이**다(벼 04-29·잡곡 04-30은 문서 없이
`<table>`로 먼저, 제품재고 05-04 `ebd7886`만 §4.2를 받아 grid). 「table로 어렵다」는 사실이 아니지만,
**그룹 헤더를 `<button>`으로 감쌀 수 있다**는 이점은 실재한다.

검증: `tsc` 0 · `eslint` 0 · 사용자 브라우저 확인(회색 덩어리 지적 → 수정 후 재확인 통과)
남은 것: 접힌 그룹이 아직 흰 배경(표준은 금지, R6 표에 없어 보류) · 모바일 실기기

**🔴 이어서 표준 자체를 뒤집었다** — 위 「제품재고만 예외」를 보고하자 원물재고·생산자관리도
같은 문제라는 판단이 나왔다. **예외가 아니라 원칙이 틀렸다.** §4.2.2·§4.2.6의 「내부 경계 `border-b-0`으로
지운다」를 폐기하고 **`border-b border-slate-200/70` 유지**로 개정, 코드 5곳 되돌림(+벼는 선 색만 `/70`).
**묶음은 배경색이, 행 구분은 선이 한다** — 하나로 둘을 하려다 둘 다 잃었다. 배경톤은 그대로라 일체감은 유지.
부수 발견: **벼는 원래부터 선이 남아 있었다**(R3 「세 화면 통일」 기록과 달리 `border-b-0` 미적용).
의도한 모습이 이미 벼에 있었고, 이번에 네 화면이 처음으로 실제로 일치했다.
디자이너에는 통보 문안(`통보-서브행구분선-표준변경-2026-09-04.md`) — 확인 요청이 아니라 결정 통보.


### 목록 표준규격 R5 — 기준서 개정 (문서-코드 격차 닫기) `docs`

커밋 (아래) · 보고서 `docs/report/report-목록표준규격-R5-2026-09-04.md`

R1~R4b로 코드는 표준값이 됐는데 **기준서 `handoff.md` §4.2는 옛 값 그대로**여서,
다음에 이 문서를 보고 만드는 화면은 다시 10.5px 헤더로 돌아가는 상태였다.

지시서가 **두 번** 「이미 개정 완료」라 했으나 저장소엔 두 번 다 없었다 → **직접 작성.**

🔴 **시안 값이 아니라 실제 코드 값으로 썼다.** 어제 사용자 확인으로 시안에서 벗어난 게 3건이라
(묶음톤 `slate-50/75`→**`slate-100`**, 헤더색 `slate-900`→**`text-foreground` 유지**,
**서브행 반복 컬럼 비우기** 신설) 시안대로 썼으면 오늘 만든 기준서가 처음부터 코드와 어긋난다.
왜 그렇게 정해졌는지도 문서 안에 각주로 남겼다 — 다시 `slate-50/75`로 되돌아가지 않게.

- `디자인시스템/handoff.md` §4.2 전면 개정(도입부·표준값 요약표 신설, 4.2.2~4.2.7)
  \+ §1.1 각주 + 부록 체크리스트(`#00a2e8`은 그룹 표현에서만 제거, 의미색 유지)
- `디자인시스템/design-system.html` §8 Group Table 라이브 렌더링 갱신
- ⚠️ `잡곡재고관리/` 사본은 **`.gitignore:61`로 저장소에 없었다** — 지시서가 「2벌」이라 했지만 저장소는 이미 1벌.
  로컬 사본에 넣은 안내 1줄은 커밋되지 않는다
- 지시서에 없던 후속: 문서를 인용하던 **코드 주석 3건**의 옛 값 정정 (동작 무변경)

🔴 **§30(제품재고)의 「판단 필요」에 지시서가 이미 답을 주고 있었다** — §4.2는 `<table>`이 아니라
**CSS Grid 문서 = 제품재고의 근거 문서**인데, 지시서 R5가 그 §4.2의 타이포 값을 콕 집어 지정했다.
즉 「grid 마크업 유지」는 **구조**를 뜻하고 타이포·밀도는 표준으로 올리라는 것이 문면이다.
대상도 특정됐다: `package-row.tsx:95`(옛 §4.2.3) · `:332`(옛 §4.2.6). 백로그 §30 갱신.

검증: `tsc` 0 · `design-system.html` JSX 8블록 esbuild 파싱 통과 · `<Section>` 20/20

## 2026-09-03

### 목록 표준규격 통일 — R1~R4b 완료, R5 문서만 남음 `style`

커밋 `59bdb7e` · `2ef056e` · `da74cc6` · `d297ae4` · `8e2d582` (로컬)
보고서 `docs/report/report-목록표준규격-2026-09-03.md` · 계획서 `docs/plan/plan-목록표준규격.md`

시안이 하루 사이 **두 번 개정**됐고 그때마다 전수 대조를 다시 했다.
**v1 → v2에서 타이포가 정반대로 뒤집혔다**(13px/700/slate-500 → 14px/500/slate-900).
그 결과 v1 계획서의 핵심이던 「색상 토큰 치환」결정이 무의미해졌고, 오히려
v2의 `text-slate-900`이 현행 기본값 `text-foreground`와 **같은 값**이라
`TableHead`의 실변경은 **`px-2` → `px-3` 하나뿐**이었다.

**블로커 4건 전부 시안 쪽 오류로 확인**(사용자가 인정하고 지시서를 고침):
A) R5의 「이미 개정 완료」가 저장소에선 거짓 — `handoff.md:346`은 옛 값이고 git 이력상
2026-06-09 폴더 이동이 마지막이었다. 디자이너 워크스페이스 사본만 고쳐진 것.
🔴 **이게 재개정본에서도 되풀이됐다** — 「개정본이 이 패키지에 들어 있다」도 사실이 아니었다
(`list-standard/`에 `handoff/` 없음, 저장소 검색 0건). R5는 직접 작성해야 한다.
B) 체크리스트가 `#00a2e8` grep 0건을 요구하나 실제 31건, 그중 29건은 인증칩·연락처 등 의미색 → 축소
C) 「그룹 헤더 44px」인데 정본이라는 잡곡도 `h-12`(48px) → 44px 우선, 잡곡도 수정
D) R4가 `*-table-row.tsx` 3개(TableCell 69개)를 누락 → 7 → **10파일**

**colgroup을 넣는 순간 드러난 잠복 결함 3건**(시안·계획서로는 안 잡혔다):
1. 생산자관리 `canManage=false`일 때 **헤더 9칸 / 본문 10칸 / 그룹헤더 11칸**으로 어긋나 있었다.
   auto 레이아웃이 가려주던 것.
2. 생산자관리에서 **작목반 없는 농가가 2명 이상이면 데스크탑에서 아예 안 보였다** —
   헤더는 `group.group`을 요구하는데 행은 `isExpanded`를 기다려 펼칠 손잡이가 없었다 → `hasHeader` 도입.
3. 벼 선택 하이라이트가 묶음톤에 묻혔다 — 그라데이션 오른쪽 끝이 **불투명 흰색**이라
   `bg-slate-100` 위에서 밝은 얼룩으로 보였다 → `bg-primary/20` 단색.

**사용자 확인으로 조정**: 시안의 묶음톤 `bg-slate-50/75`는 흰색과 **2% 차이**라 실제로 구분이 안 됐다
→ `bg-slate-100`(5.5%). 그리고 사용자 제안으로 **펼친 하위 행에서 반복 컬럼을 비웠다**
(그룹 키가 `group.id`라 년도·작목반번호·작목반·인증번호가 동일) — **색보다 이쪽이 훨씬 효과적**이었다.
잡곡의 `inExpandedGroup` 패턴을 벼·생산자관리로 확대한 것.

🔴 **가장 큰 미결 = 제품재고**(백로그 §30). `<table>`이 아니라 **CSS Grid**(`PKG_GRID`)라
`ui/table` 작업이 통째로 비켜갔다. 출발점이 「밀도가 튄다」였는데 표 10곳을 14px로 올린 결과
**가장 많이 쓰는 목록만 혼자 작아져 격차가 오히려 벌어졌다.** 시안 R5의 「grid 마크업 유지」가
구조를 뜻하는지 타이포까지인지 불분명 → 디자이너 확인 필요.
벼 단일건 그룹 낱개화는 지연 로딩 때문에 보류(§31).
...
- **뿌리** — 이 앱은 페이지 바탕이 회색(`--background: #f1f5f9`)인데 shadcn `DialogContent`가 그 토큰을 그대로 썼다. 카드 성격이니 `--card`(#ffffff)가 맞다. `dialog.tsx:63`·`alert-dialog.tsx:39` → `bg-card`. `popover`·`dropdown-menu`·`select`는 `bg-popover`(흰색)라 이미 정상이었다.
- **손으로 박아온 `bg-white` 4곳 제거** — `add-packaging`·`stock-list`·`deduct`·`movement-history`. `--card`가 `#ffffff`라 **결과 픽셀은 동일해야 정상**(달라 보이면 그게 회귀 신호). 그 이유를 적어둔 낡은 주석 2곳도 정리했고, grid→flex 사유(푸터 잘림)는 유효해서 남겼다.
- **회색 전제 보정** — `repack-dialog:429` 원본 카드가 「회색 위 흰 카드」였다. 바탕이 희어지면 묻히므로 `bg-slate-50`으로 뒤집었다.

**의도적 제외 2건**
- `SheetContent`는 회색 유지 — 유일한 실사용처 `milling-cart-sheet`가 회색 위 흰 카드 4개를 얹는 **의도된 설계**다. 흰색으로 바꾸면 카드를 전부 뒤집어야 하고 이번 목적과 성격이 다르다.
- 다크모드는 **실사용 없음** — `dark:` 유틸리티 0건, `ThemeProvider`·`next-themes`·`prefers-color-scheme` 없음. `.dark` 팔레트는 정의만 있다. 백로그 §22의 「다크모드도 확인」 항목은 검증 자체가 불가능해 제외했다.

**백로그 4건 정리**
- §22 해소 · **§25 신규** — `button.tsx:16` outline variant도 같은 병(버튼 배경이 회색)이다. `repack-dialog:592`·`deduct-dialog:467`·`movement-history:220`이 손으로 `bg-white`를 되돌린 증거. 다만 **전 앱 버튼 파급**이라 위험도가 달라 분리했다(`switch`·`multi-select` 포함, `calendar`는 `bg-transparent`로 무력화돼 조치 불요).
- **§13 해소 확인** — `getPackages` cutoff가 `lib/package-where.ts`에 **없다**. 검색 필터 확장(`f8228ab`)·기본 생산연도 제거(`77d2703`) 때 정식 필터로 대체되며 사라졌다. `packages.ts:1111` 낡은 주석도 정리.
- **§14 해소** — 비판매 차감 처리는 9/01 재고차감 D1~D7(`95527a7`)이 구현체다.
- **§26 신규** — 변경 파일 린트 8건이 나왔으나 **전부 미변경 줄의 기존 부채**(앱 전체 536건). 그중 6건이 `add-packaging-dialog.tsx`에 있고 이 파일이 §16(모바일 잘림) 대상이라, 어차피 열 때 함께 처리하기로 했다. 표시 계층 PR에 타입 정리를 섞지 않는다.

검증: tsc ✅ · test **200/200** ✅(표시 계층만이라 숫자 불변이 정상) · eslint **신규 0건** · **브라우저 표본 5개 사용자 확인 완료**(재포장/차감·이력/벼 포장·원물목록/폼 다이얼로그/장바구니 Sheet).

---

### 재포장 「쓸 재고」 규격 표기 중복 `fix` `8d7276b`

§22 브라우저 검증 중 사용자가 **「5kg · 5kg 이안규」** 를 발견했다. `{packageType} · {weightPerUnit}kg`로 늘 병기하다 보니 라벨에 이미 무게가 있는 규격에서 겹쳤다.

무조건 지울 수는 없다 — **`톤백`·`잔량`은 라벨만으로 무게를 알 수 없어 병기가 필수**다. `formatSpec()` 헬퍼를 두고 라벨이 단중과 같은 `{n}kg` 꼴일 때만 단중을 생략한다. 규격 `5kg`에 단중 4.8처럼 어긋난 경우는 겹침이 아니라 **실제 불일치**라 그대로 드러낸다.

§22와 성격이 달라 별도 커밋으로 분리(사용자 결정). 검증: tsc ✅ · test 200/200 ✅ · eslint 0.
...
### 원물재고 벼 검색에 중량 필터 `feat` `892b60e`

계획서 [plan-원물재고-중량검색.md](plan/plan-원물재고-중량검색.md) · 보고서 [report-원물재고-중량검색-2026-09-02.md](report/report-원물재고-중량검색-2026-09-02.md).

올해 처음 적용하는 방식이라 **톤백번호가 안 적힌 톤백이 많다** → 번호 대신 무게로 찾아야 한다는 현장 요구. 검색 다이얼로그의 생산자 줄을 반으로 나눠 「중량 (kg)」 칸을 넣었다(사용자 제안).

- **매칭은 정확 일치.** 소수점을 쓰지 않는 운영이라 ±오차/범위는 뺐다. 나중에 오차 허용으로 바꾸려면 `parseWeightFilter` 사용부 한 줄(`where.weightKg = { gte, lte }`)만 고치면 된다.
- 🔴**조회 함수 3곳 모두 적용** — 벼 탭은 「그룹 집계(건수·합계) → 펼치면 항목 조회」로 나뉘어 있어 `getStocks`·`getStockGroups`·`getStocksByGroup` 중 한 곳만 고치면 **그룹 건수와 펼친 목록이 어긋난다.**
- `exportStocks`는 조건을 자체 구성하므로 동일 조건을 별도 반영. 당장 엑셀은 안 쓸 예정이지만 화면 필터와 결과가 달라지는 함정을 남기지 않기 위함.
- 파싱은 콤마 제거 후 `parseFloat`, 숫자가 아니면 필터 미적용. ⚠️`parseWeightFilter`는 **export하지 않는다** — `use server` 파일은 async 함수만 export할 수 있어 엑셀 쪽은 같은 로직을 인라인했다.
- 잡곡 탭은 범위 밖. 생산자 칸이 좁아져 placeholder를 `예: 홍길동, 김영희`로 축약.

검증: tsc ✅ · test 173/173 ✅ · lint **신규 0**(HEAD 원본을 같은 설정으로 린트해도 21건 동일 = 기존 부채). 브라우저 확인은 사용자 몫으로 남김(그룹 건수·합계와 펼친 목록이 함께 걸러지는지).

### 제품재고 검색 필터 확장 + 기본 생산연도 규칙 분리 `feat` `f8228ab`

계획서 [plan-제품재고-검색필터확장.md](plan/plan-제품재고-검색필터확장.md) · 보고서 [report-제품재고-검색필터확장-2026-09-02.md](report/report-제품재고-검색필터확장-2026-09-02.md).

제품재고 검색이 생산연도·품종·출처·정렬 4개뿐이라 재고를 좁힐 수 없었다. 원물재고 탭을 참조해 **포장일자 기간·생산자/농가명·인증구분** 3종 추가.

- **인증은 벼 탭에만 노출**(사용자 확정) — 인증은 `stock → farmer → group.certType` 경로로만 알 수 있는데 **잡곡 매입(PURCHASED)은 stock이 없어 인증 정보가 아예 없다.** 잡곡에 걸면 매입 재고가 통째로 빠져 재고가 사라진 것처럼 보인다.
- **포장일자·생산자는 목록에 보이는 컬럼과 같은 기준으로** 건다. 어긋나면 "보이는 날짜로 검색했는데 안 나온다"가 된다. 포장일자 = MILLED `createdAt` / PURCHASED `incomingDate`(없으면 `createdAt`) 3분기 OR, 종료일은 당일 포함(`lt: 종료일+1일`)·로컬 자정 기준(UTC로 만들면 하루씩 밀린다). 생산자 = `farmer.name`·`actualFarmer`·`purchaseVendor` OR.
- `2026-02-31`처럼 존재하지 않는 날짜를 `Date`가 조용히 3월로 굴리는 것을 되돌려 확인해 막았다.

**신규 `lib/package-where.ts`** — 🔴`getPackages`(화면)와 `exportPackages`(엑셀)가 **where 조립을 두 벌 복붙**해 갖고 있었다. 필터 3개일 땐 버텼지만 6개가 되면 한쪽만 고치는 사고가 확정 — **화면과 엑셀 결과가 조용히 달라지는**, 알아채기 가장 어려운 어긋남이다. ⚠️`packages.ts`는 `'use server'`라 동기 헬퍼를 export 할 수 없어 `lib/`로 빼야만 공유된다(`parseWeightFilter` 때와 같은 제약, 그때는 인라인으로 갔다). 필터 타입의 단일 원천도 옮기고 `packages.ts`가 별칭 재export → 클라이언트 import 경로는 그대로. **1,275 → 1,198줄**.

**신규 `lib/production-year.ts`**(추가 요청) — 같은 「11월 기준」 한 줄이 **7곳에 복붙**돼 있었고, 🔴**벼와 잡곡은 수확철이 다른데 한 규칙을 쓰던 것이 문제의 뿌리**였다.
...
- 벼는 가을 한 번 → 수확기에 두 해를 함께(11월 → **10월**로 앞당김). 잡곡은 여름·가을 두 번, 6월부터 당해년도분 유입 → **항상 2년분**. 잡곡 1~5월에 재작년까지 보는 건 그때 올해분이 아직 없어 [올해,전년]으로 두면 한 해가 늘 0건이기 때문(사용자 확정).
- 등록 폼처럼 한 해만 찍는 곳은 `defaultProductionYear()` 별도(벼 11월/잡곡 6월). **잡곡 입고 등록**이 11월 기준이라 6~10월 입고 때 연도를 매번 손으로 고쳐야 했던 것도 함께 해소.
- 적용 7곳 — 값이 바뀌는 4곳(제품재고·원물 벼·원물 잡곡 검색, 잡곡 입고 등록) + 값은 같지만 단일 원천으로 묶은 3곳(벼 입고 등록, 작목반 `cropYear`, 판매 기간 시작).
- 🔴**`useMemo` 필수** — 기본값이 문자열에서 **배열**로 바뀌었다. 렌더마다 새 참조가 생겨 URL 동기화 `useEffect` 의존성에 그냥 넣으면 **무한 루프**다. 검색 필터 3곳 모두 감쌌다.
- ⚠️원물재고 벼 탭은 **모바일 자동열림 판정**(`val === defaultYear`)에도 기본값 비교가 있어 `join(',')`으로 함께 맞췄다 — 안 고쳤으면 기본 상태인데 필터가 걸린 것으로 오판해 자동열림이 죽는다.

검증: tsc ✅ · test **200/200**(신규 27 = where 조립 17 + 연도 규칙 10) ✅ · lint **신규 0**(연도 적용 7곳은 변경 전후 똑같이 14건 = 기존 `set-state-in-effect` 부채). 브라우저 확인 사용자 완료.

범위 밖: `app/actions/packages.ts`는 1,198줄로 줄었지만 여전히 800줄 초과. 잡곡 포장 CRUD(~450줄)·잡곡 매입 CRUD(~370줄)가 조회와 거의 안 얽혀 있어 떼면 각 400줄 안쪽이 되나, 필터 diff에 400줄 이동이 섞이면 리뷰가 불가능해 **별도 커밋으로 미뤘다.**

### 제품재고 검색 기본 생산연도 제거 — 재검토 `fix` `77d2703`

바로 앞 작업(`f8228ab`)에서 제품재고에도 기본 생산연도를 넣었는데, 사용자 지적으로 되돌렸다. 「작년 재고가 남았는데 목록에 안 보이면 묻히겠는데?」

🔴**판단 근거는 데이터 분포가 아니라 구조다.** 제품재고 목록은 **가용 0인 행이 이미 자동으로 빠진다**(소진분 제외). 그래서 목록에 남은 건 정의상 전부 **실재 재고**이고, 거기에 연도를 기본으로 걸면 **팔아야 할 물건이 숨는다** — 오래된 재고일수록 먼저 나가야 하는데 필터가 정확히 그걸 가린다. 데이터가 쌓일수록 더 위험해진다.

**원물재고는 반대라 그대로 둔다** — 도정하면 `CONSUMED`로 빠지고 작업 대상이 당해년도 위주라 기본값이 실제로 쓸모가 있다. `lib/production-year.ts`는 원물재고 2곳·등록 폼 4곳에서 계속 쓴다.

⚠️판단 과정에서 실 DB를 조회해 「목록 608행이 전부 2025년산」을 확인했지만 **근거로 쓰지 않았다** — 과거 데이터가 다 들어오지 않은 상태라 미래를 말해주지 못한다(사용자 지적). 구조가 결론을 이미 정한다.

검증: tsc ✅ · test 200/200 ✅ · lint 신규 0.
