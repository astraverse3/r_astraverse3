...
> ✅ **종결 2026-09-15** — 최종 형태는 **결정 M**(팝오버는 선택만 · 허용 폭 +10kg 고정 · 쪼개기는 제품재고에서). 브라우저 확인 M1~M5 사용자 완료.
> 결정 E′·H·K·L의 「쪼개기」 서술은 이력이며 코드에 없다. 다음 = **D2e 매칭실패 수동지정**.

- 작성일: 2026-09-14 · 종결일: 2026-09-15
...
→ **결정 M**: 쪼개기는 제품재고 화면에서. 팝오버는 발주 자루중량 ~ +10kg 안의 자루를 오래된 순으로 기본 체크해 줄 뿐이다.
...
### D5. ~~쪼갤 몫 조절 + 가위 삭제 (결정 K) · 발주 자루 단위 추천 (결정 L)~~ → **결정 M 리셋** ✅ **구현 완료 `e0a3ec6` · 브라우저 확인 완료(§7 M1~M5, 2026-09-15)** — 선택만, 쪼개기 삭제, +10kg 고정.
같은 날 곁가지: 열 부족 판정 남은 주문 기준 `429a58d` · 브레드크럼 서브컨텍스트 링크 `ec61402` · 「← 제품판매」 링크 삭제 `f724a20`
...
✅ **결정 M 이후 브라우저 확인 — 사용자 완료 2026-09-15**(아래 1~7 중 3·3′·3″은 폐기). M1에서 열 부족 판정 결함 발견 → `429a58d`.
M3·M4는 지금 데이터(발주처 하나·열마다 맞는 자루 하나)로는 재현이 안 돼 **실제 부족 상황이 나올 때** 본다.
