...
### 결정 E — 톤백 셀은 자루 목록, 추천 없음, 정렬은 요구중량 근접순

- 후보 = 그 SKU의 가용 자루 행(`available > 0`). 표시: **자루중량 · 차이(+4 / −15) · 로트 · 생산자 · 도정/입고일 · 가용 N자루**
- 정렬 = `|자루중량 − 남은 요구중량|` 오름차순, 동률이면 오래된 순(FIFO)
- 기본 선택 **없음**. 사람이 체크한다(#34 「사람이 판단한다」). 체크하면 합계 kg과 요구 kg 차이가 위에 뜬다
- `count>1` 행은 자루 수를 stepper로(1~가용)
...
- 브라우저(`/sales/purchase/19` 시아스):
  ...
  3. **쪼개기**: 1,000kg 셀에서 1,014 자루 「쪼개기」 → 1,000 입력 → 확인 다이얼로그 → 새 1,000 자루가 체크된 채 목록 갱신 → 확정 → `/packages`에 **14kg 톤백** 남는지
