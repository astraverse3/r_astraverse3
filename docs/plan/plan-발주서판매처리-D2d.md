...
### 결정 M — 🔴 **리셋(2026-09-15 사용자)**: 팝오버는 **선택만**, 쪼개기는 제품재고에서 · 허용 폭 **+10kg 고정**

K·L을 거치며 「실업무를 완벽히 커버할 수도 없고 너무 복잡해진다」 → 방침을 처음부터 리셋.
- 톤백 셀 팝오버 = 자루 목록(FIFO) + 체크 + 확정. **쪼개기 없음**(`createRepack` 호출·확인 다이얼로그·±조절 전부 삭제).
  자루를 맞춰야 하면 제품재고 화면에서 쪼개고 다시 연다
- **허용 폭 = +10kg 고정**(`BULK_FIT_KG`): 발주 자루중량 ≤ 재고 자루중량 ≤ 발주 자루중량 + 10. 1,000은 1,000~1,010, 200은 200~210.
  (중간에 나온 「1%」·「max(3kg, 1%)」는 모두 폐기)
- 추천 = `suggestBulkWhole(unitKg, units, bags)`: FIFO로 `fitsUnit` 자루를 남은 발주 수만큼 체크. 맞는 자루가 모자라면 `shortUnits`
- 모자라면 아무것도 추가로 체크하지 않고 **「맞는 자루 N자루 부족 — 제품재고에서 쪼개서 맞춘 뒤 다시 여세요」** + 제품재고 링크.
  사람이 587+450(1,037) 같은 걸 체크해 확정하는 건 막지 않는다(결정 F 그대로, 요약은 주황)
- 요약 「요구 · 실제 · 차이」의 초록/주황도 같은 폭: `bulkDelta(requiredKg, actualKg, toleranceKg)`, 폭 = 발주 자루 수 × 10.
  `BULK_TOLERANCE`(1%)는 삭제 — 폭이 둘이면 색과 추천이 어긋난다
- 맞는 자루 행에 작은 「맞음」 표시(고르기 쉽게)
- `uploadId` prop은 재포장 비고에만 쓰였으므로 팝오버 껍데기·`matrix-client`에서 제거
- lib에서 삭제: `suggestBulkAllocation`·`bulkTargetKg`·`suggestBulkUnits`·`BULK_TARE_KG`·`BULK_TOLERANCE`·`BulkSuggestion`. 테스트도 그에 맞게
...
### D5. ~~쪼갤 몫 조절 + 가위 삭제 (결정 K) · 발주 자루 단위 추천 (결정 L)~~ → **결정 M 리셋** ✅ **구현 완료** · 🔴 **브라우저 미확인**(§7 M) — 선택만, 쪼개기 삭제, +10kg 고정
...
🔴 **결정 M 이후 유효한 브라우저 확인**(아래 1~7 중 3·3′·3″은 폐기):
- M1. 시아스 1,000kg 셀 → **1,005(8/10) 하나만 「맞음」 배지 + 기본 체크**, 587·332·450·1,014는 배지 없음 · 요약 +5(+0.5%) **초록** · 확정 → 완료 · 취소 → 복원
- M2. 200kg 셀 → 203 「맞음」·기본 체크 · 요약 +3 초록
- M3. 맞는 자루가 모자란 셀(1,000×2 등) → 주황 안내 「맞는 자루가 N자루 부족 — 제품재고에서…」 + 링크가 새 탭으로 `/packages`
- M4. 안내가 떠도 587+450 체크 → 확정이 **막히지 않고**(결정 F) 요약 +37 주황
- M5. 일반 규격 셀 회귀 · 차감 직후 숫자 == F5 후 숫자
...
- 브라우저(`/sales/purchase/19` 시아스):
  ...
  3″. **자루 단위**(결정 L): 시아스 1,000kg 셀 그대로 열면 **1,005(8/10) 하나만 체크**·쪼개기 없음·요약 +5(+0.5%) 초록. 200kg 셀은 203이 1% 밖(202까지)이라 풀로 → 목표 203이 203 자루에 딱 맞아 **통째**(쪼개기 없음) · 요약 +3(+1.5%) **주황** ← ⚠️ 작은 자루는 +3이 1%를 넘어 늘 주황. 여유분을 min(3, 1%)로 할지 사용자 결정 대기
