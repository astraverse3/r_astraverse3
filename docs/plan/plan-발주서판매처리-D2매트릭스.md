...
| 톤백 실태 | **204행 중 가용 62행**(2026-09-09 재실측 — 6월말 일괄차감 반영. 계획서 최초의 199행은 낡은 값) / 자루중량 **124종** / 100kg 배수 **29행** — 「1,000kg 주문에 1,004kg 자루」가 기본값 |
...
- 🔴 **「요구 vs 실제」 차이 계산은 `lib/`의 순수 함수로 뺀다** — 차감 후에도 그 차이를 봐야 한다는
  요구가 백로그 **§40**으로 잡혀 있고(D5 또는 판매 목록), **같은 계산**이라 재사용한다.
  막지는 않는다 — #34의 「사람이 판단한다」를 뒤집지 않고 **보이게만** 한다.
- 검증: `pkg#1277`(톤백 ×4, 자루당 1,004kg, 포장일 2026-08-19, **가용 4 — 2026-09-09 생존 확인**)
  → 1,000+4 분할 → 차감 → `/packages`에서 3자루+1,000+4 확인
...
| 중 | 분할이 재고 데이터를 직접 쪼갠다 | 재포장 R1이 가용 검증·트랜잭션을 갖췄다 (결정 #43). 🔴 **단 `cancelRepack`은 액션만 있고 화면 진입점이 없다**(2026-09-09 확인 — 어디서도 호출하지 않는다). 되돌리기 UI는 기각된 상태이므로 **잘못 쪼갰을 때의 정식 경로는 역방향 재포장뿐이다.** D2d 팝오버는 이 점을 전제로 「쪼개기 전 확인」을 분명히 해야 한다 |
