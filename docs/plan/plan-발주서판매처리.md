...
### 2.2 통일 양식 (2026-06-26 결정 #26 — 구 §2.1 양식 대체)

구 §2.1은 `docs/resources/발주서.xlsx`(2채널, DELIVERY/EMART)의 실측이고, 현장엔 그 외 급식·기업별 채널이 **제각각 양식**으로 존재(`#01. 행복플러스`·`#01. 공장 발주서`·`공장출고지시서`·`2026.06.발주서`). 파싱 분기를 없애기 위해 **전 채널 단일 양식**으로 통일한다.

- **채널 5종**: `EMART` `DELIVERY` `MEAL_SEOUL` `MEAL_HAENAM` `CORPORATE`
- **채널 판별 = 시트명 prefix `XXX_YYMMDD`**:

  | 채널 | 시트명 prefix | 발주처(vendor) | 수령인(recipient) |
  |---|---|---|---|
  | 이마트 | `이마트_YYMMDD` | `이마트` 고정 | 필수(여주/대구/시화) |
  | 택배 | `택배_YYMMDD` | 주문자 | 빈칸→발주처 복사 |
  | 서울급식 | `서울급식_YYMMDD` | 구청(은평구 등) | 필수(행복플러스) |
  | 해남급식 | `해남급식_YYMMDD` | `해남급식` 고정 | 필수(배송업체) |
  | 기업별 | `거래처명_YYMMDD`(fallback) | 거래처 | 빈칸→발주처 복사 |

  - 앞 4개 prefix는 **고정 화이트리스트**, 그 외 모든 시트명은 `CORPORATE`로 처리(거래처명을 prefix로 자유 사용 → 거래처 추가 시 코드 무수정).
  - **"수령인 빈칸=발주처 복사"는 택배·기업별만**. 이마트·급식 2종은 발주처≠수령인이라 둘 다 채운다.
  - 안전장치: CORPORATE로 분류됐는데 발주처 셀·날짜가 비면 **'미인식 시트' 경고**(오타 방지, 차단 아님). 날짜는 `_YYMMDD` + 본문 셀 교차확인.
- **헤더 3행 구조**(구 양식의 농가명 행 제거): 품종+도정(한 셀 묶음) / 포장지(행) / 규격·중량(행). 품종+도정은 파서가 정규화·별칭으로 분리(#23).
- ⚠️ 구 양식 기준 단계2 파서(`lib/purchase-order-parser.ts`, 13/13 통과)는 본 결정으로 **재작업 필요**(채널 판별·발주처/수령인 좌표·헤더 행수).
...
| 26 | **발주 양식 통일 + 채널 5종** | **현장의 채널별 제각각 발주서를 파싱 친화적 단일 양식으로 통일**(2026-06-26 결정, §2.2). 기존 파일은 채널마다 발주처/수령인 표기가 달라(이마트=`여주(이마트)` 한 셀 뭉침 / 택배·급식=`(발주처)`/`(수령인)` 2열 라벨 / 기업별=업체명 한 줄) 파서 분기가 폭발 → 전 채널 동일 좌표·동일 헤더로 재제정. **(a) 채널 5종**: `EMART`(이마트)·`DELIVERY`(택배)·`MEAL_SEOUL`(서울급식=행복플러스 경유 학교급식)·`MEAL_HAENAM`(해남급식=해남군 공공급식)·`CORPORATE`(기업별). **(b) 시트명 prefix `XXX_YYMMDD`로 채널 판별**: `이마트_`/`택배_`/`서울급식_`/`해남급식_` 4개 **고정 화이트리스트** + 그 외 모든 시트명(거래처명을 자유 prefix로, 예 `광천김_260619`·`노벨_260619`)은 **`CORPORATE` fallback**. → 거래처 추가 시 코드 수정 불필요. **(c) 발주처(vendor)/수령인(recipient) 2열 좌표 고정**, 채널별 의미: EMART=vendor`이마트`고정·recipient=물류센터(여주/대구/시화) / DELIVERY=vendor=주문자·recipient **빈칸→vendor 복사** / MEAL_SEOUL=vendor=구청(은평구 등)·recipient=행복플러스 / MEAL_HAENAM=vendor`해남급식`고정·recipient=배송업체(싱싱유통 등) / CORPORATE=vendor=거래처·recipient **빈칸→vendor 복사**. **"수령인 빈칸=발주처 복사"는 택배·기업별(동일주체)만** — 이마트·급식 2종은 발주처≠수령인이라 둘 다 필수 기입. DB `recipient`는 파서가 복사하므로 NOT NULL 유지(분기 없음). **(d) 헤더 3행 구조**: 품종+도정(한 셀 묶음, 파서가 정규화·별칭으로 분리 #23) / 포장지(별도 행) / 규격·중량(별도 행). 구 양식의 농가명 행 제거. **(e) 안전장치**: CORPORATE fallback에 잘못 빨려든 오타 시트 방지 → 발주처 셀·날짜가 비면 '미인식 시트' 경고(차단 아님). 날짜는 시트명 `_YYMMDD` + 본문 셀 교차확인. **⚠️ 영향**: 구 양식(§2.1, 2채널) 기준으로 구현된 단계2 파서(`lib/purchase-order-parser.ts`, 13/13 통과)는 통일 양식 확정에 따라 **재작업 필요**(채널 판별·좌표·헤더 행수 변경). 2026-06-26 결정 |
...
enum PurchaseChannel { DELIVERY  EMART  MEAL_SEOUL  MEAL_HAENAM  CORPORATE }  // 채널 5종(#26)
...
파서 흐름(시트별 반복):
1. **channel 판별**(통일 양식 #26): 시트명 prefix `XXX_YYMMDD` 매칭 — `이마트_`→`EMART`, `택배_`→`DELIVERY`, `서울급식_`→`MEAL_SEOUL`, `해남급식_`→`MEAL_HAENAM`, 그 외 전부→`CORPORATE`(fallback). 시트명 끝 `_YYMMDD`로 발주일 1차 추출(본문 셀과 교차확인). CORPORATE인데 발주처·날짜 비면 '미인식 시트' 경고. *(구 양식: 시트명에 '이마트' 포함 여부로 2분기 — #26으로 대체)*
...
4. **데이터 행 순회**(소계행 무시): 각 행 = 1 `PurchaseOrder`(vendor=A열, recipient=B열). **수령인(B열) 빈칸이면 vendor 복사**(택배·기업별 동일주체, #26) → DB `recipient`는 항상 값. 이마트·급식은 B열 필수. 행×규격열 교차 셀의 값(수량)이 있으면 1 `PurchaseOrderItem`.
...
3. ✅ **차감 공용 액션**(2026-06-25): `app/actions/package-movement.ts`(createSale·createNonSaleMovement·cancelMovement·listMovements, 가용 재검증+동시성 사후검증 보강) + `getPackages` cutoff 제거·`available=count-SUM(movement)` 동봉·available=0 행 제외(백로그§13 종료). 권한 OPERATION_MANAGE·list는 공개. ⚠️cancelMovement는 발주서 라인(orderItemId≠null) 차감은 차단(발주서 흐름서 취소). tsc/eslint 통과(기존 any 4개는 pre-existing). 실동작 왕복은 화면 단계6에서.
4. 🔄 **발주서 액션**(2026-06-25, export 제외 완료): `lib/purchase-order-allocation.ts`(순수: FIFO `suggestAllocation`·`computeLineStatus`·`computeOrderStatus`·중복감지 `orderDuplicateKey`/`detectDuplicateOrders` + node:test 8케이스) + `app/actions/purchase-order.ts`(747줄): `uploadPurchaseOrder`(파싱→중복감지#16→트랜잭션 적재→라인 자동매칭) · `listPurchaseUploads`/`listPurchaseOrders`/`getPurchaseOrderDetail`(라인별 매칭결과·차감현황·가용·FIFO추천·부족분) · `autoMatchOrderItem`/`setOrderItemProductType`(수동지정+alias학습#22) · `confirmOrderItem`/`confirmOrder`(FIFO 차감, 가용재검증, status 파생) · `cancelOrderItemMovements`(하드삭제+복원) · `deletePurchaseUpload`/`deletePurchaseOrder`(movement 있으면 차단). tsc/eslint/test 통과. **⏭️ `exportPurchaseOrders`(원본 매트릭스 양식 복원+생산자·로트 채움)는 별도 파일 `purchase-order-export.ts`로 분리 예정**(현 파일 747줄, 800 제한).
5. ✅ **권한 등록 + /sales 2탭 골격**(2026-06-25): `permission-matrix.md` 발주서/차감 "(향후)" 제거→실 함수명(전부 `OPERATION_MANAGE`/조회 공개/export requireSession 예정). `/sales` 탭 placeholder(rice·misc ComingSoonPanel) **제거**→**제품판매(product)/원물출고(release) 2탭**(결정#13): `sales-tabs.tsx`(2탭·grid-cols-2·기본탭 `DEFAULT_SALES_TAB='product'`)·`page.tsx`(분기·VALID_TABS)·`product-sales-section.tsx` 신규(실 `listPurchaseUploads` 연결 묶음목록 골격, 빈상태 안내. 업로드 버튼은 시각만=단계6 기능연결). coming-soon-panel.tsx 삭제. tsc/eslint 통과.
