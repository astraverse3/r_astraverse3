...
동질성(품종+도정유형+출처)으로 묶으면 **병합 가능한 잔량 그룹이 9개**다.

| 그룹 | 행수 | 합계 | 로트 종수 |
|---|---:|---:|---:|
| 천지향1세 · 백미 | 24 | 210kg | 16 |
| 하이아미 · 백미 | 20 | 138kg | 10 |
| 서농22호 · 백미 | 12 | 4,894kg | 10 |
| 새청무 · 백미 | 9 | 1,171kg | 8 |
| 백옥찰 · 백미 | 9 | 123kg | 4 |

**로트가 그룹마다 4~16종으로 갈린다.** 결과 줄마다 로트를 고르게 한 §3.3이 예외 처리가 아니라
기본 동선이라는 뜻이다.

### 2.2.1 ⚠️ 같은 로트번호에 품종이 섞여 있다 (2026-08-26 발견)

처음에는 「같은 로트 = 같은 품종」으로 보고 로트 `251119-11-15100914-391`의 잔량 10행(84kg)을
검증 시나리오로 삼았다. **실제로는 그 10행이 하이아미 8행 + 서농22호 2행이었다.**

```
#6 5kg   하이아미      #201 2kg  서농22호
#175 17kg 하이아미     #221 3kg  서농22호
#185 25kg 하이아미
#196 1kg  하이아미  … (모두 농가 윤병옥)
```

로트번호는 `입고일-제품코드-인증번호-농가그룹+농가번호`로 만들어지고 **품종은 코드에 반영되지 않는다**
(일반 우루치는 전부 제품코드 `11`). 같은 농가가 같은 날 들여온 서로 다른 품종은 로트번호가 같아진다.

→ **동질성을 품종 기준으로 판정한 §3.2가 옳았다.** 로트 기준으로 묶었다면 하이아미와 서농22호를
섞어 포장할 뻔했다. 이 케이스는 `validateRepack`이 `MIXED_VARIETY`로 실제 차단하는 것을 확인했다.
...
### R2-후속 — UI 개선 (2026-08-27 완료)

세부는 **`docs/plan/plan-재고재포장-R2후속UI.md`** 참조 (결정 #44~#49).

⚠️ 「폭 줄이기」는 **다이얼로그가 아니라 목록 하단 선택 바**를 가리킨 것이었다(2026-08-27 정정).
다이얼로그 폭은 2열→1열 개편의 결과로 940 → 720px가 됐다.

- #44 좌측 「쓸 재고」 패널 → 상단 접이식 요약 · 1열 세로 흐름
- #45 규격 버튼이 곧 「줄 추가」 (포장 입력 `addToGroup` 패턴) · 「줄 추가」 버튼 폐기
- #46 결과 줄 = 1행 압축 그리드 · 개수 `⊖ n ⊕` stepper
- #47 로트는 행마다 셀렉트 유지하되 **직전 줄에서 상속** (섹션 분리는 로트 16종에서 무너져 기각)
- #48 포장지 기본값 `suggestProductType` 낙관적 채움
- #49 목록 선택 바 = 내용 폭 + 우측 정렬

서버 액션·`lib/repack.ts`·테스트 무변경(84/84 그대로).

### ~~R3. 재포장 이력 표시~~ → **R3. 정합성 수정** (2026-08-27 완료)

**원안(배지 + 되돌리기 진입점)은 기각했다.** 세부는 `docs/plan/plan-재고재포장-R3.md` (결정 #57~#60).

- **#57 되돌리기·배지 기각** — 역방향 재포장(결과를 다시 소스로 골라 합치기)이 정식 우회로다.
  `getRepackSources`가 `repackId`를 보지 않아 재포장 결과도 그냥 소스가 된다.
  게다가 **재포장의 재포장은 정상 동선**(톤백→20kg→5kg)이라, 두 번째 재포장 순간 첫 재포장은
  되돌릴 수 없게 된다 — `cancelRepack`이 유효한 창 자체가 짧다. 빈도도 낮다(실수·착오뿐).
  `cancelRepack` 액션은 **의도적 미연결**로 남긴다(지우지 말 것)
- 🔴 **#58 생산 통계 이중 계상** — 결과 행이 `batchId`를 승계하는데(§3.4) 생산 집계가
  `repackId`를 안 봐서 **같은 쌀이 두 번 세어지고 있었다**. 실측 85kg.
  `output-statistics.ts`·`dashboard.ts`(수율 분자)에 `repackId: null` 추가
- **#59 재포장 결과 행 일반 삭제 차단** — `deleteMiscPackage`/`deleteMiscPurchase`가 결과만 지우고
  원본의 REPACK movement를 남겨 **재고를 증발시킬 수 있었다**. 실질 위험은 낮으나(잡곡 재고 1행,
  벼는 삭제 기능 없음) 조건 한 줄이라 막았다
- **#60 되돌리기 거부 메시지 부정확** — 화면이 없어 보류. 붙이는 날 함께 고친다

**「~~규모가 작으면 R2에 합쳐서 끝낸다~~」는 무의미해졌다** — 원안 자체가 없어졌다.
...
- 실물 검증 시나리오(사용자 브라우저) — **§2.2.1 발견 반영해 갱신**:
  1. **병합(단일 로트)** — 하이아미 백미 잔량 중 로트 `251119-…-391` 8행(79kg) → 20kg ×3 + 잔량 19kg
  2. **병합(여러 로트)** — 하이아미 백미 잔량 20행(138kg, 로트 10종) → 20kg ×6 + 잔량 18kg.
     결과 줄마다 로트를 골라야 저장된다(§3.3)
  3. **규격변경** — 잔량 4kg ×1 → 1kg ×4
  4. **분할** — 톤백 #1277(1,004kg ×4) → 1자루를 1,000 + 4로
  5. **되돌리기** — 위 한 건을 `cancelRepack` → 소스 가용 복원 확인
  6. **차단 확인** — 하이아미와 서농22호 잔량을 함께 고르면 체크박스가 비활성(§2.2.1)
