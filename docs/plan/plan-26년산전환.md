...
사용자 지적: **작목반 기본 검색 필터가 「전체」라 제한이 필요하다.**

🔴 **이 화면은 필터 위젯 기본값만으로는 안 걸린다.** 조회는 URL만 보기 때문에(`page.tsx`), 메뉴로 그냥 들어오면 전 연도가 쏟아진다. **서버에서 기본값을 건다.**

```
/admin/farmers               → 2026·2025년산
/admin/farmers?cropYear=2024 → 2024년산
/admin/farmers?cropYear=ALL  → 전체
```

⚠️ **기본값을 한 번 잘못 잡았다가 실측으로 고쳤다.**

| 후보 | 결과 |
| --- | --- |
| 당해년도만(2026) | 생산자 179명 중 **1명** — 화면이 빈다 |
| **최근 2년(`defaultProductionYears`)** | 작목반 71/75개 · 생산자 **163/179명** |

26년산 작목반이 **1개뿐**이다. 신곡 작목반은 수확기에 하나씩 만들어지므로, 당해년도만 걸면 4단계에서 고친 대시보드와 똑같이 빈 화면이 된다. 📌 **「기본 연도」를 새로 거는 곳마다 실제 행 수를 세어볼 것.**

- `cropYear=ALL`이 **명시적 전체 탈출구**다(원래 `page.tsx`에 처리가 이미 있었다). 필터에서 연도를 모두 해제하면 `ALL`을 보낸다 — 파라미터를 생략하면 서버가 기본값을 다시 끼워 넣어 전체를 볼 방법이 사라진다
- 위젯 기본값도 서버와 **같은 값**으로 맞췄다. 어긋나면 검색창은 「전체」라고 말하는데 목록은 두 해만 나온다

14. ✅ `page.tsx` 서버 기본값 + `farmer-filters.tsx` 위젯/초기화/ALL 전송 · `tsc` 0 · **test 401/401**
15. ⬜ 브라우저 확인 → 푸시

### 📌 앞선 진단 정정 (2026-09-21)

1단계 조사에서 **「26년산을 등록해도 기본 목록에서 안 보인다」고 했는데 과한 표현이었다.** `raw-stocks/page.tsx`는 URL에 `productionYear`가 없으면 **필터 없이 전체를 조회**하고, 메뉴 링크(`/raw-stocks`)에도 파라미터가 없다. 메뉴로 들어가면 26년산은 보였다.

`defaultProductionYears`는 **필터 다이얼로그의 기본 선택값**이고, 목록이 실제로 걸러지는 건 「적용」·「초기화」로 URL에 연도가 박힌 뒤다(그때는 `[2025]`라 26년산이 빠졌다 — 2단계 수정 자체는 유효). 📌 **「기본 필터」를 말할 때 위젯 기본값과 서버 조회 조건을 구분할 것** — 5단계는 이 구분 때문에 서버에서 걸었다.
