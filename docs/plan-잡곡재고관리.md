...
| --- | --- | --- | --- |
| **위탁도정(CONSIGNMENT)** — 원물을 외부 도정업체에 위탁해 입고 | ✅ 경유 | `CONSIGNMENT` | 원물재고 > 잡곡 탭 > [+ 입고] |
...
### 잡곡의 도정/포장 특징
- 도정 과정은 **입고 시점에 이미 완료**되어 있음 (위탁도정이든 농가도정이든)
- 남는 작업은 "포장"뿐 → 벼처럼 "도정관리" 페이지에 통합하지 않음
- 잡곡 포장은 **제품재고 > 잡곡 탭**에서 시작 (원물재고 로트 선택 → 포장단위/개수 입력 → 제품재고 등록)

### 용어 통일
- **잡곡 입고 유형**: "위탁도정" / "농가도정" — UI, 다이얼로그 토글, 필터, 테이블 헤더 모두 동일 표기
...
- `SourceType` (Stock용, 잡곡 입고 유형): `CONSIGNMENT` | `FARMER_MILLED` *(PURCHASED 제거)*
...
- **위탁도정·농가도정**: 실제 생산자 작목반/농가번호로 생성 (벼와 동일 방식)
...
#### 잡곡 입고 등록 다이얼로그
- 상단에 **sourceType 세그먼트 토글** (위탁도정 / 농가도정) — **매입은 여기 없음**
- 유형별 필드 가시성
  - **위탁도정**: 생산자, 품종(MISC_GRAIN만), 원물중량, 위탁 도정업체명, 입고중량(=도정 후 생산량), 자동 로트번호, 수율 자동계산
...
2. **용어 통일** — 잡곡 입고는 "위탁도정/농가도정", 제품 출처는 "도정산/매입"
