...
/** 추천 여유분(kg). 요구 자루 수와 무관하게 **한 번만** 더한다(결정 K, 2026-09-15 사용자: 포장 때 발주량 +3~5로 맞춘다). 사람이 ±1kg로 조절할 수 있다 */
export const BULK_TARE_KG = 3

/** 추천 목표 kg = 남은 요구 kg + 여유분(고정). 남은 요구가 0이면 0 */
export function bulkTargetKg(remainingKg: number): number {
  if (remainingKg <= 0) return 0
  return Math.round((remainingKg + BULK_TARE_KG) * 1000) / 1000
}
