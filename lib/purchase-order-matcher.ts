 ...
 * 실패했다(2026-09-17 실측, #15 택배 1라인). 접두는 3종뿐이고 매번 짧아지므로
 * 반복 비용·종료 모두 문제없다.
 *
 * 🔴 **남은 게 접두 하나뿐이면 떼지 않는다.** 브랜드명이 곧 품종 별칭인 경우가 있다 —
 * `프로틴 라이스`=IPS · `자스민 라이스`=CJ6. 발주서에 품종명 없이 브랜드만 적혀 올 수 있는데
 * (사용자 확인 2026-09-17), 여기서 빈 문자열로 만들면 **별칭을 볼 기회 자체가 사라진다.**
 * 「유기농 프로틴 라이스」 → 「프로틴 라이스」로 남겨야 `Variety.aliases`가 받아준다.
 * 조합형(「유기농 프로틴 라이스 IPS」)은 접두가 다 떨어져 `IPS`가 되므로 영향 없다.
 *
 ...
 *
 * ⚠️ 접두 목록에서 브랜드를 빼면 안 된다 — 별칭은 **정확일치**라
 * `프로틴 라이스` ≠ `프로틴 라이스 IPS`가 되어 조합형이 깨진다.
 */
