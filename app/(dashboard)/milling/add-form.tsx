            ...
            <div className="flex-none lg:bg-white lg:border lg:rounded-xl lg:shadow-sm lg:overflow-hidden flex flex-col h-full lg:h-auto">
                ...
                <div className="flex-none p-4 pb-0 lg:p-6 lg:pb-0 space-y-4 bg-white z-10">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <Button
                            className="h-12 px-6 text-lg font-bold bg-primary hover:bg-primary rounded-xl shadow-lg shadow-primary/30 transition-transform active:scale-[0.98]"
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            className="h-9 rounded-lg border-slate-200 focus:ring-primary text-sm"
                        />
                    </div>
                    ...
                    <div className="space-y-1">
                        {availableStocks.map(stock => {
                            ...
                            return (
                                <div
                                    key={stock.id}
                                    className={`group flex items-center gap-2 py-2 px-2 border rounded-lg cursor-pointer transition-colors ${selectedStockIds.includes(stock.id) ? 'bg-primary/10 border-primary' : 'bg-white border-slate-100 hover:bg-slate-50 hover:border-slate-300'}`}
                                    onClick={() => toggleStock(stock.id)}
                                >
                                    {/* Checkbox */}
                                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors flex-shrink-0 ${selectedStockIds.includes(stock.id) ? 'bg-primary border-primary' : 'bg-white border-slate-300'}`}>
