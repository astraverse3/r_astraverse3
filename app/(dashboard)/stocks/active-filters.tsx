...
export function ActiveStockFilters() {
    ...
    return (
        <div className="flex gap-2 overflow-x-auto py-1 px-1 scrollbar-hide justify-end">
            ...
            {cert !== 'ALL' && <Badge variant="outline" className="whitespace-nowrap bg-transparent text-slate-500 border-slate-200 font-normal">{cert}</Badge>}
            {status !== 'ALL' && <Badge variant="outline" className="whitespace-nowrap bg-transparent text-slate-500 border-slate-200 font-normal">{status === 'AVAILABLE' ? '보관중' : '소진됨'}</Badge>}
        </div>
    )
}
