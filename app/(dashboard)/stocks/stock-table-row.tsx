...
export function StockTableRow({ stock }: Props) {
    ...
    return (
        <>
            ...
            >
                {/* 1. Year */}
                <TableCell className="py-2 px-0.5 text-center text-[10px] font-medium text-slate-500 w-[24px]">
                    {stock.productionYear.toString().slice(-2)}
                </TableCell>

                {/* 2. Variety */}
                <TableCell className="py-2 px-0.5 text-xs font-bold text-slate-800">
                    <div className="truncate max-w-[50px] sm:max-w-none" title={stock.variety}>
                        {stock.variety}
                    </div>
                </TableCell>

                {/* 3. Farmer */}
                <TableCell className="py-2 px-0.5 text-[11px] text-slate-600">
                    <div className="truncate max-w-[40px] sm:max-w-none" title={stock.farmerName}>
                        {stock.farmerName}
                    </div>
                </TableCell>

                {/* 4. Cert */}
                <TableCell className="py-2 px-0.5 text-center">
                    <span className={`text-[10px] font-bold px-1 py-0.5 rounded-md border ${stock.certType === '유기농' ? 'text-green-600 border-green-200 bg-green-50' :
                        stock.certType === '무농약' ? 'text-blue-600 border-blue-200 bg-blue-50' :
                            'text-slate-500 border-slate-200 bg-slate-50'
                        }`}>
                        {stock.certType.substring(0, 2)}
                    </span>
                </TableCell>

                {/* 5. Bag No */}
                <TableCell className="py-2 px-0.5 text-right text-[11px] font-mono text-slate-400">
                    #{stock.bagNo}
                </TableCell>
