...
export function MillingCartSheet({ onStartMilling }: Props) {
    const { items, removeFromCart, clearCart, isOpen, setIsOpen } = useMillingCart()
            ...
            <SheetContent className="w-full sm:max-w-md flex flex-col p-0 gap-0">
                <SheetHeader className="p-6 pb-2 border-b border-slate-100">
                    ...
                    <div className="text-xs text-slate-500 font-medium">
                        여러 검색 결과에서 톤백을 모아 한 번에 도정할 수 있습니다.
                    </div>
                </SheetHeader>
                ...
                <SheetFooter className="p-4 border-t border-slate-100 bg-white">
                    <Button
                        className="w-full h-11 text-base font-bold bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200"
                        disabled={items.length === 0}
                        onClick={handleStart}
                    >
                        {items.length}개 톤백 도정
                    </Button>
                </SheetFooter>
            </SheetContent>
        </Sheet>
